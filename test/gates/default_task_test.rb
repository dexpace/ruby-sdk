...
class DefaultTaskTest < GateCase
  # Phase 0's seventeen, 7b's gates:serde_boundary, phase 9's three repository-wide invariant
  # scans and phase 10's three -- gates:ledger_audit, gates:spdx_rbs, gates:sole_parse --
  # twenty-four. A gate outside DEFAULT_GATES is off `task default:` and therefore
  ...
  EXPECTED = %w[
    ...
    gates:seam_names gates:ledger_audit gates:spdx_rbs gates:sole_parse gates:clean_bundle
    gates:rbs_surface
    gates:sig_diff gates:surface_snapshot gates:single_instance gates:versions
    gates:reproducible yard bundler_audit
  ].freeze

  # The COUNT, asserted against the repository and not only against the literal above: the six
  # tasks phases 9 and 10 added are gates because they are in `task default:`, and a count read off
  # EXPECTED alone would still pass if all six left DEFAULT_GATES together.
  test "there are twenty-four gates, and every one of them is blocking" do
    prerequisites = `bundle exec rake -s -P`.split("\n").drop_while { |l| l != "rake default" }
      .drop(1).take_while { |line| line.start_with?("    ") }.map(&:strip)

    assert_equal(24, EXPECTED.size)
    assert_equal(24, prerequisites.size, "the default task's prerequisite count drifted")
    %w[gates:cause_walk gates:bounded_map gates:seam_names gates:ledger_audit gates:spdx_rbs
       gates:sole_parse].each do |gate|
