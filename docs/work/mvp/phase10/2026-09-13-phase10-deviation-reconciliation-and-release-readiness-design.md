...
| P10-27 | **`HTTP-10` repaired, a MUST.** `Dexpace::Status` is total over every Integer -- only a non-Integer, which is not a code, is refused -- and the protocol's 100-599 range moved to `Status#standard?`. (The first cut stopped at 0..999, "every code a status line can carry", and still threw on 1000 and -1: a port reading narrower than the MUST, recorded in no ledger row; review round 0's R0-5 widened it.) Phase 1 recorded the old guard as "the port's reading" in its plan and checklist and in no ledger row, so by the maintainer's rule it was a defect: a 600 or LinkedIn's 999 made both transports raise where `TRANSPORT-24` says to surface it. Two phase-1 pins and two phase-8 adaptation-failure tests changed with it (the latter now use an `HTTP/1.2` head) | `HTTP-10`, `HTTP-11`, `TRANSPORT-24`, `TRANSPORT-22` | Additive: `NFR-4` permits the widening; the manifest gains `Dexpace::Status#standard?` |
...
| P10-31 | The `chapters` probe check exempts phase 10's own design, plan and checklist (they quote the pre-correction attributions on purpose) and spells its negation vocabulary as a `Regexp.union` of phrases, never an `/x` pattern -- under `/x` the phrases' spaces vanish, which the first live run showed as sixty false fires. Measured on the live tree: two fires, both true positives, in `phase8/2026-09-11-phase8-segmentation-design.md:60` (`SEAM-13` and `SEAM-15` inside a range attributed to chapter 03), fixed in place with a dated bracket **Amended by review round 0 (R0-8):** the first cut also listed `appears in` as a negation to hide the phase-5 segmentation design's :538-539, whose second ID run bound backward to the first run's chapter (40 false fires) -- which silenced the usual positive attribution, `X appears in <chapter>`. The phrase is now a FORWARD binding instead: an ID run followed by it pairs with the chapter it introduces, or with nothing when that chapter is on the next line (the continued-clause gap). Live fires are unchanged. | `SEAM-13`, `SEAM-15`; `R10`, addendum A10 | The design measured the population before phase 10's documents joined it |
...
| P10-37 | **`docs/deviations.md` row 10 is *confirmed, narrower*, and the amendment set gains `C19`** (review round 0's R0-6). §4 and §10 item 10 say `send(:new, …)` reaches the generated constructor; as built it bypasses `.build` but runs the validating `#initialize` (`Request.send(:new, …)` with a GET and a body raises `HTTP-7`), and the residual holes are `Request.allocate`, which yields all-nil members, and duck typing -- measured on 4.0.6 and 3.2.11. The mitigation is unchanged: both transports re-validate at the wire | `HTTP-2`, `SEAM-29`, `HTTP-4`, `HTTP-7`, `XCUT-18`; `docs/deviations.md` row 10 | the register must say what the tree does (`P10-2`), and a frozen sentence is corrected by an amendment written out for a human, never in place |
| P10-38 | **The proxy warning's credential scrub runs on the RAW value first** (review round 0's R0-4). The unanchored belt of the first repair runs on the redactor's output and stops at the first `/`, `?` or `#`, so a password holding one left its prefix in `Kernel#warn` and the config sink; `ProxyResolution#scrub_userinfo` now replaces everything from the authority's start (after `scheme:/…` or a leading `/` run, never after a bare `user:`) through the LAST `@`, and the belt stays behind it. Over-redacting an `@` in a proxy URL's path is the safe direction, as the belt already accepted | `OBS-11`, `CFG-22` | `OBS-11`'s userinfo rule is "unconditional", and a belt that stops at a delimiter leaves whatever precedes it |
