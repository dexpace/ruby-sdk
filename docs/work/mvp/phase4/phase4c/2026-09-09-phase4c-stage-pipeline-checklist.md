...
| `NFR-11` | MUST | ✅ share | 4 | Cross-reference, phase 9's ID: the step protocol's static half has a named home, `_Step` and `_AsyncStep` in `sig/dexpace/pipeline/step.rbs`, every name in them a `Dexpace::` constant; `gates:rbs_surface` is green over the twelve new mirrors |
