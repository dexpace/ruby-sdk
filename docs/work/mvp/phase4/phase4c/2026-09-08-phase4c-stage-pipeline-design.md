...
P4-26 through P4-39 stand as written; every one is implemented as its row says, and the plan's nine
open-question answers were carried through unchanged — the message forms, `.freeze` rather than
`Ractor.make_shareable` on the two tables, three probe types, phase 2's predicates at both builds, one
pinned seed, one error class for both bulk paths, `untyped` on `Cursor#call`, no `Object#freeze` on
either runtime, and the driver as a `driver_class:` constructor argument. Execution added the rows
below, numbered from **P4-50** so the two phase-4 lanes built in parallel cannot collide — 4a's design
holds P4-1 through P4-11 and its own as-built additions, and 4b's as-built addendum on this build's
base took P4-40 through P4-49. The checklist's "Deviations from the plan" is the itemised list against
the plan's text, and its "Guards run red" section holds the battery; the rows here are the ones that
touch public behaviour, the contract a later phase cites, or a count this document states. Review
round 0 (2026-09-16) added the last two, P4-58 and P4-59, from experiments against the built tree
rather than from the plan's text; the checklist's items 24–26 are their itemised record.
...
- **The Module layout's counts.** Twelve new `lib/` files, **twelve** `sig/` mirrors and ten `test/`
  mirrors: the two drivers are the `private_constant`s the layout names and get no suite, no
  YARD-gate entry and no manifest row, but each gets its `.rbs`, with the comment
  `sig/dexpace/hooks.rbs` carries (P4-50). "Three test-support files" became five — three probe
  files, because `Style/OneClassPerFile` refuses two top-level classes in one, the doubles' own suite,
  and phase 2's `inline_executor.rb` extended rather than a new `fake_executor.rb` (P4-54);
  `fake_async_transport.rb` is phase 2's, reused unchanged. And `sig/dexpace.rbs` gains nothing
  (P4-51).
- **`Cursor`'s surface.** "`#may_fork? — reads the same frozen entry table #fork gates on`" is one of
  its two inputs: it also answers false on a spent cursor, because P4-39 makes `#fork` raise there too
  and a predicate that said yes to a raise would not let a step ask rather than rescue (P4-57).
- **`PIPE-30`'s three lines.** "`rescue ::Exception => e; raise unless e.is_a?(::StandardError);
  completer.fail(e)`" became `rescue ::StandardError` with no re-raising arm and no second
  `Completer`: the fatal family passes through an absent arm as it would through a re-raising one,
  which is the shape 4b's P4-47 reached for the same rule, and a step's future is returned as itself
  (P4-52). Verified fact 7's bare-`raise` licence is therefore not exercised here; nothing in the
  subsystem re-raises.
- **`map_response`'s wiring.** The bullet list under `PIPE-31` describes a completer, an `on_cancel`
  hook and an `on_settle` handler written by hand; as built the operator is `Future#then` plus one
  `ensure` (P4-53), and every clause the list names still holds, now as phase 2's property.

| # | Deviation | Requirement / document | Why |
|---|---|---|---|
| P4-50 | **Twelve `sig/` mirrors, not ten**: `sig/dexpace/pipeline/sync_driver.rbs` and `async_driver.rbs` exist for the two `private_constant`s this document said get none | `NFR-3`; P2-15; 4b's P4-41; `CLAUDE.md`'s "every one mirrored in `sig/`" | The strict `core` Steep target checks every file under `lib/` and refuses an undeclared constant at a call site — `Cursor`, `Builder`, `Pipeline` and `AsyncPipeline` all name a driver — and RBS has no visibility, so the privacy lives in the `.rb` alone and each mirror carries `hooks.rbs`'s comment. What the drivers still do not get: a manifest row, a YARD-gate entry (the gate runs `--no-private`) and a `test/` mirror — their behaviour is asserted at their call sites, in `cursor_test.rb`, `pipeline_test.rb` and `async_pipeline_test.rb` |
| P4-51 | **Every constant declares in its own `sig/` file** and `sig/dexpace.rbs` stays `module Dexpace; end` | `NFR-3`; the plan's Task 11 Step 2; 4b's P4-42 | `sig/` mirrors `lib/` one file per file, and `Dexpace::PipelineError` in `error/pipeline_error.rbs`, `Dexpace::Pipeline` in `pipeline.rbs` and `Dexpace::AsyncPipeline` in `async_pipeline.rbs` are where a consumer's `steep check` and the `gates:sig_diff` lock both look; a declaration that lived in the entry file's mirror would be the one place the mirror rule broke |
| P4-52 | **`AsyncDriver` returns a step's or the transport's future as itself, and fails the drive with `Dexpace::SeamError` when either returns something that is not a `Dexpace::Async::Future`** | `PIPE-29`, `PIPE-30`, `SEAM-16`; P4-30's cost cell; `Bridge::SyncOver#call` | Re-wrapping every step's future in a second `Completer` would allocate two objects and a hook per step per call and put a forwarding layer between a caller's `#cancel` and the step's producer; returned as itself, a failure reaches the caller as the identical object with nothing to unwrap and a cancellation reaches the producer directly. The `SeamError` is the mirror of phase 2's check on the transport `sync_over` wraps: `Step.conforms?` and `AsyncTransport.conforms?` cannot see a return type, so a sync step or transport installed through `#build_async` is caught by what comes back rather than handed to a caller as a "future" that is a `Response`. The check is per drive and costs one `is_a?` |
| P4-53 | **`AsyncPipeline.map_response` is written over phase 2's `Future#then`**, adding only the `ensure`d close | `PIPE-31`; P4-38; phase 2's `Future#then` | `Future#then` already forwards a failure as the same object, forwards a cancellation as a cancellation with its reason in both directions, and fails the derived future when the block raises — three of `PIPE-31`'s four clauses, as phase 2's tested property rather than a second copy. The operator's own contribution is the close: after the handler on success, anyway on handler failure, in one `ensure` through `close_quietly`. One consequence is inherited and stated: a handler returning `nil` fails the mapped future through `Settlement`'s "exactly one of response or error" rule, as `#then` documents. `map_response` also refuses a call without a block and a source that is not a `Future`, so a wrong call fails at the operator rather than inside a settlement hook |
| P4-54 | **Phase 2's `InlineExecutor` gains a `#posts` counter** in place of the design's `FakeExecutor` | Module layout; `PIPE-33` clause 2; `testing/7ecef8e8` | A second executor double would have been the same three lines under another name; a counter added to phase 2's is compatible — `InlineExecutor.new` and `#post` are unchanged and phase 2's suites never read `#posts` — and keeps one executor double in the repository. `FakeAsyncTransport` was likewise phase 2's already, with `settle_later:` for the design's "a `Completer` the test settles" |
| P4-55 | **`Stage`'s raw `pillar` and `terminal` readers are private**; the public spellings are `#pillar?` and `#terminal?` alone | `NFR-4`; phase 1's `Headers` and `Query` hiding a reader; the surface tool's own comment | `Data.define` generates a public reader per member and the surface walker records each as a manifest row; two spellings of one flag is `NFR-4`-locked surface with no second reader. Hidden exactly as phase 1 hides `Headers#values`, which the walker's `data_readers` is written to honour. `#name` and `#order` stay public: a caller names a stage and may order by its key |
| P4-56 | **`Stage#with` refuses**, raising `Dexpace::InvalidArgumentError` naming the closed set, on every call | `PIPE-2`; P4-32; phase 1's `Model#with` | `Data#with` is public on every `Data`. Through `Model#with` it routes to `Stage.build`, which P4-32 deliberately does not define, so the loud failure would have been a `NoMethodError` from inside `Model` naming a method the caller never called — measured on 4.0.6 and 3.2.11; without `Model`, on 3.4 and 4.0 it would mint a seventeenth stage that `Stages.of` cannot find and `Builder` would bucket under a key no table holds. An override that says "the set is closed" in the SDK's own error closes the one public route to a derivation; the `send` hole stays open and is not defended, as P8 says |
| P4-57 | **Three refusals and one predicate this document did not name on `Cursor`**: `#state` refuses a non-`Stage` rather than reading it as an empty slot; `#fork` refuses a non-`Hash` `state:`; the root cursor's `#fork` raises `the root cursor is bound to no entry and cannot fork (PIPE-15)`; and `#may_fork?` answers false on a spent cursor as well as on a slot stage | R10, R11, P4-39; `error-handling/ffdf6f4f` | A phase-6 author who writes `cursor.state(:redirect)` would otherwise read an empty hash for every hop and never see the marker; a `state: "x"` would raise a `NoMethodError` from `merge`; and the root cursor — public through `Cursor.build` — is the one cursor with no owning entry, so its refusal says so instead of naming a stage `none`. `#may_fork?` reflecting spent-ness is what makes "ask rather than rescue" true of both raises `#fork` can make |
| P4-58 | **Every `Stage` the subsystem holds is the constant by identity**: `Builder#resolve` and `Entry.build` canonicalise a `Stage` argument through `Stages.of(stage.name)` | P4-32; R10 row 3; `PIPE-18`; review round 0, R0-2 | `Data#dup`, `#clone` and a `Marshal` round-trip are public on every `Data` and each yields a `Stage` that is `==` its constant and not `equal?` to it — not a seventeenth stage, since name, order and flags are the constant's, but a second object. `Stages.of`, the bucket table and the cursor's state map all resolve it by value, so nothing misbehaved; the builder's two identity comparisons over stages did, refusing a copy with a message naming the same stage on both sides (`step declares stage retry but was installed with stage retry (R10)`; a same-stage insert beside an `Entry` built over the copy as cross-stage, on the reload path). Resolving through `Stages.of` — P4-32's "only lookup" — at the two places a `Stage` enters keeps the identity comparisons what they are rather than weakening them to `==`, which would leave "every instance is one of the constants" half true. One consequence, stated: a `send`-forged `Stage` whose name is not one of the sixteen now fails at `Entry.build` with `Dexpace::InvalidArgumentError: unknown stage: :fake (PIPE-1)` rather than as a raw `KeyError` from the bucket table, and one whose name is a real stage's resolves to that constant — the P8 hole is neither closed by this nor claimed to be |
| P4-59 | **`Builder#remove`, `#insert_after`, `#insert_before` and `#replace` refuse an anchor that is neither a `Module` nor a `Symbol` or `String`**, with `Dexpace::InvalidArgumentError` (`anchor takes a Module, Symbol or String, got Integer`), before any comparison | `PIPE-18`–`PIPE-21`; `SEAM-29`; `error-handling/ffdf6f4f`; review round 0, R0-4 | The RBS types the anchor as `Module \| Symbol \| String`, so Steep catches a mistyped anchor statically; at run time it reached `entry.step.is_a?(anchor)` and raised Ruby's `TypeError: class or module required` — and only once an entry existed to compare against, so `remove(42)` on an empty builder was a silent no-op and on a populated one a `TypeError`. `validate_anchor!` runs once at the top of `#remove` and `#find_anchor`, in the subsystem's own `takes a ..., got ...` form that `Cursor#state`, `Cursor#fork` and `AsyncPipeline.map_response` already use (P4-57), so a typo fails the same way whether or not anything is installed |

**What this addendum does not add.** No frozen-chapter sentence is contradicted by this build — §5.1's
"a small keyed map" is narrowed by P4-28 as the ledger already records, not falsified — so
`docs/first-release.md`'s `C1`–`C14` paragraph gains no `C15`. The consolidation of P4-26–P4-39 and
P4-50–P4-59 into design §10, and the §5.1 and §5.3 addenda that would state the `(stage, key)` map,
the sixteen stages and the disjoint `#call`/`#fork`, are a human's, as they were for 3a, 3b and 4b:
