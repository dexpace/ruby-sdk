...
document makes. `P9-34` and `P9-35` were added in **review round 1**, on 2026-09-24, `P9-36`
through `P9-39` in **review round 2**, `P9-40` and `P9-41` in **review round 3**, and `P9-42` in
**review round 4**, all on the same day; each is marked as such in its own row, and
nothing earlier is rewritten except `P9-32`'s count, which was wrong as written and is re-stated in
every round since, for the same reason.
...
| P9-32 | The mutation set run here is **this phase's own**, re-derived from the design's testing strategy and the plan's per-task "non-conforming double" rule | the brief's minimum set | The reviewer's list was in this run's opening brief and did not survive the session's context compaction. **Forty-four** mutations were re-derived and all forty-four go red; a forty-fifth shape — conditioning `Registry#resolve`'s `return hand_out(resolved)` guard on anything else — turned its `loop do … end` into an unbounded spin, ran for six minutes and was **killed rather than counted**, which is the finding routed to phase 10's inbound list and not a guard. Review round 1 adds **seven** more (rows 46–52), review round 2 **two** (rows 53–54), review round 3 **seven** (rows 55–61) and review round 4 **two** (rows 62–63), for sixty-two red in all (checklist, "Guards run red"). The two round-2 rows are shapes that SURVIVED round 1 — the brief's own minimum list named both — so the honest reading is that the set run at implementation was short by two, not that two more were found. The count exceeds the thirty-five the brief named as a minimum, but the SET is not the reviewer's, and a reviewer re-running the original should expect to find it covered rather than assume it |
...
| P9-40 | The audit's own driver is **one added file under `gems/dexpace-core/test/`**, and the Module layout's file list above is wrong where it says otherwise | `R6`; `P9-9`; the Module layout | Review round 3. Two sentences in this document cannot both hold: the Module layout says "**no file in `gems/dexpace-core/` at all**", and `P9-9` says "the driver is `dexpace-core`'s own suite and knows which ivar is which". The second is the one that survives, because the seven subjects `InvariantSuite` cannot name itself — the seam, the transport, the bounded map and its store, the cnonce draw, the redirect hop and the credential hop — are core's own objects, and core's test-support doubles are reachable from core's suite and from nowhere else; 8a's precedent already puts a driver in the tree whose objects it drives. The bend is as narrow as it can be: **`gems/dexpace-core/test/dexpace/cross_cutting_invariants_test.rb` is ADDED and nothing else under `gems/dexpace-core/` is modified, renamed or deleted** — not a test, not a support double, not a manifest row, and above all nothing under `lib/` or `sig/`. `R6` exists so that phase 9 does not REPAIR what it audits, and an added test file repairs nothing. `git diff --name-status main..<the docs tip> -- gems/dexpace-core` is one path, status `A`, which is the mechanical statement of it. The Module layout's sentence is left standing rather than quietly rewritten: this row IS the correction, and an edited file list would leave the next reader unable to tell which of the two was decided. The checklist's and the roadmap's boundary sentences name the file too |
| P9-41 | Seven `XCUT` assertions shipped with **no committed non-conforming double**, and five of them could be neutralised WHOLE with the suite green | `XCUT-1`, `XCUT-2`, `XCUT-6`, `XCUT-7`, `XCUT-10`, `XCUT-19`, `XCUT-24`; `R3` | Review round 3, and the same class of defect rounds 1 and 2 found — a guard this phase wrote that could not go red — at its widest. Nothing in the repository drove a subject that made any clause of `XCUT-1`, `XCUT-2`, `XCUT-6`, `XCUT-7` or `XCUT-10` fail, so each reported `:passed` because it could not report anything else; `XCUT-19`'s and `XCUT-24`'s clauses survived a literal-`true` replacement for the same reason, their apparent coverage being an artefact of `-w` raising on an orphaned local rather than of any assertion. `invariant_suite_core_test.rb`'s own header named `XCUT-6`, `XCUT-7` and `XCUT-10` among the IDs it drove, and the checklist claimed the (b) evidence universally. **In an audit phase this is `R3`'s own failure one level up**: `R3` stops an assertion passing because its subject was never built, and this is an assertion passing because its own falsifiability was never built. Thirteen defective stand-in cores now drive all seven, in the shape that file already used — a patched `Resilience` or `Instrumentation` whose `const_missing` falls through to the real core, so one test changes one subject. Measured: neutralising each of the seven assertions whole and warning-free (`return nil if subject` as the body's first line, so no local is orphaned and `NFR-6`'s hook cannot mask the result) reddens exactly the new tests for it, thirteen across the seven. No first-party code changed and none should have: the assertions were right and only the negative controls were missing. `XCUT-3` and `XCUT-12` are left without one deliberately, and the checklist names them — a double for either would have to stay in the wait past the assertion's own bound, which leaks the thread the base case's teardown counts |
| P9-42 | `XCUT-18`'s **model-layer** assertion had no committed non-conforming double either, and the claim that covered it counted rows rather than assertions | `XCUT-18`; `R3`; the checklist's evidence paragraph | Review round 4, and `P9-41` one row narrower. Round 3's sweep neutralised all forty-five assertion bodies in the four suites one at a time and four survived; three were accounted for — `XCUT-3`'s and `XCUT-12`'s, which the checklist names, and a private clause helper whose parent assertion is caught — and the fourth, `HeaderSyntax#header_syntax_validation`, was named nowhere. It could be neutralised whole with the whole of `rake test:gems` green, because both of its committed tests pin the model assertion `:passed` and drive only the call-site one. **The claim could not see it because it was counted in rows**: an ID appears once however many assertions carry it, `XCUT-18` carries two, and the row was covered on one of them — so the evidence paragraph now says ASSERTION and names the sweep as what makes it checkable. Two defective stand-in cores drive it: a permissive validator that refuses nothing, and one applying the NAME rule to outbound values as well — the second because HTAB refused everywhere satisfies every other clause, so the asymmetry that IS the requirement would otherwise rest on the real subject alone. Measured: neutralising the assertion whole reddens both (row 62), deleting the HTAB-accepted `Check` alone reddens the second (row 63). No first-party code changed |
