...
floor and 1,430 runs across the six gems (140 of them the eleven new suites, one more the smoke suite gained);
the matrix set is green on 3.2.11 (99.97% line coverage there too — the one uncovered line is the registry-claim
race branch, reached nondeterministically), 3.3.12 and 3.4.10; the code tip is green on all seventeen gates too,
its `test:gems` at 1,290 runs and **95.21%** (95.16% on 3.2.11), above the floor. **Every guard the plan and
the brief ask to be run red was run red, and thirty-two single-edit mutations were run** — thirty-one caught, the
...
as well as 4.0.6. **What shipped of `PIPE-39`**: `Pipeline.direct` / `AsyncPipeline.direct`, `Builder#install_preset`
...
raises `Dexpace::SeamError` for a delivered `Future`). Twenty-three departures from the plan's text are itemised
in the checklist — nine where the built tree overrode its assumptions, the rest this build's — none lowering a
gate, and the eight that touch public behaviour or a stated count are the design ledger's As-built rows
P4-50–P4-57: the drivers' `sig/` mirrors, every constant declared in its own mirror, `AsyncDriver` returning a
step's future as itself and refusing a non-`Future` with `SeamError`, `map_response` written over `Future#then`,
`InlineExecutor`'s counter, `Stage`'s raw readers private, **`Stage#with` refusing** (the closed-set hole the brief's
as-built point 9 named: `Data#with` would otherwise mint a seventeenth stage on 3.4 and 4.0, and `Model#with`
would route to a `Stage.build` that does not exist), and `Cursor`'s three refusals with `#may_fork?` false on a
spent cursor. No frozen-chapter sentence is contradicted by this build, so `docs/first-release.md`'s `C1`–`C14`
paragraph gains no `C15` and that file is untouched. `docs/sdk-documentation/pipelines.md` is the as-built page, its
eleven fences run verbatim on 4.0.6 and 3.2.11 (`Hash#inspect`'s spelling differing on the floor and nothing
else). The counts that changed: `gems/` is still six; `dexpace-core`'s `lib/dexpace/` is ninety-one phase-1,
...
P4-26–P4-39 and P4-50–P4-57 into design §10, and §5.1's and §5.3's addenda, are a human's, as they were for 3a,
