...
The six rows above stand. This sub-phase was cut from `main` at `c53638b`, which holds the whole of
phase 6, concurrently with `7a` and `7b` on the same base: nothing of theirs exists on this tree, so
`R11`'s "if `7b` landed first" branch did not arise and its "if `7c` lands first" branch was **not
taken either** — the manager's decision of 2026-09-20 gives the boundary-5 audit to `7b`'s Task 11
(`tools/serde_boundary.rb`, running concurrently with a `PENDING` row for `page/**`), and `7c` builds
no gate, no fixture and no `tasks/gates.rake` edit; the checklist's boundary-5 row cites that task, and
`page_test.rb` scans the fifteen files (`page.rb` and the fourteen under `page/`) for the two bare tokens 7b's scan refuses, comments included,
until that row flips to `GUARDED`. Every interpreter in the matrix was installed and every fact below
was re-run on 3.2.11, 3.3.12, 3.4.10 and 4.0.6; the plan's "install ruby@3.2.11 and ruby@4.0.6" was
stale. Execution added the rows below, numbered from **P7-101** as the manager fixed it — `7a`'s
as-built rows start at P7-61 and `7b`'s at P7-81, and P7-1–P7-6 above collide with `7a`'s P7-1–P7-9,
numbered in isolation; phase 10's consolidation into design §10 resolves the collision, and outside
this document another lane's row is cited as "7a's P7-n". The checklist's "Deviations from the plan"
is the itemised list against the plan's text; the rows here are the ones that touch public behaviour,
the contract a later phase cites, or a statement this document makes.
...
- **`R8`'s `$!`-reading `ensure` is not what shipped, and must not be.** The measurement under `R8` is
  right — a bare `ensure` inverts the primary — but its prescription, `primary = $!` at the top of the
  `ensure`, is wrong in the other direction: `$!` is dynamically scoped to the thread and is non-nil
  inside anything called from a **caller's** `rescue` (`pipeline/7ce4431d`'s own measurement, re-run
  here), so a walk driven from inside a consumer's rescue block would attach its close failure to an
  unrelated in-flight error and swallow it. The shape built is 6b's: the frame that owns the walk
  records the primary in a `rescue ::Exception => error` arm that re-raises unchanged, and its `ensure`
  hands that local to `Closing.close_walk`. `Walk#hold`'s advance close is a bare close for the same
  reason. Neither `$!` nor a bare `ensure` appears anywhere under `lib/dexpace/page/`, and the
  lifetime suite scans for both and drives a walk from inside a caller's rescue (P7-105).
- **`Info#items` is not `Model.own`'d.** `Model.own` is `Ractor.make_shareable(copy: true)`: it
  deep-copies and freezes every element and raises on a Proc or a Mutex. The collection is owned — a
  shallow `dup.freeze` — and the elements stay the caller's domain objects, on both `Info` and `Page`
  (P7-101).
- **The private constants carry `sig/` mirrors.** "`::Walk`, `::Closing` and `::LinkHeader` are
  `private_constant` and carry no `sig/`" fails `test/gates/gem_layout_test.rb`, which maps `lib/` to
  `sig/` one for one; the tree's rule since 4a is a mirror with `hooks.rbs`'s comment and no manifest
  row. And the module layout's `sig/dexpace/page/strategy.rbs` cannot exist for the same reason: the
  three interfaces live in `sig/dexpace/page.rbs`, nested in `class Page`, the way `io.rbs` holds
  `_Source` and `http/body.rbs` holds `_ResponseHandler` (P7-106).
- **`Pages#each` raises `Page::PageStateError`, not `Dexpace::InvalidArgumentError`.** The roadmap's
  phase-10 inbound bullet of 2026-09-13 already calls the argument family wrong for a state violation
  and names this sub-phase's Task 12 as the fix point (P7-103).
- **The `Walk` is generic over a per-walk drive** rather than holding `@paginator` and
  `@next_request` itself, so the fetcher front-end reuses it unchanged (P7-111); and
  `Closing.parse_or_close` closes the response on a fatal-family raise too, quietly and with no trail,
  where this document said `rescue ::Exception` "is not written" (P7-110).

Things this document did not say and the build decided, each a row below: `Cancellation.none` on the
blocking engine (P7-102), the dispatchability screen in `next_request_from` (P7-104), the strict
shapes `Page.build`, the extractors, a strategy's answer and a fetcher's answer are held to (P7-107),
the gate not built here (P7-108), what the walk future settles with (P7-109), the executor running the
first dispatch too (P7-112), the single-use latch claimed on obtaining the enumerator (P7-113), the
page-number screen's alphabet (P7-114), the link grammar's malformed-input rule (P7-115), from
review round 0, its first-`rel`-only rule (P7-116) and, from review round 1, the fragment-only
same-document form joining P7-5's empty one (P7-117).

| # | Deviation | Touches | Why |
|---|---|---|---|
| P7-101 | `Page#items` and `Info#items` are the caller's Array copied **shallowly** and frozen (`items.dup.freeze`), never `Model.own`'d; the elements are not copied and not frozen | `PAGE-2`, `PAGE-4`; `HTTP-5`, `XCUT-15`; the plan's Task 4 ("`Model.own` freezing the list") | Measured on every row: `Model.own([proc {}])` raises `TypeError`, `own([Mutex.new])` raises `Ractor::Error`, and `own([Object.new])` returns a copy whose element is not `equal?` to the caller's and is frozen. The model owns its collection; a page's items are the caller's domain objects |
| P7-102 | The blocking engine takes **no per-walk cancellation** in v1: `Paginator` and `Fetchers` have no `cancellation:` member or keyword, and the transport receives `Dexpace::Cancellation.none` on every exchange — never `nil` | `PAGE-36`; `SEAM-11`'s third positional; the plan's Task 10 fence (`nil`) | Measured: `Pipeline.standard(transport).call(request, EMPTY, nil)` dies with `NoMethodError` inside 6a's `RetryStep` (`cursor.cancellation.check!`), and nothing validates the token before the pillar. The design's object model has no member for it; the constant is the minimum that works against a standard pipeline, and a `cancellation:` keyword is a pure widening phase 8 or 10 can add without moving a signature. The manager's decision of 2026-09-20 |
| P7-103 | `PAGE-14`'s second `#each` raises `Dexpace::Page::PageStateError < ::StandardError` (`include Dexpace::Error`), in `lib/dexpace/page/page_state_error.rb`, and never `Dexpace::InvalidArgumentError`; `Walk#buffer`'s refusal of a second staged page raises the same class | `PAGE-14`; `SEAM-29`; the roadmap's phase-10 inbound bullet of 2026-09-13 ("`PAGE-14` and `SSE-26`/`SSE-40` guard the identical single-use latch with two different error families") | A second iteration is a state violation with no argument involved; `InvalidArgumentError < ::ArgumentError` would let a caller's `rescue ArgumentError` around a page loop catch it. The bullet's first half — one family across both subsystems — is closed here; its second half, one shared core supertype both subclass, is a pure widening (a superclass added narrows no signature) and stays on phase 10's list |
| P7-104 | `Page.next_request_from` screens the resolved target for an `http`/`https` scheme **and** a non-empty host, and answers `nil` — end-of-stream — for `mailto:`, `javascript:`, `ftp:`, a host-less `http:foo` and an empty-host `http:///p`; 6b's `Location#dispatchable?` copied, never called (a `private_constant`). No diagnostic is emitted: the engine has no `logger:` | `PAGE-19`; `REDIR-18`'s third trigger (6b's P6-95) | Every one of those is a SUCCESSFUL `URI::RFC3986_PARSER.join` (verified on every row), so without the screen each becomes a `Request` the transport then fails on — an error where `PAGE-19` wants an end. Consistent with `REDIR-18`, which the manager's decision of 2026-09-20 fixed |
| P7-105 | The `R8` close discipline is built **without `$!`**: `Items#each` and `Pages#each` record the primary in a `rescue ::Exception => error` arm that re-raises unchanged and hand the local to `Closing.close_walk(walk, primary)` in their `ensure`; `Walk#hold`'s advance close is bare; `$!` appears nowhere under `lib/dexpace/page/` | `PAGE-13`, `PAGE-15`, `PAGE-32`; `R8`; `pipeline/7ce4431d` | `$!` is dynamically scoped and non-nil inside a method called from a caller's `rescue`; `R8`'s `primary = $!` would attach a walk's close failure to an unrelated in-flight error and swallow what `PAGE-15` says to surface. Measured on every row; `lifetime_test.rb` drives a walk from inside a caller's rescue and scans the source |
| P7-106 | Every `private_constant` has a `sig/` mirror with `hooks.rbs`'s comment (`walk.rbs`, `closing.rbs`, `link_header.rbs`; `Paginator::Drive`, `Fetchers::Drive` and `AsyncPaginator::Pump` declared inside their owners' mirrors) and no manifest row; the three interfaces `_Strategy`, `_Extractor` and `_Executor` live in `sig/dexpace/page.rbs`, nested in `class Page`; there is no `sig/dexpace/page/strategy.rbs` | `NFR-3`, `NFR-4`; P7-2's "carry no `sig/`"; the module layout | `test/gates/gem_layout_test.rb` maps `lib/**/*.rb` to `sig/**/*.rbs` one for one, so a `sig/`-only file and a `lib/` file without a mirror both fail `test:gates`; the strict `core` Steep target types every call site of a private constant. `_Executor`'s name and `def post: () { () -> void } -> void` are as `bridge/async_over.rb:61`, `docs/deviations.md:63` and 8b's plan Task 9 already cite; only the file differs, and 8b's plan is told |
| P7-107 | Shapes checked at the boundary and named: `Page.build` refuses anything but a `Dexpace::Response` and an `Array`, and a `next_link` or `continuation_token` that is neither nil nor a String (the rule `Info` applies to the same two members; `Page`'s half arrived in review round 1, R0-1); `CursorStrategy` refuses an extractor answer that is not `[Array, String?]` (a 2-element Array, items an Array, cursor a String or nil); `PageNumberStrategy` and `LinkStrategy` refuse a non-Array items answer; `Closing.parse_or_close` refuses a `#parse` answer that is not an `Info`; `Fetchers` refuses a fetcher answer that is neither a `Page` nor nil — every one `Dexpace::InvalidArgumentError`, none read as end-of-stream | `PAGE-4`, `PAGE-16`, `PAGE-34`; `HTTP-4`; the construction pattern | `items, cursor = "oops"` destructures silently into a bare String and a nil, which the strategy would read as a terminal page; a wrong shape is the caller's mistake and is named at the strategy, not discovered as a truncated stream |
| P7-108 | Spec-forced boundary 5's audit is **not built here**: no `gates:serde_isolation`, no `tools/` file, no fixture, no `tasks/gates.rake` edit. `R11`'s "if `7c` lands first" branch is superseded by the manager's decision of 2026-09-20: 7b's Task 11 builds `tools/serde_boundary.rb` with a `PENDING` row for `page/**` that whichever lane rebases second flips to `GUARDED`. Until then `page_test.rb` scans the fifteen files for the tokens `Serde` and `JSON`, comments included | `R11`; spec-forced boundary 5; the plan's Task 1 Steps 2–4 and its open question 5 | Both lanes' "does it exist?" branches answer ABSENT on today's `main`, so both would have built one, under two names, in two places; the tree's rule — a gate body lives in `tools/`, tested from `test/gates/` — is 7b's shape. 7b's scan does not strip comments, which is why `R7`'s own sentence ("a caller who wants JSON …") is spelled "a caller who wants a codec" in every YARD block under `page/` |
| P7-109 | `AsyncPaginator#walk` and `#walk_pages` settle their future with the **number of pages delivered**, an Integer, never nil; a transport-side cancellation is forwarded as a cancellation carrying its reason (`Completer#request_cancel`), a transport failure as the same error object | `PAGE-25`, `PAGE-28`; `SEAM-16`; 5a's P5-52 | `Settlement` carries exactly one of response or error, so a nil success cannot be constructed; `Async.delay` settles with `true` for the same reason, and a count costs nothing more and says something. The cancellation rule is `Future#then`'s |
| P7-110 | `Closing.parse_or_close` rescues the fatal family too: a non-`StandardError` out of `#parse` closes the response quietly, attaches nothing, and propagates unchanged | `PAGE-13`; `RECOV-2`; this document's "`rescue ::Exception` is **not** written" | 6a's `guarded` and 6b's `closing_on_error` both close on the fatal family, and `RECOV-2`'s split is about the trail, not the close: "surfaced unchanged with no trail attached" is exactly what a quiet close followed by a bare `raise` does |
| P7-111 | `Walk.new(drive, cap:)` is generic over a per-walk **drive** (`#call -> Page | nil`); the strategy engine's drive is the private `Paginator::Drive` (holding the next request) and the fetcher front-end's the private `Fetchers::Drive` (holding the previous page's key); `Items.new(opener)` and `Pages.new(opener)` take the engine's walk factory. The `Walk` owns the cap, the exhaustion latch, the two slots and the release discipline | `PAGE-7`, `PAGE-9`, `PAGE-12`, `PAGE-34`; this document's `Walk` sketch (`@paginator`, `@next_request`) | The design's `Fetchers` promises "a `Walk` whose drive routine calls the two fetchers instead of a transport and a strategy"; a walk holding a paginator cannot do that, and two walks would duplicate the one lifetime discipline this sub-phase exists to write once |
| P7-112 | With an `executor:`, the async engine posts the **first dispatch** too (`submit { resume }`), not only each settlement's continuation: the whole driver runs on the executor, a queued executor fetches nothing until it runs, and a rejecting first `#post` fails the walk before any exchange | `PAGE-29`, `PAGE-30`, `PAGE-6` | "A mode running the driver — and therefore every consumer invocation — on a caller-supplied executor": the first dispatch is the driver's too, and `PAGE-30`'s "(re-)dispatch" names the first. Inline, "begins fetching immediately" holds as written |
| P7-113 | `Pages#each` without a block claims the single-use latch on **obtaining** the enumerator (`to_enum` over a private drive), so `view.each` followed by `view.each { }` raises; the plan's `Timeout_free_value` helper is `Future#value(deadline:)`, which settles a hung future as `:deadline_expired` instead of hanging the suite | `PAGE-14`, `PAGE-32`; 5a's `deadline:` keyword | "Its iterator/stream may be obtained at most once" — obtaining is the use; a `to_enum(:each)` that re-entered the latching method would have raised on the first pull instead. `Timeout_free_value` is a naming offence and a second wait |
| P7-114 | `PageNumberStrategy`'s numeric screen is `\A[0-9]+\z` with its own `timeout: 1.0`, a `private_constant`; a value with any non-ASCII digit, a sign, a fraction or a space falls back to `start`, and a percent-encoded digit run is read decoded and counts as numeric | `PAGE-17`, `PAGE-22`; design §4's per-pattern timeout | `\d` can admit non-ASCII decimal digits under Unicode matching; the requirement's "garbage" is anything that is not a page number this SDK would itself write |
| P7-115 | `LinkHeader`'s malformed-input rule: a link-value that does not open with `<` or never closes its `>` stops the scan and yields what was scanned so far; an unterminated quoted value or a trailing escape drops that parameter; parameter names fold (`REL=next` is `rel=next`); a blank `<>` target comes back as `""` for `P7-5` to end | `PAGE-18`, `PAGE-20` | The grammar never raises — `PAGE-18`'s "absence of a rel=next segment" is the reading of anything it cannot parse — and RFC 8288 §3 makes parameter names case-insensitive |
| P7-116 | `LinkHeader` reads only the **first** `rel` parameter of a link-value: `<u>; rel="prev"; rel="next"` is a prev link and not a next one, and a second `rel` neither adds nor removes a relation (review round 0's R0-3; the first build's `params.any?` honoured a later `rel`) | `PAGE-18` | RFC 8288 §3.3: "The rel parameter MUST NOT appear more than once in a given link-value; occurrences after the first MUST be ignored by parsers." `PAGE-18`'s text does not decide the case; the RFC does, and the liberal reading could follow a link the server marked `prev` |
| P7-117 | **P7-5 widened to the fragment-only form**: a `rel=next` target that is empty OR fragment-only (`<#>`, `<#frag>`), surrounding whitespace aside, is end-of-stream **before resolution is attempted**, read off the raw target by a private `Page.same_document?`. The screen is **syntactic**, never on the resolved URL: `<?>` (which empties the query), `<//>` (which uri resolves to the base itself, measured on every row and under uri 0.12.5) and the current URL spelled out are next requests like any other, bounded by `PAGE-9`'s cap, and the suites pin that they are followed (review round 1's R1-2) | `PAGE-19`, `PAGE-18`; P7-5; `PAGE-9`, `PAGE-10` | Measured on every row: `join(base, "#frag")` succeeds and answers the base plus a fragment the wire never carries, so the first build re-fetched the current page until the cap, which `PAGE-10` defaults to unbounded. RFC 3986 §4.4 names the empty and the fragment-only reference together as the same-document forms whose dereference "should not result in a new retrieval action" — P7-5's own reasoning, applied to the second form. A screen on the resolved URL was considered and refused: it would silently end a walk against an endpoint that legitimately advances server-side state under one URL, with no knob to turn it off, where the cap is the caller's and documented; `PAGE-9` assigns a server that never advances to the cap, and the two spellings a browser distinguishes (a fragment-only link navigates nowhere; a spelled-out link to the current page reloads it) are the two this screen distinguishes |
