...
checked before the line is applied and reset at **every blank line, dispatching or not** (a run of
fieldless blocks is a run of blocks, `SSE-1`; review round 0's R0-1 found the nil branch returning
before the reset, so their bytes accumulated into the cap). This sub-phase was cut from `main` at
...
- **The typed layer's two drive shapes** (P7-86): the block form is a plain loop over `Stream#each`
  ...
  drive routine's `ensure`. The external typed drive carries an `ensure` of its own for the same exit
  on `#values`: it runs the raw enumerator, so `first(n)`, `take`, `find` or an `each { break }` there
  never reaches the Stream's ensure, and without one the resource stayed open until `#close` where
  the raw form released it (review round 0's R0-2; the mirror the section above promises now holds).
