...
| P1-9 | `Request` and `Response` are frozen and deep-frozen in their collections but are **not** `Ractor.shareable?` — **retired as built, 2026-09-15; see P1-13** | design §4; `data-modeling/996c0b12` | The design's "deep-freezing at construction makes the whole wire model Ractor-shareable" holds for every model whose members are strings and collections, and not for the two that hold a `URI::Generic`: verified on 3.2.11 and 4.0.6, a frozen URI is not shareable because `@host` and `@path` stay unfrozen, and `Ractor.make_shareable(uri)` reaches shareability only by deep-freezing `URI::RFC3986_PARSER` — a process-global object — in place, which is the one thing §4 says never to do. `Ractor` is load-bearing nowhere (§4, §9), so the claim is narrowed rather than the global frozen. Filed as a corpus note. **As built:** the first half is right and the second was not needed — `URL.parse!` freezes the component strings it owns (P1-13), and `URI::RFC3986_PARSER` is already frozen by the uri gem on both interpreters, so both models are shareable with no global touched — given a `nil` or frozen body, which is opaque and carried as given in this phase; the reason phrase, the one caller-supplied `String` a model carries, is copied and frozen at build — and design §4's original claim stands; both suites assert it, and the corpus note's planning-time entry is rewritten as built, under `## Conflicts`, confirming `data-modeling/996c0b12` rather than superseding it |
...
| P1-11 | `HeaderName` is `Data.define(:original)`; the fold is a derived attribute set before `super`, not a second member, and `.build(original:)` takes no `folded:` | this design's HeaderName section (`Data.define(:original, :folded)`); `HTTP-21` | Added at implementation, 2026-09-15. A derived value that is also a member has to be accepted and ignored — `#with(original: …)` carries the stale fold in `to_h` — which `Lint/UnusedMethodArgument` refuses and which is the argument `Status` already makes for keeping its name out of its members. Setting the ivar before `super` is the one moment a `Data` is mutable; on 3.2.11 the inherited `Data#with` leaves it `nil`, so `Model#with`'s route through `.build` is what makes the type sound on the floor, and the floor run proves it |
| P1-12 | `Query#==`/`#hash` compare the encodings, overriding `Data`'s generated equality over the pair list | `HTTP-30`; `api-design/e4fa3438` | Added at implementation, 2026-09-15. Two Strings holding the same non-ASCII bytes under different encoding tags are not `String#==` yet encode to one wire query; `HTTP-30` states equality as "equal iff they encode identically", so comparing the encodings is the requirement literally. The override carries the why-comment the styleguide requires |
| P1-13 | `URL.parse!` re-parses a URI input from its text and freezes the URI's component Strings, rather than `dup.freeze` | this plan's Task 12; `XCUT-15`; design §4 | Added at implementation, 2026-09-15. `URI::Generic#freeze` is shallow and `URI#dup` shares `@host`, `@path` and `@query` with the source (probed on 3.2.11 and 4.0.6), so the plan's `input.dup.freeze` left `request.url.host << "x"` able to change a model in place through a reader, and a caller's later mutation of their own URI able to reach it. Parsing from the text yields components the method owns, which it freezes without touching the caller's object or the process-global parser. The consequence retires P1-9 |
| P1-14 | `RequestOptions` tag **values** are `String` only — `tags` is a `String → String` map, and a non-`String` value is refused at construction | `HTTP-34` ("opaque string-keyed tags"); `HTTP-1` ("per-call tag values are opaque so their own mutability is the caller's concern") | Added at implementation, recorded 2026-09-15 after the round-2 review. The spec fixes the key type and leaves the value opaque, so an arbitrary object would be admissible and its mutability the caller's problem; this design's RequestOptions section chose `String → String` from the start so `Model.own` can deep-copy and deep-freeze the map at construction without meeting an object it cannot copy — a `Thread` or a `Proc` as a tag value is exactly the `TypeError` the round-2 review's collection finding removed elsewhere. Narrowing is the safe direction: a later phase that needs a non-`String` tag value (an object, a duration) widens this row and the `validate_tags!` check together, and nothing built on the narrow reading breaks when it does. Neither the ledger nor the `HTTP-34` checklist row named this as a reading before; both do now |
