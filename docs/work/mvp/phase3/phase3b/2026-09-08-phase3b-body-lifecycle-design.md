...
| `HTTP-38`'s **serialized-object** clause — "serialized-object bodies are replayable". It has no subject until a codec exists; 3b ships eight factories and phase 7a's `Body.serialized` is the ninth | 7, Task 9. The clause is phase 7a's, and its own P7-3 records the widening |
...
- **`Dexpace::Builder`** — not used by 3b, and **`Dexpace::Model` only for `Model.required!`**, which
  `FileBody`, `MultipartBody` and `MultipartBody::Part` call at their own validation boundaries so
  `SEAM-29`'s one message form covers them too. No body is a `Data` value type: every
...
| `Dexpace::MultipartBody` (+ nested `::Part` and `::Builder`) | `BODY-2`, `HTTP-51`, `HTTP-3` | One shared framing routine so declared length cannot drift from bytes written. `::Builder` and `#new_builder` are `HTTP-3`'s: its canonical text names "the multipart body" among the builder-based models, design §4's restatement of that list omits it, and phase 1 — the ID's owner — had no multipart body to build, so the clause reaches the phase that ships the subject |
