...
| `REDIR-3` | MUST | ✅ | 8, 9, 12 | A 301 or 302 is followed iff the **original** request's method — `Chain#seed_method`, captured at the seed and never re-read off the current hop — is in the allowed set, default `Step::DEFAULT_ALLOWED_METHODS = {GET, HEAD}`, a fresh frozen `Set` of `Method`s and deliberately not `Method::IDEMPOTENT`: `OPTIONS`, `PUT`, `DELETE`, `POST` and `PATCH` are not followed by default, `GET` and `HEAD` are; and when followed the method and the body are preserved — no `POST` → `GET` rewrite: a followed 301 and a followed 302 on a `POST` and on a `PUT` (under `allowed_methods: [that method]`) re-issue the original method token, the **same body object** and the `Content-Type` still travelling — **the clause that had no test until review round 2 found an automatic `POST` → `GET` rewrite on 301/302 surviving on both rows** (guards 57–59). The seed-method rule's witness is `POST` → 303 → `GET` → 301, which stops at the 301 under the default set although the chain is now a `GET` (`step_test.rb` `ConstructionTest`, "the default is NOT Method::IDEMPOTENT"; `ReissueTest`, "a followed 301 and a followed 302 re-issue the ORIGINAL method", "the ORIGINAL method decides"; guards 31, 32, 57–59) |
...
| `REDIR-5` | MUST | ✅ | 8, 12 | `follow303: false` by default (a 303 over a `POST` returns as it is); opted in, `Reissue.rebuild_as_get` re-issues a `GET` with `body: nil` and every `Content-*` header removed **by prefix** under the no-argument fold — `Content-Type`, `Content-Length`, `Content-Language` and `Content-MD5` all gone, `Accept` kept — whatever the original method, and `Authorization` stripped with them (`REDIR-7`); **the MUST's "case-insensitively" has its own case since review round 2 found the fold dropped surviving on both rows** — a `POST` carrying `content-type`, `CONTENT-LENGTH` and `cOnTeNt-Language`, stored in that casing (asserted on `Headers#names` before the hop), rebuilds a `GET` carrying `Accept` alone (guard 60); a 303 over a NON-replayable body is followed, because it drops the body (`step_test.rb` `RebuildTest`, the five `REDIR-5` 303 cases; guards 33–35, 60) |
...
| `REDIR-13` | MUST | ✅ | 1, 4, 11 | One call to `URI::RFC3986_PARSER.join` and no re-rendering: `https://user:pass@h/a%2Fb?x=%26y%2Bz` reaches the wire as `https://h/a%2Fb?x=%26y%2Bz`, `https://[2001:db8::1]:8443/p%2Fq` byte for byte, and — **the fragment clause, which had no test until review round 1 found the fragment dropped surviving on both rows** — a fragment (`/y#frag`), a percent-encoded one (`/y?q=a%23b#c%2Fd`, the `%23` and the `%2F` untouched), an empty query (`https://h/y?`) and an empty fragment (`https://h/y#`) reach the transport byte for byte. **The residue, stated and not hidden**: an EXPLICIT scheme-default port is elided — `Location: https://a.example:443/y` reaches the wire as `https://a.example/y` — because `URI#to_s` drops it on every supported Ruby and phase 1's `URL.parse!` re-parses a URI from its text (5b's P5-91); the origin triple reads the parsed port and is unchanged, every non-default port survives, and the suite never asserts that `:443` survives (P6-96) (`step_test.rb` `LocationTest`, four `REDIR-13` cases; `matrix_facts_test.rb`; guards 13, 53–56) |
| `REDIR-14` | MUST | ✅ | 1, 4, 11 | A relative `Location` resolves against the **current** hop — `/v1/x` → `/v2/a/b` → `c` → `/v3/x` sends `https://h/v2/a/c`, not `https://h/v1/c` — a query-only, a fragment-only and a network-path reference all resolve — the fragment-only one against the **current** hop, keeping that hop's path and query (`?page=2` then `#only` sends `https://h/list?page=2#only`, never `page=1#only`; review round 1 found this case titled for a fragment-only reference its script did not carry, and it carries one now) — and an absolute value is used as-is after the strip (`step_test.rb` `LocationTest`, two `REDIR-14` cases; `matrix_facts_test.rb`; guards 14, 53) |
| `REDIR-15` | MUST | ✅ | 6, 11 | `Reissue.downgrade!` compares the two schemes with a bare `downcase` per hop: `https` → `http` emits `Events::SCHEME_DOWNGRADE_REJECTED` and raises `Redirect::SchemeDowngradeError` — a `Dexpace::Error` naming the two authorities and the opt-in, never a path or query — after the current response is closed (`REDIR-22b`); `http` → `https` → `http` fails on the second transition; `allow_scheme_downgrade: true` follows it and emits `SCHEME_DOWNGRADE_PERMITTED`, never the rejected name, and the credential stripping still applies (P6-97) (`scheme_downgrade_error_test.rb`; `step_test.rb` `RefusedTargetTest`, three `REDIR-15` cases; `EmissionTest`, "the rejected downgrade is emitted"; guards 15–17) |
...
| `REDIR-18` | MUST | ✅ | 4, 9, 11, 13 | Never throws: `Step#resolve` is one `rescue ::URI::InvalidURIError` — BY CLASS, never by message, which differs between uri 0.13.3 and 1.x — that logs the RAW value and answers nil, the "return current" signal, and `Location.resolve` raises that one class for a malformed reference (`ht!tp://user:pass@bad`, a space, `<`, a non-ASCII byte, `:notaport`), for an unsupported scheme (`mailto:`, `ftp:`, `javascript:`, `data:`) and for a host-less target (`http:foo`, `http:///p`, `https://`) — the third the design did not name (P6-95). The current response is returned open, on both decision routes (`step_test.rb` `RefusedTargetTest`, two `REDIR-18` cases; `PredicateTest`, "unsupported scheme ... on BOTH decision routes"; `matrix_facts_test.rb`; guards 22, 23) |
...
| `HTTP-13` | ✅ | `Origin.of`'s scheme and host folds and `Reissue.rebuild_as_get`'s `Content-*` prefix test are the no-argument `downcase`, which the `Dexpace/NoLocaleCaseFold` cop makes a rule — and the cop enforces that a fold has no locale argument, not that a fold happens, so the rebuild's fold has a behavioural proof since review round 2: `content-type`, `CONTENT-LENGTH` and `cOnTeNt-Language` removed from the rebuilt `GET` (`step_test.rb` `RebuildTest`, the `REDIR-5 / HTTP-13` case; guard 60), beside `Origin.of`'s `A.EXAMPLE` case (the `REDIR-8` row) |
...
**Review round 1 (2026-09-19) ran fifty-nine of its own and found one more surviving on both rows**
— the fragment dropped from the resolved target, with the empty query dropped folded into the same
finding: `REDIR-13`'s "path, query, and fragment" had a test for the first two words only, and the
`REDIR-14` case titled for a fragment-only reference carried none — so the battery was fifty-six,
fifty-five caught on 4.0.6 and fifty-five on 3.2.11, guard 3 still the one equivalent mutant:
guards 53–56 are the round's, each red on both rows after the tests-branch fix, 53 on two cases.
**Review round 2 (2026-09-19) ran seventy of its own and found four more surviving on both rows,
on two MUST clauses** — an automatic `POST` → `GET` rewrite on a followed 301/302 (the method
rewritten, the body dropped, or both, on 301/302 alone), because no test followed a 301 or 302 on a
non-`GET`/`HEAD` method and asserted the re-issued method or body; and the case fold dropped from
the 303 rebuild's `Content-*` prefix test, because every 303 case carried canonical casing — so the
battery is **sixty, fifty-nine caught on 4.0.6 and fifty-nine on 3.2.11**, guard 3 still the one
equivalent mutant: guards 57–60 are the round's, each red on both rows after the tests-branch fix.

| # | Fix reverted | Guard | What it said (4.0.6; identical on 3.2.11 unless stated) |
|---|---|---|---|
| 1 | `REDIR-7`: `Authorization` not stripped on the re-issue | `CredentialHygieneTest`, `RebuildTest`, the convergence test | `Expected #<data Dexpace::Headers values={"authorization" => ["Bearer t"]} …> to not include "Authorization"` (5 failures) |
...
| 14 | `REDIR-14`: a relative `Location` resolved against the seed | `LocationTest` | `--- expected [… "https://h/v2/a/c" …] +++ actual [… "https://h/c" …]`; since round 1 the fragment-only hop too, `page=1#only` for `page=2#only` (2 failures) |
| 15 | `REDIR-15`: the downgrade followed by default | `RefusedTargetTest`, `EmissionTest` | `[Dexpace::Redirect::SchemeDowngradeError] exception expected, not …` (4 failures) |
| 16 | `REDIR-22b`: the closing frame dropped | `RefusedTargetTest`, `ReissueTest`, `PredicateTest`, `LifecycleTest` | `Expected: 1 Actual: 0` on `closes` (5 failures) |
| 17 | `REDIR-15`: `allow_scheme_downgrade: true` refused | `RefusedTargetTest`, `EmissionTest` | 4 failures, 3 errors — the opt-in raises |
| 18 | `REDIR-16`/`REDIR-22c`: a "return current" outcome closed | `DecisionTest`, `RefusedTargetTest`, `PredicateTest` | `Expected: 0 Actual: 1` on `closes` (5 failures) |
...
| 22 | `REDIR-18`: a malformed `Location` raises | `RefusedTargetTest`, `PredicateTest`, `EmissionTest` | `URI::InvalidURIError` out of the pipeline (7 errors) |
| 23 | `REDIR-18`: the scheme-and-host screen dropped | `RefusedTargetTest`, `EmissionTest` | `NoMethodError: undefined method 'downcase' for nil` out of `Origin.of` on `mailto:` (3 errors) |
...
| 33 | `REDIR-5`: the `Content-*` headers kept on the 303 rebuild | `RebuildTest` | `Expected … to not include "Content-Type"` |
| 34 | `REDIR-5`: the 303 body and method kept | `RebuildTest`, `ReissueTest` | `NoMethodError` — the rebuild reaches a helper that does not exist (3 errors) |
| 35 | `REDIR-5`: a 303 followed under the method set, ignoring `follow303` | `RebuildTest`, `ReissueTest` | `Expected: 200 Actual: 303` and the default followed a 303 (2 failures, 1 error) |
...
| 52 | `REDIR-9`: `Cookie`/`Proxy-Authorization` kept on a CROSS-ORIGIN 303 rebuild (`strip(cross_origin: false)` for 303 alone) — round 0's second survivor | `RebuildTest` | `Expected #<data Dexpace::Headers values={"accept" => …, "cookie" => ["sid=1"], "proxy-authorization" => …}> to not include "Cookie"` |
| 53 | `REDIR-13`: the fragment dropped from the resolved target (`target.fragment = nil`) — round 1's survivor | `LocationTest` | `Expected: "https://h/y#frag" Actual: "https://h/y"`; and the `REDIR-14` chain stops at two URLs — the fragment-only hop, its fragment gone, revisits the current URL and the loop check hands the 302 back (2 failures) |
| 54 | `REDIR-13`: an empty query dropped (`target.query = nil if target.query == ""`) — folded into round 1's finding | `LocationTest` | `Expected: "https://h/y?" Actual: "https://h/y"` |
| 55 | `REDIR-13`: the percent-encoded fragment decoded (`%2F` → `/`) | `LocationTest` | `Expected: "https://h/y?q=a%23b#c%2Fd" Actual: "https://h/y?q=a%23b#c/d"` |
| 56 | `REDIR-13`: an empty fragment dropped | `LocationTest` | `Expected: "https://h/y#" Actual: "https://h/y"` |
| 57 | `REDIR-3`: an automatic `POST` → `GET` rewrite on a followed 301/302 (`rebuild_as_get` for `[301, 302, 303]`) — round 2's first survivor | `ReissueTest` | `Expected: "POST" Actual: "GET"` on the 301 re-issue |
| 58 | `REDIR-3`: the body dropped on a 301/302 re-issue and kept on 307/308 (`body: [307, 308].include?(code) ? request.body : nil`) — round 2's second survivor | `ReissueTest` | `Expected nil (oid=4) to be the same as #<FakeBody …>` |
| 59 | `REDIR-3`: the method rewritten to `GET` and the body dropped on 301/302 alone — round 2's third survivor | `ReissueTest` | `Expected: "POST" Actual: "GET"` |
| 60 | `REDIR-5` / `HTTP-13`: the case fold dropped from the `Content-*` prefix test (`name.start_with?("Content-")`) — round 2's fourth survivor | `RebuildTest` | `Expected #<data Dexpace::Headers values={"content-type" => …, "content-length" => …, "content-language" => …, "accept" => …}, casing={"content-type" => "content-type", "content-length" => "CONTENT-LENGTH", "content-language" => "cOnTeNt-Language", …}> to not include "content-type"` |
...
12. **RuboCop's answers**: `extend self` on `Origin`, `Location` and `Reissue`; `=> error`; every method
    ...
    recorded exception `.rubocop.yml` prescribes, 6a's and 6c's shape); the step suite split into eleven
    nested classes and the constructors' into three; no `.freeze` on a plain literal; no `Regexp` in the
...
31. **Review round 0's two tests-branch additions** (2026-09-19): the sync wiring case in
    ...
    default schedule reproduces (guard 51); and `ReissueTest` gained the cross-origin 303 rebuild case
    over an `ORIGIN_SCOPED_HEADERS` `POST` (in `RebuildTest` since the split departure 34 records), so
    `REDIR-9`'s "or the 303 GET rebuild" clause has its own proof (guard 52). Neither the plan's Task 13a
    tests nor its Task 12 tests carried either.
32. **`pipeline/standard_test.rb` is three nested classes, not two**: the addition pushed `WiringTest`
    ...
    inline disable, the way `step_test.rb`'s twelve nested classes are split by concern.
33. **Review round 1's tests-branch addition** (2026-09-19): `LocationTest`'s `REDIR-14` case drives
    the fragment-only reference its title named, and a `REDIR-13` case sends a fragment, a
    percent-encoded fragment, an empty query and an empty fragment — the MUST's "path, query, and
    fragment" had a test for the first two words only, and the fragment dropped from the resolved
    target survived every suite on both rows (guards 53–56). The addition pushed `LocationTest` over
    `Metrics/ClassLength` (106/100), so `REDIR-15`'s three downgrade cases and `REDIR-18`'s two screen
    cases sit in a `RefusedTargetTest` — the targets the step refuses to follow, raising for the one
    and returning the current response open for the other — and `LocationTest` keeps resolution and
    the strip; the step suite was then eleven nested classes with eight concerns in its header. No
    `lib/` line changed.
34. **Review round 2's tests-branch addition** (2026-09-19): `ReissueTest` gains a `REDIR-3` case
    that follows a 301 and a 302 on a `POST` and on a `PUT` under an allowed set admitting the
    method, asserting on the transport's second call the original method token, the **same body
    object** and the `Content-Type` still travelling — the MUST's "there is deliberately NO
    automatic POST→GET rewrite" had no test that followed a 301 or 302 on a non-`GET`/`HEAD` method,
    so a rewrite on those two statuses survived every suite on both rows (guards 57–59); and a
    `REDIR-5 / HTTP-13` case whose `POST` carries `content-type`, `CONTENT-LENGTH` and
    `cOnTeNt-Language` in the caller's casing, asserted on `Headers#names` before the hop and each
    asserted gone from the rebuilt `GET` with `Accept` alone left — every 303 case carried canonical
    casing, so the fold dropped survived (guard 60). `ReissueTest` was at 97 code lines, so its four
    303 cases, their two constants (`CONTENT_HEADERS`, `ORIGIN_SCOPED_HEADERS`) and their two
    helpers (`post_with_content_headers`, `rebuilt_get_for`) moved unchanged into a `RebuildTest` —
    the one re-issue that changes the method, drops the body and strips by prefix — which the new
    fold case joins, and `ReissueTest` keeps the method-preserving re-issue (`REDIR-3`, `REDIR-4`,
    `REDIR-6`): a split by concern, never an inline disable; the step suite is twelve nested classes
    and its header lists ten concerns, and guards 1, 33, 34, 35 and 52 cite `RebuildTest` where
    their case moved (each re-run at the new tip on both rows and still firing there). No `lib/`
    line changed.
