...
`REDIR-14` case titled for a fragment-only reference carried none — so the battery is **fifty-six,
...
32. **`pipeline/standard_test.rb` is three nested classes, not two**: the addition pushed `WiringTest`
    ...
    inline disable, the way `step_test.rb`'s eleven nested classes are split by concern.
33. **Review round 1's tests-branch addition** (2026-09-19): `LocationTest`'s `REDIR-14` case drives
    ...
    the strip; the step suite is eleven nested classes and its header lists eight concerns. No
    `lib/` line changed.
