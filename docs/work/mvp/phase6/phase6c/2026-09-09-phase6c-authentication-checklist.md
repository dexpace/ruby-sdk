...
outside the design every citation reads "6c's P6-n") and as-built rows P6-71–P6-87 are cited below;
...
| `AUTH-35` | MUST | ✅ | 10, 12 | `validate(fetched)` inside the lock: nil → `ProviderError`; not a `BearerToken` → `ProviderError`; `expired?(now:, margin: 0)` → `ProviderError`, with `expiry == now` accepted (strictly after); and — the port's fourth rejection, review round 3's R3-1 — a token whose `Bearer <token>` wire form `HeaderSyntax.valid_outbound_value?` refuses (a trailing newline read off a file, a CR, a non-ASCII byte) → `ProviderError` whose message never names the token: such a token can never be sent, so no 401 could ever evict it (`AUTH-36`), and cached it failed every call until it expired — never, for a token with no expiry (6c's P6-87); a raising provider propagates its own error; on every one of those `@token` is untouched and the next call fetches again. The async stamper's `invalid(token)` is the same four checks as a value, failing the waiters with the `ProviderError`, caching nothing and freeing the slot — the "nothing" asserted for the rejected already-expired token through `#evict_if_matches` and the cache slot, since a token cached although rejected was behaviourally near-invisible and a mutation caching it survived round 1 (R1-5; guard 80), and for the refused token through the slot, `#evict_if_matches` and the next `#stamp`, which returns a future that stamps the clean token where round 3's tree raised synchronously; through both pipelines one dispatch fails with the transport never reached and the next refetches once (`bearer_stamper_test.rb` `RejectionTest`, `async_bearer_stamper_test.rb` `FailureTest`, `step_bearer_challenge_test.rb` `RetryTest`, `async_step_test.rb` `CoalescingTest`; guards 48, 80, 88–95) |
| `AUTH-36` | MUST | ✅ | 10, 11a, 13 | Cache half: `#evict_if_matches(rejected_header)` compares `"Bearer #{token}"` with the rejected VALUE under the lock — exact, `"Bearer  current"` does not match, and on the async half neither `"Bearer  cur"`, `"Bearer curator"` nor the bare token (review round 2's R2-3: the sync suite's pins, which a substring comparison on the async stamper had survived; guard 87) — clears only on a match and reports which happened. Step half, `bearer_retry` before the hook: on a 401 whose challenges include `bearer`, with the rejected request carrying `Authorization`, and a replayable body, evict, close the 401, and re-stamp ONE retry — a fresh fetch when the eviction fired (`["Bearer old", "Bearer new"]` on the wire), the preserved token when another request already refreshed it (`["Bearer old", "Bearer refreshed-elsewhere"]`, one fetch in all); regardless of method (a POST retries); surfacing the 401 unchanged and unclosed with no eviction and no fetch when the request carried no `Authorization` (cross-origin suppression) or the response advertises no `Bearer`; a second 401 surfaces after the one retry, two forks and no `#call`; the hook is never consulted for a Bearer challenge and IS consulted for a Digest one. Mirrored on the async step through `#stamp_fresh` after an eviction and `#stamp` after a preserved token — the routing itself asserted through `SpyBearerStamper`, whose two stamps differ on the wire (`Bearer cached` / `Bearer fresh`) and which records the call order, because the real stamper fetches through either method once its cache is empty and the first suite could not tell them apart (review round 0's R0-1) (`bearer_stamper_test.rb`, `step_bearer_challenge_test.rb`, `async_step_test.rb` `BearerTest`; guards 49–51, 62) |
| `AUTH-37` | MUST | ✅ | 12, 13 | `AsyncBearerStamper#stamp` reads one lock-free token and decides the zone from one clock reading: **fresh** (not expired with the margin) stamps in an already-settled future with no provider call, and takes no lock; **expiring-but-valid** stamps the still-valid token in a settled future at once and starts a refresh it never awaits (the future is settled before the refresh is; the boundary is exactly the margin — 1030 against 1000+30 is fresh, 1029 is expiring); **expired or missing** settles a Completer of its own from the coalesced fetch's settlement — R12's "second `#on_settle` and a second `Completer`", written that way since review round 2's R2-1: until then the waiter was derived through `Future#then`, whose derived future cancels its SOURCE, and the source was the one slot every coalesced caller shares, so cancelling one request's future cancelled every other waiter and every arrival until the provider settled (6c's P6-86). ONE single-flight slot: eight missing-zone and four expiring-zone callers cost one fetch. The fetch is shared and a cancellation is not: cancelling one of three coalesced waiters (a `#stamp_fresh` one among them) leaves the others pending, the provider's fetch unsettled and a new arrival coalescing onto the live slot, one fetch in all, and the survivors stamp the token once it lands — through the stamper and through a real async pipeline, where the cancelled request never reaches the transport; a provider cancelling its own fetch cancels every waiter AS a cancellation carrying the provider's reason, caches nothing and frees the slot; a fetch that lands a token the outbound grammar refuses is a failed fetch since review round 3's R3-1 — `AUTH-35`'s fourth rejection, never reaching the stamp — so an UNUSABLE background refresh logs the same `http.auth.refresh` WARNING without naming the token and caches nothing, the still-valid token stamped meanwhile (P6-87), and `#deliver`'s rescue keeps the one raise left, a request whose own derivation refuses, off the settling thread. A failed BACKGROUND refresh is reported through `logger:` as one `http.auth.refresh` WARNING (`Events::AUTH_REFRESH`, P6-77) carrying the cause, the in-flight request already stamped, and caches nothing; a failed awaited fetch fails its waiters with the same object, caches nothing, and the next call retries. The fetch is started OUTSIDE the lock — a `#fetch`-only provider mirrors into an already-settled future whose `#on_settle` runs inline, and starting it under the lock is `ThreadError: deadlock; recursive locking` (R12, guard 55). `#stamp_fresh` bypasses the cache and awaits a fetch, the post-eviction clause, and `AsyncStep` routes a successful eviction through it and a failed one through `#stamp` — proven through `SpyBearerStamper` since review round 0 (R0-1), the mutation that survived the first suite now caught (guard 62). No `#value`, `#wait` or `Async.delay` in the file, asserted by scan (`async_bearer_stamper_test.rb` `ZonesTest`, `FailureTest`, `CancellationTest`, `async_step_test.rb` `BearerTest`, `CoalescingTest`; guards 52–57, 62, 81–85, 91, 95) |
...
| `HTTP-18` | ✅ consumed | The outbound header grammar decides three things the requirements leave open: a non-ASCII Digest username takes the `username*` form, and a challenge whose realm, nonce or opaque cannot be echoed is unsatisfiable (P6-76); `KeyStamper` refuses a key the grammar cannot carry at construction; and both bearer stampers refuse a fetched token the grammar cannot carry as `AUTH-35`'s fourth rejection, uncached (P6-87, review round 3) |
...
After review round 2's repair (2026-09-18), one per line the repair made load-bearing and one per
mutation the round reported surviving, applied through the same harness and reverted, **on 4.0.6 and
on 3.2.11** — all seven caught on both.

| # | Fix reverted | Guard | What it said (identical on both rows unless stated) |
|---|---|---|---|
| 81 | `AUTH-37` (R2-1): the waiter derived through `Future#then` again — round 2's tree | `async_bearer_stamper_test.rb` `CancellationTest`, `async_step_test.rb` `CoalescingTest` | the second waiter `to not be settled?` after the first is cancelled, in both suites |
| 82 | `AUTH-37` (R2-1): the waiter's own `Completer` wired back to the slot through `#on_cancel` | the same two | the same: the second waiter settled (cancelled) by the first's cancellation |
| 83 | `SEAM-18` (R2-1): a cancellation of the slot forwarded to the waiters as a plain failure | `async_bearer_stamper_test.rb` `CancellationTest` | the waiter `to be cancelled?` after the provider cancels its fetch |
| 84 | `AUTH-37` (R2-1): `#deliver`'s rescue dropped | `async_bearer_stamper_test.rb` `CancellationTest` | `Dexpace::InvalidArgumentError: value for header Authorization contains a byte no outbound header value may carry (HTTP-18)` raised out of `Completer#fulfil` on the settling side |
| 85 | `AUTH-37` (R2-1): the slot left set after a provider cancellation | `async_bearer_stamper_test.rb` `CancellationTest` | the `@in_flight` future `to be nil` after the cancelled fetch |
| 86 | `AUTH-24` (R2-2): `DigestHandler#next_count` as `@nonces[nonce]` then `@nonces.set` — the round's M55, which survived the sixteen-thread race seven times of seven | `digest_handler_test.rb` `CounterTest` | `RuntimeError: the counter bypassed #update (AUTH-24)` |
| 87 | `AUTH-36` (R2-3): `AsyncBearerStamper#evict_if_matches` by `rejected_header.include?(token.token)` — the round's M58 | `async_bearer_stamper_test.rb` `EvictionTest` | `Expected true to not be truthy.` (`"Bearer  cur"` evicted `cur`) |

The round's two nits have no mutation: a comment line beginning `@lock` that YARD read as an unknown
tag is reworded, and this document's "Audit groups run" sentence below is qualified.

After review round 3's repair (2026-09-18), one per line the repair made load-bearing — the round's
one finding was a gap in the code, not a survivor — applied through the same harness and reverted,
**on 4.0.6 and on 3.2.11**: all eight caught on both.

| # | Fix reverted | Guard | What it said (identical on both rows unless stated) |
|---|---|---|---|
| 88 | `AUTH-35` (R3-1): the grammar check dropped from `BearerStamper#validate` — round 3's tree | `bearer_stamper_test.rb` `RejectionTest`, `step_bearer_challenge_test.rb` `RetryTest` | `[Dexpace::Auth::ProviderError] exception expected, not Class: <Dexpace::InvalidArgumentError>` (2 failures) |
| 89 | `AUTH-35` (R3-1): the grammar check dropped from `AsyncBearerStamper#invalid` — round 3's tree | `async_bearer_stamper_test.rb` `FailureTest`, `async_step_test.rb` `CoalescingTest` | the same `ProviderError expected, not … InvalidArgumentError` through the step, and the background case's `Expected #<… token=[REDACTED] …> to be nil` (3 failures) |
| 90 | `AUTH-35` (R3-1): the sync stamper writing `@token` before `validate` | `bearer_stamper_test.rb` `RejectionTest` | `Expected #<Dexpace::Auth::BearerToken token=[REDACTED] expiry=nil> to be nil.` |
| 91 | `AUTH-37` (R2-1's M64, re-pinned): `#deliver`'s rescue dropped — round 3's pin fed a refused token, which the fourth rejection now pre-empts | `async_bearer_stamper_test.rb` `CancellationTest` | `Dexpace::InvalidArgumentError: this request refuses to derive` raised out of `Completer#fulfil` on the settling side (1 error) |
| 92 | `AUTH-8`/`HTTP-20` (R3-1): the sync refusal's message names the token | `bearer_stamper_test.rb` `RejectionTest` | `Expected /abc\|bad\|\u00f6\|[\r\n]/ to not match "… may carry (HTTP-18): \"abc\\n\""` |
| 93 | `AUTH-8`/`HTTP-20` (R3-1): the async refusal's message names the token | `async_bearer_stamper_test.rb` `FailureTest` | `Expected /abc\|\n/ to not match …` (2 failures, the background case's cause too) |
| 94 | `AUTH-35` (R3-1): the async settle block writing any `BearerToken` into the cache, the error kept | `async_bearer_stamper_test.rb` `FailureTest`, `async_step_test.rb` `CoalescingTest` | `Expected #<Dexpace::Auth::BearerToken token=[REDACTED] expiry=nil> to be nil.` (3 failures, 1 error) |
| 95 | `AUTH-35`/`AUTH-37` (R3-1): the refusal raised out of the settle block instead of failing the slot | `async_bearer_stamper_test.rb` `FailureTest` | `Dexpace::Auth::ProviderError: the provider returned a token no outbound header value may carry (HTTP-18)` raised out of `Completer#fulfil` on the settling side (2 errors) |

The round's one nit — two 73-character body lines in the round-2 documentation commit's message —
has no mutation and is not repaired: rewrapping them means rewriting an earlier fixer's commit, which
the brief forbids, so it is deferred to the PR body.
...
guard 28 is the qualified spelling) and `pipeline/7ce4431d` (at implementation, no carried re-raise
existed in 6c — every `raise` was of an error just rescued or freshly built, so `cause: nil` applied
nowhere, and the one re-raise in `Step#consult` was a bare `raise` of the rescued object; after review
round 1's repair the two encoding failures are raised `cause: nil` on four sites — `digest_handler.rb`'s
two and `basic_handler.rb`'s two, deviation 31 — and the one bare re-raise is `Step#closing_on_error`'s,
which both runtimes' `#consult` call; review round 2's R2-5).

| Group | Result at implementation |
|---|---|
| Public API surface | `module-organization/1828a984` (one public constant per file) holds for every new file — `bearer_provider.rb` carries the module and two RBS interfaces by design; `api-design/b0e18938` is why `Validation`, `Parser`, `Computed`, `Exchange`, the two tables and the eight patterns are private and every public name is in the design's object model or in P6-71–P6-87; the one-method handler protocol (P6-2) and the `BearerProvider.fetch_async` function are the two surfaces the design named and the plan did not write |
...
| Minitest conventions | Every suite subclasses `DexpaceTestCase`, every "never raises" claim is asserted on the value (`assert_kind_of(Array, parse(input))`), `assert_same` wherever identity is the claim (the unclosed 401 IS the original), a test helper named `dispatch` rather than `run` (which shadows `Minitest::Test#run`), nine suites split into nested classes under `Metrics/ClassLength` (`bearer_stamper_test.rb` the ninth, since review round 3), no `#inspect` containing a Hash asserted |
...
gives them; 19–28 are this build's; 29 and 30 are review round 0's, 31 and 32 review round 1's, 33
and 34 review round 2's and 35 review round 3's (all 2026-09-18). The ones that touch public
behaviour, the contract a later phase cites, or a statement the design makes are also the as-built
ledger rows P6-71–P6-87.
...
6. **The async pivot as built**: `Future#then` exists (never `then_map`) and derived the expired zone's
   waiter until review round 2 — its derived future cancels its source, the wrong direction for a waiter
   on a SHARED fetch, so the stamper now settles a second `Completer` from the slot's settlement
   (deviation 33); `Settlement#success?`,
...
12. **RuboCop's answers**: `extend self` everywhere (`Challenges`, `Resolver`, `BearerProvider`,
    ...
    recorded inline with its reason on `DigestHandler` (one RFC, one class), `AsyncStep` (one `#call`
    as continuations) and, since review round 2, `AsyncBearerStamper` (R12's two Completers written out) —
    the recorded exceptions `.rubocop.yml` prescribes; nine suites split into
    nested classes (`bearer_stamper_test.rb` since review round 3); the test helper `dispatch`
    because `run` is `Minitest::Test#run`.
...
33. **The expired zone's waiter is a second `Completer`, never a `Future#then` derivation** (review
    round 2's R2-1, P6-86). The plan's Task 12 took `then_map` as "a small combinator over `#on_settle`
    + a derived `Completer`", the tree offered `Future#then`, and the build used it — but `#then` wires
    the derived future's cancellation back to its source, and the source here is the single-flight slot
    every coalesced caller shares, so one request's cancellation (which `AsyncStep#observe` forwards
    from the step's future, P6-79) cancelled every other waiter, and every new arrival coalesced onto
    an already-cancelled slot until the provider settled. The design's `R12` had prescribed "chaining
    through `#on_settle` and a second `Completer`" all along; `#awaiting` now builds each waiter's future
    that way, settled from the slot's settlement and never wired back, through one private `#settle`
    that classifies both completers by `Future#then`'s three rules — so a provider cancelling its own
    fetch cancels the slot and every waiter as a cancellation, a failure is the same object, and a stamp
    the outbound grammar refuses fails that waiter alone — a token, review round 3 found, that should
    never have reached the stamp (deviation 35). `Metrics/ClassLength` is recorded inline on
    the stamper for it (deviation 12). No surface change; the RBS mirror gains the two private methods.
34. **Two suite pins the round asked for** (R2-2, R2-3). The handler-level `AUTH-24` test was
    probabilistic — sixteen threads on one nonce never observe a lost increment under the GVL, so a
    read-then-set counter survived it seven times of seven — and now pins the primitive: the frozen
    handler's store is narrowed in place so `#[]`, `#set` and `#put` raise and `#update` records its key.
    The async `AUTH-36` case gains the sync suite's exactness pins (a doubled space, a superstring, the
    bare token), under which the reviewer's `include?` comparison runs red on both interpreters.
35. **A fetched token the outbound header grammar cannot carry is `AUTH-35`'s fourth rejection, on
    both stampers** (review round 3's R3-1, P6-87). The design's `AUTH-35` validation is the
    requirement's own two checks plus the class check, and the plan's fences the same; a token whose
    `Bearer <token>` wire form `HTTP-18`'s grammar refuses — a trailing newline read off a file, a
    CR, a non-ASCII byte — passed all three and was cached, where it could never be sent and so
    never evicted (`AUTH-36` matches a 401's rejected value, and no 401 ever comes), failing every
    later call until it expired: never, for a token with no expiry. The sync stamper raised
    `HTTP-18`'s `InvalidArgumentError` on every call with the provider never asked again; the async
    stamper's fresh zone raised it synchronously out of `#stamp`, a method that returns a `Future`,
    and through an `AsyncStep` every later request failed with the transport never reached — round
    2's own cancellation test fed exactly such a token and asserted only that its waiter failed.
    `BearerStamper#validate` and `AsyncBearerStamper#invalid` now refuse it as a `ProviderError`
    whose message never names the token, caching nothing, so the next call fetches again; the check
    lives where the token arrives, as `KeyStamper`'s does at construction, and not in
    `BearerToken.build`, whose contract is `AUTH-9`'s non-blank rule. A cached token therefore always
    stamps, `#stamp`'s fresh and expiring zones cannot raise, and `#deliver`'s rescue is re-pinned
    through a request whose own derivation refuses. `bearer_stamper_test.rb` is split into three
    nested classes for the case that pins it (deviation 12). No surface change; the RBS mirrors are
    unchanged.
...
- **The design's ledger** gains an "As built" addendum (P6-71–P6-87); the consolidation of P6-1–P6-7 and
  P6-71–P6-87 into design §10 and the §6.3 addendum are a human's, as for 3a, 3b, 4a, 4b, 4c, 5a, 5b
