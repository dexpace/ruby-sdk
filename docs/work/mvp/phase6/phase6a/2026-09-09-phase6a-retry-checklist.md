...
| `RETRY-26` | MUST | ✅ | 9, 10, 11 | Every sync wait is `settings.clock.sleep(delay, cancellation: token)` — asserted by identity on the token the fake clock recorded — and the async wait is `Async.delay`, which parks a fiber under a scheduler (`ParkingScheduler#block_count >= 2`, `kernel_sleep_count == 0`) and never a thread; no `Timeout.timeout`, `Thread#raise` or `Thread#kill` anywhere in the layer (the cop and the RETRY-45 scan), and no blocking sleep of any kind on the async path — the three async wait tests assert the settings' `FakeClock` recorded no `#sleep` and a text scan refuses the token `sleep` in `async_retry_step.rb`, whose one wait is `Async.delay`'s (review round 2, R1-2: a blocking `Clock#sleep` inserted beside `Async.delay` had survived the whole async suite, because every async case runs on a `FakeClock` whose `#sleep` records and returns) (`retry_step_test.rb`, "every wait is the clock's cancellable sleep"; `async_retry_step_test.rb`, `TrampolineTest`). Guard 39 |
...
| `RETRY-31` | MUST | ✅ | 10 | A zero-length delay completes inline — `Async.delay(0.0)` settles before its scheduler check, plan fact 2 — and its callback re-arms the running loop without a frame; the future is settled before `#call` returns and `Fiber.scheduler` is nil throughout. A positive delay parks the fiber under a scheduler, and the "never a blocking sleep" clause is mechanised since review round 2: the inline, parked and no-scheduler cases each assert `clock.sleeps` empty on the `FakeClock` the settings carry, and the text scan refuses `sleep` in the driver's source (`async_retry_step_test.rb`, "a zero-length delay completes inline", "a positive delay under a scheduler parks the fiber", "the async driver names no blocking sleep, by text"). Guards 10b and 39 |
...
| `RETRY-33` | MUST | ✅ | 10 | Every callback body runs inside `Pump#guarded`, which closes any open response, fails the completer on a `StandardError` and — for the fatal family — fails it and re-raises: a throwing `should_retry`, a throwing tracer callback (in `attempt_started`, in `attempt_failed` and — from review round 2 — in `retries_exhausted`, the terminal error-status response closed), a throwing factory, a throwing delay computation (a `NotImplementedError`) and a scheduler-less positive delay's `SeamError` each settle the future exceptionally rather than leaving it hanging, the response closed first (`async_retry_step_test.rb`, `TerminalPathsTest` and, since round 2, `TracerFencesTest` — the four tracer-and-factory cases moved there because both existing classes stood at the class-length ceiling — nine tests between them) |
...
| `RETRY-35` | MUST | ✅ | 9, 10, 11 | Three orderings, asserted: the delay is resolved from the still-open response (`Retry-After: 3` read, then closed, then slept 3.0); the response is closed BEFORE the wait (a clock whose `#sleep` records `body.closed?` sees `[true]`); and a throwing decision, delay computation OR tracer emission — `attempt_failed` on the retry path, `retries_exhausted` on the terminal path — closes the response before propagating (`fenced` on the sync step, `guarded` on the async one — the sync `attempt_failed` moved inside the fence in round 1, R0-4, and the sync `retries_exhausted` in round 2, R1-1, each having leaked the open response on the sync driver while the async pump closed it; a tracer that does not raise leaves the returned terminal response open, RETRY-34). On the recovery stack the release is `Recovery.buffer_error_body`, which drains the connection and hands every later line the buffered copy (`retry_step_test.rb`, `DelayTest` and `PredicateAndBudgetTest`; `async_retry_step_test.rb`; `recovery_retry_test.rb`, `IntegrationTest`). Guards 5, 6, 36 and 38 |
...
| `OBS-30` | ✅ | A throwing tracer callback propagates and fails the request on the sync drivers, and completes the async future exceptionally; core wraps no tracer callback — and on the two stage drivers the propagation never pins a response: a raise in `attempt_failed` closes the superseded response and a raise in `retries_exhausted` closes the terminal error-status one before it propagates, on both (`retry_step_test.rb`, "a throwing tracer propagates", "a tracer raising in attempt_failed", "a tracer raising in retries_exhausted"; `async_retry_step_test.rb`, the same three) |
...
| `NFR-4` | ✅ | Every addition is a widening: no existing signature moved. The runtime manifest grew by exactly 49 rows, 956 to 1005 — the 48 the object model names, read row by row at Task 13, plus review round 1's `Policy#cancellation?` (`P6-60`); the RBS baseline diff is vacuous until the first tag |
...
| `NFR-13` | ✅ for `.rb`; the `.rbs` half is phase 10's | The twenty new `.rb` files — nine under `lib/`, eleven under `test/` — open with `# frozen_string_literal: true` and `# SPDX-License-Identifier: MIT`, which phase 0's `Dexpace/SpdxHeader` cop gates. The nine new `.rbs` files carry NO SPDX line, as no `.rbs` in the repository does: the header reaches `sig/` with phase 10's Task 5 (`tools/spdx_rbs.rb`, `gates:spdx_rbs`; `docs/work/mvp/phase10/2026-09-13-phase10-deviation-reconciliation-and-release-readiness.md`), which covers 6a's nine with every other, and 6a adds none ahead of it so the tree stays one convention. Review round 2 corrected this row, which had claimed both halves (R1-3) |
...
`RetryPredicateError`, and the 46 rows of the six public `Resilience` constants and their namespace
(`Policy`'s eight functions and nine constants, `RetrySettings`' ten readers plus `.build`,
`#backoff_arguments` and `#header_order`, the three drivers' `.build`/`#call` and the two stage
drivers' `#stage`, `Resend#eligible?`) — 49 rows over `main`'s 956 — every row read
...
Guards 38 and 39 are review round 2's (2026-09-18): the terminal-path twin of guard 36, and round 1's one
surviving mutation (m20, R1-2), each seen red on 4.0.6 and 3.2.11.
...
| 38 | `RETRY-35` / `OBS-30` — the tracer's `retries_exhausted` outside the fence on the sync step | the `fenced(response)` block removed from `RetryStep#settle` (round 1's tree) | "a tracer raising in retries_exhausted propagates, terminal closed": `Expected #<Dexpace::ResponseBody …> to be closed?` — the async twin stays green, as it should: `Pump#finish` was fenced all along |
| 39 | `RETRY-31` / `RETRY-26` — a blocking `Clock#sleep` beside `Async.delay` on the async path | `@settings.clock.sleep(delay, cancellation: @cursor.cancellation)` inserted before `Dexpace::Async.delay(delay)` in `Pump#wait` (round 1's m20) | four failures: the inline, parked and no-scheduler cases each `Expected [{duration: …}] to be empty`, and the text scan `Expected ["sleep"] to be empty` |
...
17. **CLAUDE.md, the READMEs and the pages** were rewritten for 6a only, on top of what `main` says, with the
    ...
    checklists, the manifest at 1005 rows since round 1, twelve as-built pages beside `architecture.md`).
...
31. **Review round 2, the terminal fence (R1-1)**: round 1 closed the sync/async drift for `attempt_failed`
    and left its terminal twin open — `RetryStep#settle` emitted `retries_exhausted` outside any fence, so
    a tracer that raised there propagated with the terminal error-status response still open, while
    `Pump#finish` ran inside `guarded(response)` and closed it. The trail attachment and the emission now
    run inside `fenced(response)` on the sync step; a tracer that does not raise leaves the returned
    response open exactly as before (`RETRY-34`), the exception path is untouched, and both stage suites
    assert the close (guard 38). Not a ledger row: no design statement changed, as with R0-4.
32. **Review round 2, the no-blocking-sleep clause (R1-2)**: round 1's one surviving mutation — a blocking
    `Clock#sleep` inserted beside `Async.delay` in `Pump#wait` — left the async suite green, because every
    async case runs on a `FakeClock` whose `#sleep` records and returns and nothing read `clock.sleeps`,
    so the driver's "never `Clock#sleep`" and this checklist's `RETRY-26` row rested on nothing that would
    go red. The inline, parked and no-scheduler cases now assert `clock.sleeps` empty and a text scan
    refuses the token `sleep` in `async_retry_step.rb` (guard 39); no `lib/` line changed.
33. **Review round 2, the `NFR-13` row (R1-3) and three stale counts (R1-4)**: the row had claimed every new
    `.rbs` opens with the SPDX header; none does, as none in the repository does, and the header reaches
    `sig/` with phase 10's Task 5 — the row now says so. The `NFR-4` row's "48 rows", item 17's "1004
    rows" and the postponed-work sentence's `P6-51`–`P6-58` are 49, 1005 and `P6-51`–`P6-61` after round
    1; the "What was built" sentence names six `Resilience` constants (not five) and the two stage
    drivers' `#stage` (not three).
...
The consolidation of `P6-1`–`P6-12` and `P6-51`–`P6-61` into design §10 is a human's — `docs/sdk-design-ruby/`
