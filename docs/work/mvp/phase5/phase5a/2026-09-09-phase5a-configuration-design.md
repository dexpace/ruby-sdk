...
| `TRANSPORT-30`, `TRANSPORT-13` — proxy *use* and header-drop reporting on a real adapter | 8. 5a ships `CFG-22`–`CFG-28`'s proxy **model and resolver**; nothing in core opens a socket. **Corrected in place 2026-09-13 by phase 10's planning:** this row named `TRANSPORT-3` and `TRANSPORT-8`, and neither ID is about either subject — `TRANSPORT-3` is sync-path cancellation surfacing as a terminal interrupt-shaped `IOException`, `TRANSPORT-8` a cancellation originating inside the native client. The IDs meant are **`TRANSPORT-30`** (the proxy requirement, whose text opens "When the SDK's proxy configuration carries a feature the native client cannot honor") and **`TRANSPORT-13`** (the header-drop logging policy, which is 8c's). Found by phase 8a's review; the code half was never waiting on it — 8a's `R17` and its plan Task 19b implement proxy use, wire `Dexpace::Proxy.resolve` into the per-call client and disposition `TRANSPORT-30` — so what was wrong is only the pointer a later audit would follow. The same substitution was made in phase 5b's `OBS-19` condition and corrected there by the phase-8 segmentation design on 2026-09-12, which is why it is corrected here in the same shape rather than left as a second instance |
...
**Review round 0 (2026-09-17)** found the round-0 tree reading `CFG-22` more narrowly than its
text — `Proxy#to_s` and `#inspect` printed the username verbatim beside a masked password, against
"never emit username/password in cleartext" — and a `CFG-28` case that resolved against the
empty slot's real `ENV`; both were closed on the owning branches, the first by masking both
credentials (no ledger row: it is the requirement's own text) and the second by driving the slot
through `Dexpace.configure` with a fake seam. Its seven nits were taken, one of which amended
P5-55 above; the checklist's deviations 24–26 and its second guard table carry the rest.
