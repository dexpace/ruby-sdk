...
P5-16 through P5-39 stand as written; every one is implemented as its row says, and the plan's open-question
answers were carried through — the redactor's rebuild route excepted, which P5-91 replaces. This phase was cut
from the **reconciled 5c docs tip**, which holds 5a and 5c already built, so the plan's interleaved order
collapsed to Tasks 1–16 straight through and `diagnostics.rb` was the file 5c created (P5-71), extended in
place. Execution added the rows below, numbered from **P5-91** because the three phase-5 lanes are numbered
apart — 5a's as-built additions start at P5-51, 5c's at P5-71 — and the checklist's "Deviations from the plan"
...
Eight statements above read differently against the source — four found by the build, two by review round
0, two by review round 1 — and the difference is recorded here rather than by rewriting the text it corrects:
...
- **Verified fact 6's rebuild route and P5-27's mechanism.** `u.userinfo = "***:***"` does render
  ...
  supported Ruby, dropping the default port `OBS-14` requires written back byte for byte. The redactor therefore
  never rebuilds through the setters and `#to_s`: it reassembles from `RFC3986_PARSER.split`'s nine raw
  components (P5-91), the same primitive 5a's proxy resolver reached for the raw port. P5-27's rule — never
  assign an absent component — holds by construction, since nothing is assigned; the opaque-URI raise the fact
  reports is unreachable rather than guarded against, and `OBS-15`'s rebuild-failure branch is reached only
  through a policy read that raises, which the suite constructs directly (its testing-strategy entry already
  said so). The fact stands as the description of a route not taken; it is on phase 10's inbound list as
  audit work against this document.
- **Verified fact 13's "no combinator".** Phase 4c added `Async::Future#then` after this document was written,
  and `AsyncPipeline.map_response` beside it. The async step's shape at `NONE` and `HEADERS` is the fact's —
  `#on_settle` and the same future — and at `BODY` it is a `#then`-derived future (P5-94), because `OBS-36`'s
  "every byte reaches the caller" needs the wrapped response to be the one the caller's future settles with.
  `map_response` is not used: it closes the response after the block, a terminal operator's job. Also on
  phase 10's inbound list.
- **"Requires exactly two entries", `uri` and `set`.** One: the 3.2 floor autoloads `Set`, the cop set's
  `Lint/RedundantRequireStatement` refuses `require "set"`, and `redactor.rb`'s `require "uri"` is the phase's
  one plain require.
- **The Task 10 cop amendment and P5-38's "not covered".** The bare-`Logger` watch the plan asked for cannot
  be expressed in `Dexpace/QualifiedCoreConstant`'s shape and, examined against the tree, guards nothing: the
  cop's model is a `Dexpace::X` that shadows `::X` for a bare `X` anywhere inside `module Dexpace`, and this
  constant is `Dexpace::Instrumentation::Logger`, so a bare `Logger` anywhere inside `module Dexpace` but
...
- **The reserved-key table on `Event`, and P5-35's placement** (review round 0, R0-4). The table routes a
  ...
  covers `OBS-39`.
- **"Redaction runs on the way into `#field`", and the table "on the `Event`"** (review round 1, R1-2). The
  paragraph states the mechanism at one of `OBS-5`'s three sources and the "whoever calls `#field`" argument
  covers only that one: the logger's global context and the folded diagnostic context are merged at `#emit`,
  and the text gives them no table to meet, so a `url.full` or a header-prefixed key put in either reached
  the sink raw. `OBS-39`'s "the logged `url.full` MUST always be the redacted URL" and `OBS-18`'s "MUST NOT
  have its value logged" are stated on the emitted record. As built the table is the private `ReservedKeys`
  module and every source meets it — a per-event field at `#field`, the context once at `Logger.build`, the
  fold at `#emit` (P5-104); the sentence's "never at the sink" and "keyed by the field name and not by the
  call site" stand, and now hold for the record and not for one entry point.
- **"`MediaType#charset` returning `nil` for an unrecognised charset is what makes `Encoding.find`
  unreachable from here", and "'Decoding MUST NOT throw' … met by the replacement options"** (review round
  1, R1-4). Both are true of a charset Ruby lacks and false of one Ruby knows as a dummy encoding with no
  converter: `Encoding.find("utf-7")` and `("iso-2022-jp-2")` answer, so `#charset` answers the name, and
  `#encode` raises `Encoding::ConverterNotFoundError` with both replacement options set — from the step's
  response event, which `OBS-20` then turned into a diagnostic in place of the event. As built the decode
  falls back to UTF-8 for the `EncodingError` family (P5-106), the answer the "unrecognised" case already
  took; `Preview.render` is total by that fallback and not by the options alone.

| # | Deviation | Requirement / document | Why |
|---|---|---|---|
| P5-91 | **`Redactor#url` reassembles the redacted form from `URI::RFC3986_PARSER.split`'s nine raw components and never through `URI#to_s`**; `#header_value`'s absolute branch is `#url` and its relative branch the split path plus `?***`; a value `split` rejects takes string surgery on the raw value | `OBS-11`, `OBS-13`, `OBS-14`, `OBS-15`, `OBS-16`; verified fact 6; P5-27; `Dexpace/NoUriDefaultParser` | `URI#to_s` drops a default port (`http://h:80/` → `http://h/`) on every supported Ruby, and `OBS-14` requires scheme, host, port and path written back unchanged; `URI::Generic` exposes the raw port only through `split`. Reassembly writes back only what was present — a present-but-empty query keeps its `?`, an absent one writes nothing — so P5-27's rule holds by construction and the opaque-URI setter raise is unreachable. One consequence, stated in the suite: `split` accepts a bad percent-encoding in a query *value* (`?a=%zz`) that `#parse` rejects, and the redactor redacts it rather than sentinelling. The fragment is tokenised by hand on `&`, as the design already said |
| P5-92 | **`Dexpace::Instrumentation.diagnostic(logger, event:, cause: nil, message: nil)` is a second public module function beside `.contain`**: a contained WARNING event under one of `Events::INSTRUMENTATION_*`, carrying the throwable as its cause and an optional `Keys::MESSAGE` field | `OBS-20`, `OBS-39`, `SEAM-25`, `CFG-21`, `CFG-24`/`CFG-25`; P5-17; `NFR-4` | The three wirings — `close_quietly`'s second route, `Hooks.notify`'s dropped-failure diagnostic and `ProxyResolution#warn_and_nil`'s config event — and phase 8b's shutdown event share one contained shape, and writing it out three times pushed 5a's `ProxyResolution` past `Metrics/ModuleLength` for a body no test could tell apart. A manifest row (`Dexpace::Instrumentation.diagnostic`) that P5-17 does not enumerate |
| P5-93 | **`AsyncStep#call` closes its correlation scope at the end of the synchronous head, and bridges the diagnostic context into the settlement with `Diagnostics.capture` / `Diagnostics.with`** rather than closing the scope from the settlement callback | `OBS-24`, `OBS-22`, `OBS-23`; the plan's Task 15 (`scope&.close` in the callback); 5c's `Scope#close` contract | `Scope#close` restores the previous span into the fiber storage of the fiber that calls it, and `#on_settle` runs on the settling fiber or thread — so a close from the callback would write the settler's storage and leave the caller's slot holding the finished span. The scope is closed on the caller's fiber, where it was opened; what the settlement needs from the caller's context is the snapshot, which is exactly `OBS-24`'s bridge. The span itself is finished and the two instruments recorded in the settlement, off the head's `started` and the clock |
| P5-94 | **At `HTTPLogging::BODY` `AsyncStep#call` returns a future derived with `Future#then`**, carrying the response wrapped in `ResponseLoggingBody`; below `BODY` it returns the same future the cursor produced, with `#on_settle` registered | `OBS-36`, `BODY-22`, `PIPE-28`; verified fact 13; 4c's `Future#then` | The wrapped response is what the caller must receive — the wrapper mirrors on the caller's read — and only a derived future can settle with a different value; `#on_settle` observes and cannot substitute. Below `BODY` nothing is substituted and the fact's "same future" shape holds. `AsyncPipeline.map_response` closes the response after the block and is a terminal operator's, not a step's |
| P5-95 | **One redactor per logging path, the logger's**: `Logger` gains a public `#redactor` reader, `Step.build` takes no `redactor:` keyword, and the private `Emitter` reads its header-name gate from `logger.redactor` | `OBS-17`, `OBS-18`; P5-17, P5-34; the plan's Task 14 (`Step.build(logger:, redactor:, level:, …)`) | Found by the `OBS-17` guard: with the plan's shape the step's redactor gated which header *names* were logged while the logger's redacted the *values*, two policies where the requirement asks for one that "cannot drift". The names a step logs and the values its events redact now come from one `RedactionPolicy` by construction. A manifest row (`Dexpace::Instrumentation::Logger#redactor`) and one fewer keyword than P5-17 lists |
| P5-96 | **`Keys::MESSAGE = "message"` is a sixteenth key**, and **`Render::TRUNCATION_MARKER` is the ASCII `"...[truncated]"`**, not `"…[truncated]"` | `OBS-39`, `OBS-7`, `CFG-24`/`CFG-25`; P5-16 (fifteen keys); §8.1's "cap+suffix" | The configuration diagnostic carries the warning's text and no throwable, and an emitted key outside the vocabulary is what `OBS-39` forbids; the fifteen were written before the wiring's shape was. The marker: appending a UTF-8 ellipsis to a BINARY or Latin-1 rendering with high bytes raises `Encoding::CompatibilityError`, and a rendering is whatever the caller's `#to_s` returned; ASCII concatenates onto every ASCII-compatible encoding, and a non-ASCII-compatible String is transcoded to UTF-8 first |
...
| P5-103 | **The proxy resolver's warning and its `http.instrumentation.config` diagnostic name the proxy URL through the redactor and never raw**: `ProxyResolution#shown` renders it with `Redactor::DEFAULT#header_value`'s total form — the placeholder for a userinfo on every route, the path kept, never the sentinel — and then `CFG-24`'s own grammar rule, `CREDENTIAL_BEFORE_AT`, for a value with no `//`; the parser's message, which repeats the rejected value, is no longer quoted | `OBS-11`, `CFG-24`, `CFG-25`; §8.1's redaction paragraph; P5-8, P5-92; 5a's `resolution.rb` (Task 14's edit set); review round 1's R1-1 | 5a's `split_url` interpolates the raw URL into both of its warnings and `URI::InvalidURIError#message` repeats it, and this phase's config diagnostic carries that text under `Keys::MESSAGE`, which is not a reserved key — so `HTTPS_PROXY=http://user:secret@proxy.corp`, the no-port misconfiguration `CFG-25` refuses, wrote `user:secret` into the sink through core's own code, on every malformed-URL path. The URL a warning names is a display form, and the display form of a URL is the redactor's; `#header_value` is the entry point whose failure policy keeps the value readable (P5-25), reached under the policy's first URL-valued name because it is keyed by header name. The grammar belt is the resolver's and not the redactor's: `user:secret@proxy.corp:3128` — the scheme forgotten — is an opaque URI with no userinfo under RFC 3986 (P5-100), and only the resolver knows that in a proxy URL whatever precedes the `@` is a credential. 5a's `proxy_test.rb` asserts phrases and host names, never the raw value, and stays green |
| P5-104 | **The reserved-key table is the private `ReservedKeys` module, applied to all three of `OBS-5`'s sources**: at `Event#field` for a per-event field, once at `Logger.build` for the frozen global context (`Logger.context!` takes the redactor), and at `Event#emit` for the folded diagnostic context in either mode; `Event#field` and the two `Logger` privates read one table | `OBS-39`, `OBS-18`, `OBS-9`, `OBS-10`, `OBS-5`; §8.1's "no sink implementation can bypass it" (`observability/c2ebb968`, `db4bd229`); the reserved-key table above; P5-102; review round 1's R1-2 | The two requirements are stated on the emitted record and the round-0 repair closed only the `#field` entry point: `Logger.build(context: { "url.full" => …, "http.request.header.authorization" => … })` and, in the unfiltered or a listed fold, `Fiber[:"url.full"]` and `Fiber[:"http.request.header.authorization"]` handed the sink the raw value on every supported Ruby, against `event.rb`'s and P5-102's own "no sink and no caller can bypass either". The context meets the table once, because it is frozen and attached by reference, so `OBS-9`'s "not deep-copied per event" costs nothing more; the fold meets it per emit, on the enabled path only, where an Array of the record's keys is an allocation `OBS-1` does not constrain; a per-event field still wins the merge. Omit mode drops the key from either source. A `private_constant` beside `CollisionLatch`, with a `sig/` declaration and the smoke suite's unreachable pin; no manifest change |
| P5-105 | **`SURGERY_USERINFO` tolerates a leading run of whitespace and control bytes** (`[\x00-\x20\x7F]*`: SP, HTAB, the C0 controls and DEL) before the optional scheme and the `//`, and the surgery route writes those bytes back as they came | `OBS-11`, `OBS-16`; `HTTP-19`'s inbound value grammar; P5-100; review round 1's R1-3 | The pattern was anchored at the scheme, and a value the parser rejects BECAUSE of a leading OWS never matched it: `Headers.inbound_builder.add("Location", " http://user:secret@evil/x")` builds, `location` is allow-listed, and the value took the surgery route with its userinfo intact through the step at `HEADERS`. The two MVP transports strip the OWS after the colon; a hand-built or third-party `Headers` does not. Two bounded ASCII classes and one literal kept the pattern linear and BINARY-safe. **Superseded by P5-107**: the closing reading this row first carried — that a non-whitespace prefix (`x http://…`) is not an authority and is not matched — conflated R1-7's spellings with no RFC 3986 authority with a real authority merely preceded by bytes the parser rejects, and review round 2 falsified it; the pattern now tolerates no prefix because it is not anchored at one |
| P5-106 | **`Preview.decode` falls back to UTF-8 for the `EncodingError` family**: the declared charset is tried first and, when the interpreter has no converter for it, the bytes are decoded as UTF-8 with both replacements, the answer an unrecognised charset already takes | `OBS-38`, `XCUT-20`; `HTTP-24`; P5-31; review round 1's R1-4 | `Encoding.find` knows `utf-7` and `iso-2022-jp-2` as dummy encodings without a converter, so `MediaType#charset` is not nil for them and `#encode` raises `Encoding::ConverterNotFoundError` with `invalid: :replace, undef: :replace` set; phase 3b's `Response#body_string` encodes to the same declared encoding and does not raise for the same body, and through the step at `BODY` the response event became an `http.instrumentation.log` diagnostic. `dummy?` is not the discriminator — UTF-16 is dummy and decodes through its BOM — so the fallback is by rescue, over the family rather than the one name, because the fallback is total for the family; UTF-8 to UTF-8 with both replacements is a scrub and cannot itself raise |
| P5-107 | **`SURGERY_USERINFO` is unanchored and the surgery route substitutes EVERY `//`-authority's userinfo in a value the parser rejected, wherever it sits**, through `gsub` over one literal and one bounded class (`//[^/?#]*@`), every other byte written back as it came; the `[\x00-\x20\x7F]*` prefix class and the optional scheme of P5-105 are gone | `OBS-11`, `OBS-16`; `HTTP-19`'s inbound value grammar; P5-100, P5-105; review round 2's R2-1 | P5-105 tolerated exactly the whitespace-and-control prefix round 1 named, so any other byte before a real `scheme://user:secret@host` — RFC 3986 Appendix C's own `<…>` delimiters, quotes, parentheses, `x `, `%20`, `+`, `@`, a backslash, an NBSP, a BINARY obs-text byte — carried the userinfo to the sink through the step at `HEADERS` (sixteen inputs on both interpreters), and through the resolver's `shown` a QUOTED or bracketed `HTTPS_PROXY` (the dotenv and ConfigMap misconfiguration) wrote the client's own credential into the warning and the sink; a doubled URL kept its second authority behind the first, where an anchored `sub` never reached. Three rounds of the same class (R0-3, R1-3, R2-1) were three spellings of one fact: a value the parser rejected has no grammar left to honour, and `HTTP-19` admits every printable byte and obs-text, so the bytes before an authority are whatever a hostile server chose and a pattern that tolerates a listed prefix set will always be one prefix short. Linear: each `//` is checked in constant time and the run after it scanned once to the next `/`, `?` or `#` — measured at 10 KiB, 100 KiB and 1 MiB on 4.0.6 and 3.2.11, the worst shapes under 100 µs per KiB, an order of magnitude inside the per-pattern timeout at a size no header value reaches. What is still written back as given, and why, is P5-100's list narrowed to what RFC 3986 decides: no `//` before the `@` (`user:pw@h/p` — the resolver's grammar belt, P5-103, covers the proxy case), the backslash spellings, and what the parser ACCEPTS without an authority and which therefore never reaches this route — `http:///user:pw@h/p` (an empty authority, the credential in the path), `/http://user:pw@h/p` (a path-absolute reference), `//@user:pw@h/x` (which splits as a path), and a valid URL whose *path* spells a second authority (`http://user:pw@h/phttp://user:pw@h/p`, the `*100` spelling of round 2's battery), because `OBS-14` forbids altering the path. The one assertion the round's reading cost — `x http://user:secret@h/p` written back verbatim, P5-105's "a non-whitespace prefix is not an authority" — was that reading's error and is gone |
| P5-108 | **An `Array` under a header key is redacted one value at a time and joined with `", "` afterwards**: `ReservedKeys.header_value` maps each element through `Redactor#header_value` for an allow-listed name, and the `Emitter` hands every header's value list over unjoined, so the join is the event's and runs after the redaction | `OBS-11`, `OBS-16`, `OBS-17`; the checklist's as-built departure 24; review round 2's R2-2 | The `Emitter` joined a multi-valued header with `", "` BEFORE `Event#field`, so a URL-valued header with two values reached the redactor as one string no parser accepts, the surgery pattern substituted once, and everything after the first authority was written back: two `Location`s through a real pipeline at `HEADERS` logged `https://***:***@a/x, https://user:secret@b/y?***` on both interpreters. A response carrying two `Location`s is malformed, but a hostile server sends what it likes and `HTTP-19` admits it; `OBS-17`'s "redacted through the URL-value redactor" is stated per value, and the URL-value redactor reads one URL. Done at the table and not in the `Emitter`, because the `Emitter` "redacts nothing and gates nothing" by design (P5-95, P5-102) and a caller writing an `Array` into a header field by hand — which took the surgery route on its `#inspect` form — must get what the writer gets. A `case` and not `#is_a?`, because a `BasicObject` answers neither that nor `#nil?` and `OBS-6` renders it anyway |
| P5-109 | **`AsyncStep` registers its settlement work on the SOURCE future at every level, outside `Future#then`'s rescue, and the head's teardown is owned by exactly one side**: the head's `ensure` finishes the span and records the instruments only when the head raised before the chain handed back its future (`Pending` still nil), and the settlement callback does so otherwise, with `#attach` outside the head's `ensure` altogether | `OBS-20`, `OBS-30`, `OBS-34`; 4c's `Future#then`; P5-93, P5-94; review round 2's R2-3 | At `HTTPLogging::BODY` the settlement callback was registered on the DERIVED future, and `Future#then` fulfils the derived future — and so runs its callbacks — inside its own `rescue ::StandardError`, whose `derived.fail(error)` then loses the race to the fulfilment that just happened: a throwing meter's failure vanished, contradicting the class comment and the checklist's `OBS-20` row ("a tracer or meter that raises WILL propagate"), while at `HEADERS` the same meter raised into the producer as stated. And on an already-settled future — a transport that settles inside `#call`, which a real adapter does for a cached or immediately-failed request — the callback ran inline inside `#attach`, raised, left the `attached` flag false, and the `ensure` closed the scope and finished the span a second time: `span.finish` and `counter.add` twice, an OTel-API violation on the SDK's side. Both reachable only through an instrument that violates `OBS-30`, and `OBS-20`'s behaviour under that violation is specified. As built, `Hooks.notify` on the source collects a meter's raise, still runs every later callback, and re-raises it into the producer — the caller itself when the future settled inline, where the driver normalises it into a failed future (PIPE-30) and the request fails as it does on the sync path; the derivation is registered first so the source's callbacks run in the sync path's order (response event and the derived fulfilment, then the finish). The head is one private method with an explicit `begin`/`ensure` — Steep types no local across a method-level `ensure` — and the two sides cannot both reach the teardown because the ensure's condition is exactly "the future does not exist yet" |
...
**Review round 1 (2026-09-17)** found a credential the redactor never saw — 5a's proxy resolver
interpolating the raw proxy URL into every malformed-URL warning, and this phase's config diagnostic
carrying that text under `Keys::MESSAGE` — and three narrower readings: the reserved-key table running at
`Event#field` alone while `OBS-5`'s other two sources were merged raw, the surgery pattern anchored at the
scheme so a leading OWS carried a `Location`'s userinfo through, and `Preview.decode` raising for a charset
Ruby knows but cannot convert. All four were closed on the code branch (P5-103–P5-106). Two mutations that
survived the round — the async failure event's `OBS-24` bridge and the `BODY-35` `-1` filter — have a test
each on the tests branch and no ledger row; P5-100's closing sentence gains the three non-authority
spellings the round asked it to name. The checklist's deviations 32–37 and its third guard table carry the
round, ten mutations run red on 4.0.6 and 3.2.11.

**Review round 2 (2026-09-17)** found the surgery route's tolerated-prefix reading one prefix short for
the third round running — RFC 3986 Appendix C's own delimiters, quotes, a word, an NBSP, an obs-text byte
before a real authority, and a quoted `HTTPS_PROXY` writing the client's own credential into both
channels — and two narrower readings beside it: the `Emitter` joining a multi-valued `Location` before
the per-value redaction, and the async step's settlement work sitting inside `Future#then`'s rescue at
`BODY` while the head's `ensure` re-ran the teardown after an inline settlement. All three were closed on
the code branch (P5-107–P5-109): the surgery substitutes every `//`-authority wherever it sits, the
`Array` under a header key is redacted per value at the table, and the settlement work sits on the
source future with the teardown owned by one side. The round's fourth finding was the suite's, on the
3.2.11 floor: the `OBS-1` zero-allocation assertion's two-loop measurement came back negative in about
one whole-file run in fifteen, a one-time cost of 7 or 28 interpreter objects landing inside a measured
block after the warm-up, so 5c's shared `AllocationDelta` helper now returns the figure two consecutive
measurements agree on — a recorded change to a 5c support file, its five suites re-run green, the `M02`
guard still red. The checklist's deviations 38–41 and its fourth guard table carry the round, eight
mutations run red on 4.0.6 and 3.2.11.
...
and P5-91–P5-109 into design §10, and the §8.1 addendum that would state the split reassembly, the one-redactor
rule, the reserved-key table over all three sources, the async bridge and the proxy warning's redacted URL,
are a human's, as they were for 3a, 3b, 4a, 4b, 4c, 5a and 5c: `docs/sdk-design-ruby/` is frozen.
`docs/deviations.md` is left as phase 4 left it, for phase 10 to flip.
