...
conformance suite, lands the `OBS-19` header-drop policy phase 5b postponed to phase 8, and lands its own
half of the wire-boundary re-validation phase 1 postponed to the adapters.
...
- **`TRANSPORT-12`'s antecedent is live on the HTTP/1.1 path and absent on the HTTP/2 path of the same
  ...
  verbatim (verified fact 4). The wire-boundary re-validation is therefore not a
...
- **`dexpace-transport-async_http` cannot declare the repository's Ruby 3.2 floor.** `async-http 0.95.0`
  ...
  it is mechanised on both sides, and it is settled under `R15` as deviation `P8-36`; the gate that has
  to change is owned by the plan's Task 3, the per-gem Ruby floor gate edit.
...
- `docs/sdk-design-ruby/10-deliberate-deviations-from-the-reference-contract.md` items **3** (`:18-22`,
  ...
  **7** (`:63-67`), **10** (`:77-81`, the wire-boundary re-validation) and **12** (`:87-91`, stream ownership).
...
- `docs/sdk-design-ruby/12-appendix-requirement-coverage-index.md` — the `TRANSPORT` row (`:41`), the
  ...
  say about `TRANSPORT-8`; the charter's own finding — on phase 10's inbound list — already records that
  the same row is wrong about `Net::HTTP`'s retry, and this document adds a second, independent
  correction to it.
...
- `docs/deviations.md` and `docs/first-release.md` (the post-v1 and unsatisfied-MUST
  entries under § What v1 ships without, and § Post-release triggers), plus the plan tasks, knowledge
  notes and phase-10 inbound entries that carry the findings earlier phases raised.
...
- **`concurrency-and-async/f75816b6`** — the entry 8c must read and **not** act on: "dexpace-async-async
  ...
  ASYNC-6". That is the post-v1 `dexpace-async-async` (`docs/first-release.md` § What v1 ships without ›
  Post-v1 gems) and `SEAM-24`'s second sentence. 8c satisfies `ASYNC-6` for the
...
- **`pipeline/7ce4431d`** — `raise error, cause: nil` for an error a component is *carrying* rather than
...
the negotiated protocol, and the wire-boundary re-validation is load-bearing rather than belt-and-braces on
one of the two.
...
1. **`--gaps` measures corpus coverage, not specification coverage** (the roadmap's gap paragraph says
   the same from the other side, where `RECOV-17`–`34` are appendix-C only). Both
...
2. **The `*Conformance:*` clauses appendix C drops are load-bearing in six places here.** `TRANSPORT-7`'s
   ...
   those six tests.
3. **Corpus coverage says nothing about whether a *design* sentence is true.** The charter's two
   findings — §12's `Net::HTTP` retry rows and §3.2's `read_body` sentence, both on phase 10's inbound
   list — are design sentences that are wrong about `Net::HTTP`; this document adds a third and a fourth
   about `async-http` (§12's `TRANSPORT-14` scoping, also for phase 10's inbound list, and
   `Async::Task#cancel` superseding the deprecated `#stop`, which is a knowledge note). Only running Ruby
   finds them, and only against the exact versions the gemspec will declare.
...
and no `MUST NOT`. **No postponed item moves an ID into 8c and none moves one out.** `TRANSPORT-1`,
...
- **`TRANSPORT-8` is satisfied here and §12 says it is vacuous.** Appendix C's antecedent is "Where the
  ...
  in the SDK asked for it. The row says **satisfied**, cites verified fact 8, and names the §12
  correction it cannot make — phase 10's inbound list carries it. `R14`.
...
- **The wire-boundary re-validation call site in this adapter** — `Dexpace::HeaderSyntax` re-run over every
  header name and
  ...
  such.
- **`OBS-19`'s three-mode drop-logging policy**, which phase 5b postponed to phase 8 and which lands here
  because 8c is the first adapter in the repository that drops rather than raises. Built from 5b's two
...
| `SEAM-24`'s second sentence — a caller-facing cancellation bridge over the host's own primitive | **Post-v1, riding on `dexpace-async-async`** — `docs/first-release.md` § What v1 ships without, the `SEAM-24` entry. 8c satisfies `ASYNC-6` for the `Async::Task` **it creates itself** and bridges no caller-held task. |
| `OBS-29`'s operation-lifecycle tracer triple, and its transport-milestone group | One surface decision on phase 10's inbound list — the operation-lifecycle triple, and the fact that no transport adapter can reach an `HTTPTracer`. 8c's position under *Work phase 8c postponed* below: no route exists, so 8c wires no emitter and says so. |
...
| `BODY-12` clause 2 and `TRANSPORT-28`/`TRANSPORT-30` | **`8a`'s dispositions** (its design's `R5` and *Work phase 8a postponed*; the v1 declines are stated in `docs/first-release.md` § What v1 ships without). 8c reports that `protocol-http` ships `Protocol::HTTP::Body::File` with `#offset`, `#rewind` and `#rewindable?` (charter fact 15), so `TRANSPORT-28`'s reachable half is *more* reachable here — a report to `8a`'s `R5`, not a row. |
...
- **8c needs nothing from `8a` at design time, and two artifacts from it at execution time.** The
  `TCPServer` fixture and the conformance assertion protocol are **assigned to `8a`** by the charter, and 8c is
...
floor` line". `R15`, and the plan's Task 3 owns the gate edit.

### From phase 1
`Dexpace::HeaderSyntax`, "a module of pure functions, and **the public entry point every transport adapter
calls again immediately before dispatch** (phase 8; the re-validation phase 1 postponed to the adapters) …
public API in the full sense — YARD, RBS,
...
**Two unresolved findings 8c inherits:** `BufferedSource.wrapping` delivers one byte per read ("and it
will reach every transport phase 8 writes"), which 3a plan Task 10's `fill(count)` refill fix repairs —
8c's response body is built with `BufferedSource.over`, over an `#each`-yielding wrapper, **not**
`.wrapping`, which is why that defect does not bite here and why that is a decision rather than luck; and
§3.1's decode sentence, on phase 10's inbound list, which 8c does not touch because it decodes nothing.
...
may not assume its caller is a pipeline. **The `async_over` return-type check is open and named as phase
8's or a phase-2 amendment's — phase 2 plan, Task 11 owns it**: `Transport.async_over` accepts an *async*
transport silently and yields a future of a
...
and the once-per-key throttle, which is what the postponed `OBS-19` policy targets. Phase 8 writes the
three-mode policy at the
...
constraints in direct contradiction. `R15`, deviation `P8-36`; the plan's Task 3 owns the gate edit.
...
**on the HTTP/2 path, the wire-boundary re-validation is the only thing between a forged
...
- `async-2.45.1/lib/async/node.rb` carries `# @deprecated Use {#cancel} instead.` immediately above
  ...
  Recorded in `docs/knowledge/notes/concurrency-and-async.md`.
...
- **a non-ASCII byte in a header *name* (`X-B\xE9d: v`) raises `Protocol::HTTP1::BadHeader: Could not
  ...
  adapter.** Deviation `P8-38`, a named waiver in the conformance run, and §12's `TRANSPORT-14` scoping
  on phase 10's inbound list.
...
5. **Whether a proxy is reachable at all through `Async::HTTP::Proxy`.** `TRANSPORT-30` is `8a`'s ⏳ row,
   declined for v1 (`docs/first-release.md` § What v1 ships without), and 8c touched none of it.
...
runs on a cancellation while it does run on `Async::TimeoutError`.
`docs/knowledge/notes/concurrency-and-async.md` records it.
...
begin
  ...
  completer.fail(Errors.wrap(e))                  # pipeline/7ce4431d: never a bare `raise`
ensure
...
**2. `Dexpace.close_quietly` is still correct and 8c adds no second exit.** The concurrency note is
careful about this and 8c agrees: `close_quietly` rescues `StandardError` **from `#close`**, which is a different question
...
writes both has written one, as `docs/knowledge/notes/concurrency-and-async.md` records. 8c writes *no*
rescue of either: a cancellation propagates out of
...
and `async-http` has the path. §12 is frozen. **Phase 10's inbound list** carries the correction, which
is the same species as the charter's finding on §12's `Net::HTTP` retry rows and is routed the same way;
the difference, and it is in the port's favour,
...
| `TRANSPORT-14`'s malformed-**name** clause is unreachable here; `8a`'s row should say the requirement is satisfied on `Net::HTTP` and waived on `async-http`, not satisfied outright | fact 11 | `P8-38`, and §12's `TRANSPORT-14` scoping on phase 10's inbound list |
...
**8c is the consumer.** The charter **assigns** §9.3's `TCPServer` fixture and the conformance assertion
...
- **One thing this document had wrong in the other direction.** It reported `TRANSPORT-27`'s
  ...
  8c's driver's alone**, and this document's postponed-work section, `R14` report table and testing
  strategy are corrected to say so.
...
**4. Wire-boundary re-validation — the mitigation phase 1 postponed to the adapters.** `Dexpace::HeaderSyntax`
is re-run over **every** header name
...
| control / non-ASCII in a **name** | **unreachable**: `protocol-http1` raises `BadHeader` out of the read and the response never materialises | `TRANSPORT-14` waived here; `P8-38`, and §12's scoping of the ID on phase 10's inbound list |
...
with `.wrapping(io)` — which is what keeps `.wrapping`'s one-byte-per-read defect, the subject of 3a plan
Task 10's `fill(count)` refill fix, out of this adapter. §7.1's
...
*carrying* rather than one it just rescued uses `raise error, cause: nil` (`pipeline/7ce4431d`), so a
...
`Fiber#storage=`**, which means the warned setter `docs/knowledge/notes/observability.md` records
(`Fiber#storage=` warns; use per-key writes) — the hazard that dominates `8b`'s `R8` — does not
...
(`concurrency-and-async/f75816b6`), which is post-v1. **8c satisfies `ASYNC-6` for the `Async::Task` it
creates itself and bridges no caller-held task**, so it does not meet the second sentence. The charter
already narrowed the `SEAM-24` entry to say this — it is stated in `docs/first-release.md` § What v1 ships
without, the `SEAM-24` entry; 8c confirms it from the adapter's side and adds nothing.
...
gems/dexpace-transport-async_http/
  ...
  lib/dexpace/transport/async_http/request_mapper.rb     ::RequestMapper — steps 4-9 (wire-boundary re-validation, TRANSPORT-10/11/12/26)
  ...
  lib/dexpace/transport/async_http/drop_policy.rb        ::DropPolicy — TRANSPORT-13's three modes, OBS-19's postponed policy
...
Presence-gated auto-activation's restriction is honoured and named (phase 2's decision, carried with the
entry in `docs/first-release.md` § What v1 ships without): it is "for **instrumentation
...
#source               -> Dexpace::IO::BufferedSource          (built with .over, never .wrapping — one
                                                               byte per read)
...
### `Dexpace::Transport::AsyncHTTP::DropPolicy` — `TRANSPORT-13`, `OBS-19`

A frozen `Data` over phase 5b's two ingredients, which is exactly what phase 5b's hand-forward predicts
("Phase 8 writes a small `Data` over both").
...
| 20 | `OBS-19` rides on `TRANSPORT-13` | `DropPolicy`; the policy phase 5b postponed lands here. |
...
- **`TRANSPORT-14`** — the malformed-inbound-**name** clause, unreachable (fact 11, `P8-38`; §12's
  scoping of the ID is on phase 10's inbound list).
...
| `Dexpace::Transport::AsyncHTTP::DropPolicy` | `TRANSPORT-13`'s three modes and `#report`; the object `OBS-19`'s postponed policy closes on |
...
(§12's vacuity claim for it, corrected on phase 10's inbound list), and the HTTP/2 fixture, which is the
only thing in the MVP that can drive an `XCUT` audit over
a second protocol.
...
`dexpace-core`'s floor or to any other gem's. `R15`; the plan's Task 3 owns the gate edit.
...
stays visible. §12's `TRANSPORT-14` scoping is on phase 10's inbound list.
...
satisfied, not deviated from; the HTTP/2 validation gap is a library fact that makes the wire-boundary
re-validation load-bearing
...
The five rows above stand as decided; `R13`, `R14`, `R15` and `R16` were built as written, with `R13`'s
orphan close reached through one more channel than this document named and `R14`'s `TRANSPORT-8`
measured live. This sub-phase was cut from `main` at `a7cfeb6`, which holds every phase through 7 and
phase 8a, and executed concurrently with 8b off the same base, so it lands **second** of the two:
`Dexpace::TransportError`, `Events::TRANSPORT_HEADER_DROPPED` and `Keys::REQUEST_TIMEOUT` were 8a's
and were on the base; `Keys::TRANSPORT_CONNECTION_LIMIT` is this phase's one core widening; and the
wire-boundary re-validation is complete in both adapters with this phase's call site. The bundle
resolved `async-http` 0.105.0 over `async` 2.46.0 where this document measured 0.104.0, and two of the
thirteen verified facts above do not hold on that pair (rows P8-91 and P8-102's note). Execution added
the rows below, numbered from **P8-91** as the charter fixes it — 8a's as-built rows are P8-51–P8-65
and 8b's start at P8-71 — and nothing is renumbered. The checklist's "Deviations from the plan" is the
...
|---|---|---|---|
| P8-91 | **The cancellation bridge is queue-marshalled, in both directions.** The token's hook settles the pivot cancelled and pushes its reason onto a `Thread::Queue`; `Future#cancel` settles the pivot whose own `on_cancel` hook pushes; a transient watcher task spawned beside the exchange under the caller's task pops the queue and, on the reactor's thread, cancels the exchange task with `cause: Dexpace::CancelledError.new(reason)` while it is in flight or closes the delivered response afterwards. The queue is closed when the exchange ends undelivered or when the delivered body is released, and the watcher exits; nothing in the adapter calls `Task#cancel` from outside the reactor | `ASYNC-6`, `TRANSPORT-7`, `TRANSPORT-9`; the dispatch path's steps 12–13 and verified fact 8, which put `exchange.cancel` in the hook | `Async::Task#cancel` on a task whose fiber is not current is `Fiber.scheduler.raise`, and `Fiber.scheduler` is nil on every OS thread but the reactor's: a hook that reached the task directly raised `NoMethodError: private method 'raise' called for nil` on the canceller's thread and left the exchange running (measured on 3.3.12, 3.4.10 and 4.0.6), and the conformance suite cancels its token from a `Thread.new`. Closing the native body from the canceller's thread instead corrupted the reactor's selector. `cause:` must be an `Exception` to be readable from `$!.cause` inside the task — a Symbol is replaced by the runtime's own `Cancel::Cause` (fact 8 corrected) |
| P8-92 | **The owning adapter's client map is keyed by (reactor, origin), never by origin alone**, the reactor being `Fiber.scheduler` at the call compared by identity; `MAX_ORIGINS` (32) caps pairs, the drain evicts clients whose reactor has closed before the oldest, and every evicted client is released | `ASYNC-22`, `TRANSPORT-29`, `XCUT-14`; *The object model* → `Clients` | An `Async::HTTP::Client` belongs to the reactor whose tasks drive it: one client shared by two OS threads each in their own `Sync` hung (the cross-check's point 5), so a per-origin map made `ASYNC-22`'s "safe for concurrent calls from multiple threads" false. The manager's decision on the cross-check's open question 1, option (a). Two threads in two reactors through one adapter now each get their own client and mismatch nothing; fibers inside one reactor share one multiplexed client |
| P8-93 | **`TRANSPORT-8` is asserted in this gem's own suite and not in the portable one; the portable groups carry six assertions** — `TRANSPORT-7`, `-9`, `-21`, `-23` in `Asynchronous` and `-12`, `-13` in `HeaderDrops` — and `TransportSuite::PREAMBLE` names `TRANSPORT-8` as the third thing a green run does not prove | `TRANSPORT-8`; the plan's Task 19 Step 1a ("seven portable assertions"); 8a's `P8-9` and `R16` | The row's antecedent is a cancellation the host **runtime** originates while the SDK future is live, which only an adapter's own suite can produce — a portable assertion has no reactor to cancel a parent task in. The manager's decision (the brief's point 26): six portable assertions plus the preamble sentence, which is 8a's own mechanism for what a green run does not prove |
| P8-94 | **A response does not outlive the reactor that produced it, and the conformance driver honours that**: `settle:` on a thread with no reactor opens one, awaits the future, reads the body inside it through `Response#body_bytes` and hands the assertion a `BufferBody` of the same media type; inside `around:`'s reactor the response streams. The as-built page and the README state the rule for a consumer | `TRANSPORT-29`, `TRANSPORT-19`, `SEAM-11`; the plan's Task 19 `settle:` lambda; `R16`'s "the reactor `settle:` needs" | `Sync { }` returns only when the reactor has no non-transient work left, and on its way out it cancels the transient tasks — the pool's gardener among them, whose `ensure` calls `Pool::Controller#close`, which drains, and `drain` waits on every busy connection; the connection behind an unread body is busy. `TRANSPORT-29`'s eight-thread assertion hung under the plan's `Sync { value }` on every row until the body was read inside the reactor |
| P8-95 | **The per-gem Ruby floor is one row in `VERSIONS`** (`ruby floor:dexpace-transport-async_http 3.3`), read by the gemspec (`DexpaceVersions.ruby_floor(gem)`), by `gates:versions` and `gates:gemspec_audit`, and by the `Gemfile`, `test:gems` and `gates:clean_bundle` through `DexpaceVersions.gem_supported?`, which skip a gem on a row below its floor with the count printed | `P8-36`, `NFR-10`, `NFR-14`; the plan's Task 3, which wrote the literal `">= 3.3"` into the gemspec and left the `Gemfile` to a comment | `NFR-14` makes `VERSIONS` the single source of truth for every version-shaped fact, and a floor that lived in a gemspec literal and a gate table alike would drift; the two gate fixtures `per_gem_floor_ahead` prove the gates read it. On 3.2.11 `bundle install` resolves five gems, `test:gems` loads five suites, `gates:clean_bundle` prints "5 gem(s)", and every gate is green |
| P8-96 | **Every head `dexpace-conformance`'s `Scripts` writes carries `Connection: close`**, `Scripts.write_response` takes `close: true`, and a script that deliberately serves two responses on one connection passes `close: false` for every response but its last | `TRANSPORT-5`, `TRANSPORT-29`, `PAGE-36` and every multi-settle assertion; 8a's `Scripts` (8a's Task 6) | `WireServer` closes the socket after one exchange; without the header a client that pools keep-alive connections re-used one the server had already closed and read EOF on its next request — 12 of 20 immediate sequential GETs, measured — which made every multi-settle assertion a coin flip against this adapter. `Net::HTTP` builds a client per call and reads the header as nothing; every 8a count is unchanged |
| P8-97 | **No `Content-Type` is invented**: the caller's explicit header, then the body's own media type, then nothing — a body with neither goes out with no `Content-Type` | `TRANSPORT-10`, `TRANSPORT-26`; 8a's `P8-4` (`application/octet-stream` on every body-permitted method) | `async-http` stamps no `Content-Type` and emits no warning for its absence, so neither of `P8-4`'s reasons — the warnings-fatal build and a form type as a claim about the bytes — exists here, and a default the library did not need would be the adapter's own claim about a payload it has not read. The two adapters differ on this one header, and both as-built pages say so |
| P8-98 | **`test/support/async_http_warmup.rb` allocates and frees one `IO::Buffer` at this gem's test-helper load**, with `Warning[:experimental]` off around it | `NFR-6`; the CI matrix | On 4.0.6 alone the first `IO::Buffer` the scheduler allocates prints Ruby's once-per-process "IO::Buffer is experimental" warning to stderr, which the test base makes fatal in whichever test first drives a socket under the reactor; a test-support arrangement on the precedent of 8a's `net_http_warmup.rb`, not the SDK mutating a host global. The README tells a warnings-fatal host to expect the line once |
| P8-99 | **The `Steepfile`'s `:async_http` target downgrades `Ruby::UnknownConstant` to `:information`** with `library "openssl", "uri"`, and `rbs_collection.yaml` carries the plan's `- name: async / ignore: true` row | `NFR-3`, `NFR-11`; the plan's Task 17 | None of `async`, `async-http`, `protocol-http` or `async-pool` ships a `sig/`, and the one collection entry for the closure — `gems/async/2.12`, whose `Task` declares `#stop` and neither `#cancel` nor `.current?` — is ignored by name, because installing it would type the primitives this gem calls as missing and turn `steep` red (measured by review round 0's R0-2: it installed the moment the workspace gem's own `ignore: true` was lifted; rbs 4.2.0 cuts its dependency walk at an ignored gem, which is the only reason the committed tree never reached it, and the first cut of this row wrongly said the collection carried none). So every `::Async::HTTP::Client.new`, `::Protocol::HTTP::Request.new` and the `< ::Protocol::HTTP::Body::Readable` superclass is an unknown constant; the relaxation is the one 7a's `:serde_json` target established, on this target alone, with every runtime handle typed `untyped` |
| P8-100 | **`P8-37` as built: `Clients.release` retires every pooled resource through `Pool::Controller#retire` and then closes the pool** — never `pool.close` alone, never `Client#close` | `P8-37`, `TRANSPORT-16`, `XCUT-13`; *The object model* → `Clients` ("`client.pool.close`, not `client.close`") | `Pool::Controller#close` is `drain` then clear, and `drain` is "acquire every resource with zero usage, waiting on the condition while any is busy" — so `pool.close` under an open response waits exactly as `Client#close` does, which this document's fact 9 measured for the latter and not the former. `retire` deletes the resource and closes it without waiting; a close under an open response returns in under a millisecond and the open read then fails |
| P8-101 | **`ResponseBody#release` closes the native body through `Dexpace.close_quietly` with the adapter's `logger:`**, and a failure there is one `http.instrumentation.close` WARNING that never reaches `Response#close` | `TRANSPORT-16`, `TRANSPORT-25`, `XCUT-13`; *The object model* → `ResponseBody` | Closing an HTTP/2 body before it was read to the end resets the stream, and `async-http` 0.105.0 writes the `RST_STREAM` frame before it transitions the stream's state, so a peer's `END_STREAM` landing during that write closes the stream twice and releases the pooled connection twice — `RuntimeError: Trying to reuse unacquired resource` out of `Input#close`, five of five through a response obtained in a child task and closed unread by its parent. The connection is retired either way and the client stays sound; the raise is a library artefact a caller can do nothing with |
| P8-102 | **The public surface as built**, against *The object model 8c ships*: `AsyncHTTP.build(timeout:, logger:, drop_policy:, connection_limit:, ssl_context:, configuration:)` and `AsyncHTTP.using(client, logger:, drop_policy:)` — not `Adapter.new(**settings)` and `Adapter.over(client)` — with `AsyncHTTP.default`; `Adapter.owning` / `Adapter.borrowing` behind them and `Adapter.new` private; `Adapter::REACTOR_MESSAGE`; `DropPolicy.build(mode:)` with `MAX_TRACKED_NAMES`, `EVERY`, `ONCE_PER_NAME`, `QUIET`, `MODES`, `#mode` and `#report`; the entry file's `DEFAULT_TIMEOUT_SECONDS`, `DEFAULT_CONNECTION_LIMIT`, `MAX_ORIGINS`, `REGISTRY_KEY`, `FRAMING_HEADERS` and `ALPN_PROTOCOLS`; the private `Exchange::WATCHER_ANNOTATION`; and **no `_Client` interface** — a borrowed client is checked with `respond_to?(:call)`, `respond_to?(:retries)` and `retries.zero?` | *The object model 8c ships*, *The `sig/` shape*, *The interface surface later phases may cite* | 8a fixed the construction vocabulary — `.build` / `.using` over `.owning` / `.borrowing` — and one vocabulary for two adapters is what lets `docs/sdk-documentation/architecture.md` describe them together; `configuration:` is the injection the tests need for the two configuration keys; a `_Client` interface would name a duck no core type can check and `NFR-11` could not carry. The twenty-three rows the `async_http` manifest gained (2 → 25) were read one by one against this list, and `dexpace-conformance`'s manifest gained none — both new groups are private |

**Two facts this document states that `async` 2.46.0 does not bear out, corrected in
`docs/knowledge/notes/concurrency-and-async.md` and not deviations of this phase's**: verified fact 8's
`cause: :sym` reaching the task's `$!.cause` (it is replaced by `Async::Cancel::Cause`; row P8-91),
and verified fact 10's "`Async { }` inside a reactor is not a child of the caller" — `Kernel#Async`
inside a running task delegates to `Task.current.async`, measured `inner.parent.equal?(task)` true, so
the reviewer's mutation 14 is an equivalent mutant and the adapter's `caller_task.async` spelling is
the honest one rather than a distinction the runtime still draws.

**One property of the portable `TRANSPORT-7` row, found by review round 1 (2026-09-21) and not a
deviation**: against a streaming adapter the row's cancel lands either before the adapter has checked
its token on the delivered head or after the consumer's read has blocked — a race between the
server's signal and the client's head parse — so it proves the in-flight clause on every adapter and
the delivered-body clause by chance: half the runs on 4.0.6 and a third on 3.3.12 against a mutant
with the watcher's delivered-response close deleted, which hung the rest until the driver's `around:`
was bounded from a parent task (a bound raised into the assertion's own fiber meets the token-first classifier
and passes the row late). The body path's deterministic proof is the adapter's own
`cancellation_test.rb`, which refutes its own bound as the wake's cause; the row's comment states the
race, and a deterministic portable form is on phase 10's inbound list.

---

## Work phase 8c postponed, and who owns what it inherited

**8c postpones nothing new.** Every postponement 8c would want is already recorded somewhere with an
owner, and every one of its ten IDs is implemented or dispositioned here.

The roadmap's execution step 1 requires a phase to disposition everything earlier phases postponed, and
the **charter did that for phase 8** (its *What earlier phases postponed to phase 8, and what phase 8
decided*, covering every item once). 8c does not repeat it; it states only where its own work changes a
disposition the charter recorded, which is four items, each written so a reader needs no other record.

- **Wire-boundary re-validation — postponed by phase 1 (2026-09-05) to "each adapter's dispatch path";
  8c lands its half.** Why phase 1 postponed it: design §4 and §10.10 admit that constructor privacy
  cannot be closed in Ruby, and name one mitigation — `HTTP-17`/`HTTP-18`/`XCUT-18` validation run
  **again** inside every transport adapter immediately before dispatch — for which phase 1 shipped
  `Dexpace::HeaderSyntax` as public API and had no transport to call it from. Step 4 of the dispatch path
  is that call site (plan Task 9); `8a`'s is its Task 16; the portable forged-`Request` `XCUT-18`
  assertion is phase 9's Task 7. 8c's checklist carries its own row, and the work is complete once both
  adapters' call sites exist — the sub-phase that lands second says so in the phase status note (this
  plan's Task 19 Step 5a under the recommended order, mirrored in `8a`'s Task 25 Step 3a). **What 8c adds
  to the charter's account**: on the HTTP/2 path this is not a mitigation for a forged model, it is the
  **only** validation between the model and the wire (fact 4), which strengthens the obligation rather
  than changing its disposition.
- **`OBS-19`'s header-drop verbosity policy — postponed by phase 5b (2026-09-09); landed and closed by
  8c.** Why phase 5b postponed it: `OBS-19`'s subject is "*a transport that drops*", core has no transport
  and drops no header (the wire-boundary re-validation **raises**), and a public, `NFR-4`-locked
  three-mode policy with no caller and no test is exactly the shape the phase-5 segmentation design's
  `R10` forbids; 5b shipped the two ingredients — `Severity`'s `WARNING`/`VERBOSE` and the once-per-key
  latch — and postponed the policy to "phase 8, at the first adapter that drops a caller-set header rather
  than raising on it", naming `TRANSPORT-12` as that adapter's subject and `TRANSPORT-13` as the
  transport-side twin (the ID was corrected from `TRANSPORT-8` in place on 2026-09-12, and the same wrong
  ID in phase 5b's forward table, `…phase5b…-design.md:2110`, was corrected in the same change). The
  condition is met (fact 5), and `DropPolicy` is the object — plan Tasks 7, 9 and 15, bounded at 64
  distinct folded names per adapter instance. One fact travels with the closure so a reader does not
  over-generalise: **the antecedent is live on the HTTP/1.1 path and absent on the HTTP/2 path of this
  same adapter**, and 8c drops on both anyway (`P8-40`), so "the first adapter that drops" is true of the
  gem and not of every exchange it performs.
- **`OBS-29`'s transport-milestone group — postponed by phase 5c (2026-09-09) to "phase 8 with the first
  adapter"; not wired by 8c, and the reason is that no adapter can reach a tracer.** The charter makes
  8c's emitter call sites conditional on `R6` ("once `R6` settles the route"), and `R6` is `8a`'s. The
  charter's finding, on phase 10's inbound list, records that **no route
  ...
  answer the charter says it expects and the same answer phase 10's inbound list already records for
  `OBS-29`'s operation-lifecycle triple. The per-attempt group is phase 6a's Task 9; both residuals — the
  operation-lifecycle triple and the transport-reachable `HTTPTracer` —
  are one surface decision on phase 10's inbound list (the roadmap's 2026-09-13 status note). If `8a`
  settles `R6` the other way, 8c's plan gains one task and this paragraph is what it amends.
- **`SEAM-24`'s cancellation bridge — declined for v1 by the MVP scope design (2026-09-05); 8c confirms
  the adapter half.** 8c satisfies `SEAM-24`'s **first** sentence by construction (`Fiber[]` inheritance
  across a child task, fact 10) and `ASYNC-6` for the `Async::Task` it creates itself. It bridges no
  caller-held task, so it does **not** meet the second sentence, which design §3.3 `:252-254` assigns to
  the post-v1 `dexpace-async-async`. What stays declined is narrower than the original entry read — the
  "beyond fiber storage" scope and the caller-facing bridge — and is stated in `docs/first-release.md`
  § What v1 ships without, the `SEAM-24` entry; `SEAM-28`'s identifier half is phase 5c's Task 4, done.

**Four more items 8c checked and left alone**, stated because a reader will ask:

- **`TRANSPORT-28`'s zero-copy clause and `TRANSPORT-30`** — `8a`'s ⏳ rows, declined for v1 under
  `docs/first-release.md` § What v1 ships without, the `TRANSPORT-28`/`TRANSPORT-30` entry, on a condition
  ("a transport adapter beyond the two MVP transports") phase 8 does not meet. 8c reports that
  `Protocol::HTTP::Body::File` makes `TRANSPORT-28`'s reachable half *more* reachable here (charter fact
  15) — a report to `8a`'s `R5`, not a change of disposition.
- **`dexpace-async-async`** — the post-v1 gem (`docs/first-release.md` § What v1 ships without › Post-v1
  gems; design §2.2 is the authority). 8c is the gem most likely to be mistaken for it and is not it:
  ...
  **bridge** a caller's own `Async::Task` to the pivot. The charter says the same thing from the
  segmentation side; 8c says it here so the post-v1 entry is not read as met.
- **The unsatisfied MUSTs, `ASYNC-3` and `PIPE-33`'s interrupt clause** (design §10.5;
  `docs/first-release.md` § What v1 ships without › Unsatisfied MUSTs). 8c adds no unsatisfied MUST and
  re-opens nothing; both rows are `8b`'s. Worth one sentence because it is counter-intuitive: 8c is the
  one adapter in the MVP that *can* abort an in-flight blocking operation (fact 8), and that does **not**
  close `ASYNC-3`, whose antecedent is "a blocking task on a worker thread" and whose row belongs to the
  gem that has one.
- **Presence-gated auto-activation** (phase 2's decision, post-v1 under `docs/first-release.md` § What v1
  ships without, and the restriction travels with it) — instrumentation-only and "no transport or codec
  adapter may ever use it". 8c registers explicitly at require time and never because `Async::HTTP` is
  defined. Named because "activate because `async-http` happens to be loaded" is exactly the convenience a
  reader of this gem would reach for.
...
**Owner: this sub-phase's plan, Task 3 — the per-gem Ruby floor gate edit, with Task 18's two
`docs/first-release.md` lines.**

> ### `dexpace-transport-async_http` cannot declare the repository-wide Ruby 3.2 floor, and a phase-0 gate asserts that it must
>
> - **Opened:** 2026-09-11, phase 8c design
> - **Status:** open
> - **Cites:** NFR-2, NFR-10, NFR-14, P0-9, P8-36; the post-v1 `dexpace-async-async` (`docs/first-release.md` § What v1 ships without › Post-v1 gems)
>
...
**Owner: phase 10's inbound list in the roadmap — §12's `TRANSPORT` row gains `TRANSPORT-14` to its
adapter-scoped list; `docs/deviations.md` carries the interim note beside `P8-38`.**

> ### `TRANSPORT-14`'s malformed-inbound-header-**name** clause is unreachable on `dexpace-transport-async_http`, and §12 records `TRANSPORT-14` as satisfied without qualification
>
> - **Opened:** 2026-09-11, phase 8c design
> - **Status:** open
...
**Owner: `docs/knowledge/notes/concurrency-and-async.md` — an entry beside
`concurrency-and-async/f75816b6` recording that `Async::Task#cancel` replaces `#stop`.**

> ### design §3.3 and the corpus name `Async::Task#stop`, which `async` 2.45.1 deprecates in favour of `#cancel`
>
> - **Opened:** 2026-09-11, phase 8c design
> - **Status:** open
> - **Cites:** ASYNC-6, SEAM-24, TRANSPORT-7; the `SEAM-24` and `dexpace-async-async` entries in `docs/first-release.md` § What v1 ships without
>
...
> same species as the charter's two findings — §12's `Net::HTTP` retry rows and §3.2's `read_body`
> sentence — a frozen design chapter that is wrong about a
> library — and it is smaller than either: the *mapping* §3.3 describes is right, only the method name has
> moved. Phase 8c calls `#cancel` everywhere. What would resolve it: `dexpace-async-async` (post-v1) is
> written against `#cancel`; §3.3's sentence is corrected the next time §3 is deliberately
...
**Owner: phase 10's inbound list in the roadmap — §12's `TRANSPORT` row and its MUST-level vacuity count;
`docs/deviations.md` carries the interim note.**

> ### `TRANSPORT-8` is satisfiable on `dexpace-transport-async_http`, and §12 counts it among the eight MUSTs that hold vacuously
>
> - **Opened:** 2026-09-11, phase 8c design
> - **Status:** open
...
> vacuous one — the inverse direction from the charter's finding on §12's `Net::HTTP` retry rows, and
> equally a defect in a frozen chapter. What would
...
**Recorded rather than proposed — the HTTP/1.1-specific sentence on `OBS-19`'s drop policy.** An earlier
revision proposed this as an amendment to the record of the policy phase 5b postponed; it was applied on
2026-09-13 and now lives in this document's *Work phase 8c postponed* entry for that policy, beside the
requirement-ID correction (`TRANSPORT-12`/`TRANSPORT-13`, not `TRANSPORT-8`) that the charter made and that
phase 5b's forward table (`docs/work/mvp/phase5/phase5b/2026-09-09-phase5b-logging-and-redaction-design.md:2110`)
received in the same change. The sentence: *the condition is
...
**Owner: `docs/first-release.md`.** Two lines, both owed by the phase-level PR rather than by
this sub-phase:

- **A supported-Ruby line.** `dexpace-transport-async_http` requires Ruby **>= 3.3**, narrower than every
  other gem in the workspace (`P8-36`; the plan's Task 3 makes the gates accept it). A consumer on Ruby
  3.2 composes `dexpace-core`,
...
**What `ruby .claude/skills/housekeeping/probe.rb` reported while these findings were carried as register
rows, and why it no longer does** *(updated 2026-09-13)*. Re-run on 2026-09-12 with all seven phase-8
documents in place, the probe reported **18 findings across two checks**, every one of them a consequence
of those documents existing and none fixable from inside this one.
...
- **`citations` (17)** — register ids cited across phase 8 with no row filed for them, the charter's four
  findings and each sub-phase's in sub-phase order. **That group is gone rather than pending**: the
  find-list was retired on 2026-09-13 and every phase-8 finding was routed to an owner instead — a
  numbered plan task, phase 10's inbound list, a knowledge note or `docs/first-release.md` — so nothing
  about phase 8 waits on a paste. Phase 7's findings deliberately carried no numbers and produced no such
  finding.

`links`, `inbox`, `root`, `readmes`, `registers` and `guard` were clean.


**Two inherited findings explicitly do not close.** The one-byte-per-read defect in
`BufferedSource.wrapping` is phase 3a's code to fix — 3a plan, Task 10's `fill(count)` refill fix; 8c
avoids it by using `.over` and says so rather than closing it. `Transport.async_over` accepting an async
transport silently is phase 2's or a phase-8 amendment's; 8c is the first object that can be fed to it by
mistake and does not fix it — the repair is in `dexpace-core` and belongs with phase 2 plan, Task 11's
return-type check.
...
rule does not name, and that the wire-boundary re-validation changes role across it — from the
...
- **`TRANSPORT-12`'s "stricter wire grammar" is per-protocol, not per-adapter, and on the looser protocol
  the wire-boundary re-validation is the only defence.** Annotates `transport-adapter/cb7901ef`. Verified 2026-09-11 on
  ...
  protocol and no validation at all on the other, and the wire-boundary re-validation is, on the
...
2. **What exactly does `VERSIONS` gain, and who edits `gates:versions`?** `R15` fixes the shape
   (`ruby floor dexpace-transport-async_http  3.3`) and records that the gate itself is phase-0
...
4. **Does `DropPolicy` live in this gem or in `dexpace-core`?** Phase 5b's hand-forward says "Phase 8 writes a small
   ...
   same object. **This document's recommendation: this gem**, because the shape `TeeSink#clear_tap` is
   in — public, `NFR-4`-locked API with no caller, which is what 3a plan Task 14's keep-or-drop decision
   is about — is exactly what a core-resident policy with one external caller
   would be, and moving it to core later widens rather than narrows. The plan states the decision either
   way, and this document's postponed-work entry for the policy records it.
