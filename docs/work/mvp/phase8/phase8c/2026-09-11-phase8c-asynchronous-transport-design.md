...
| P8-99 | **The `Steepfile`'s `:async_http` target downgrades `Ruby::UnknownConstant` to `:information`** with `library "openssl", "uri"`, and `rbs_collection.yaml` gains no row | `NFR-3`, `NFR-11`; the plan's Task 17 (`- name: async / ignore: true`) | None of `async`, `async-http`, `protocol-http` or `async-pool` ships a `sig/` and the collection carries none, so there is nothing to ignore and every `::Async::HTTP::Client.new`, `::Protocol::HTTP::Request.new` and the `< ::Protocol::HTTP::Body::Readable` superclass is an unknown constant; the relaxation is the one 7a's `:serde_json` target established, on this target alone, with every runtime handle typed `untyped` |
