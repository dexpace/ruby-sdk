...
rather than numbering a second row for the same trade. The checklist's "Deviations from the plan" is the
...
**One statement of this document corrected by measurement and not a deviation.** The plan's Task 7
Step 8 — widening the timer's critical section to wrap `pop(timeout:)` "deadlocks two fibers of one
thread under a probe scheduler" — is false: the pop runs on the TIMER thread, which has no scheduler
and never routes through the caller's `#block` hook, so the mutant stays green. The two-fiber test is
...
the *Thread-safety proof obligations* table stands on the source, and the plan's mutation is not among
the guards run red. The design's finding 2 (`AsyncOver`'s check before dispatch) was built by phase 2
and its finding 3 (the thread count at `teardown`) by phase 0; both are verified closed, not routed.
