...
The six rows above stand as decided; `R8`, `R9`, `R10`, `R11` and `R12` were built as written, with
`R11`'s settlement value and its timer's containment needing what this document did not name. This
sub-phase was cut from `main` at `a7cfeb6`, which holds every phase through 7 and 8a, and executed
concurrently with 8c off the same base, so it landed **second** of the phase-8 lanes: the charter's
convergence point 2 — `Transport.async_over(net_http_transport, executor: thread_pool)` over a real
socket — is this lane's. Execution added the rows below, numbered from **P8-71** as the charter fixes it
(8a's as-built rows ran `P8-51`–`P8-65`, 8c's start at `P8-91`), and extends `P8-22` and `P8-25` in place
...
| # | Deviation | Requirement / document | Why |
|---|---|---|---|
| P8-71 | **`#delay`'s future settles with `true`, never nil, and a zero delay settles inline with no timer entry** | `ASYNC-18`, `SEAM-16`; this document's "What `#delay` returns and what it settles with", which said nil, and `R11`'s clause table, which said the same | `Completer#fulfil(nil)` raises `Dexpace::InvalidArgumentError` — SEAM-16's "MUST NOT complete successfully with a null/absent value", structural in the pivot since phase 2 — so the sentence "settled with nil" described a call that cannot be made: the plan's zero delay raised synchronously and its positive delay killed the timer thread from inside `Completer#fulfil`, stranding every later delay and re-raising through `#close`'s join. Phase 5a's `Dexpace::Async.delay` settles with `true` (P5-52) for the same reason, and this document's own argument — the two primitives interchangeable at the call site — is what fixes the value: `ELAPSED = true`, a `private_constant` of the pool. The zero delay was always inline; it is stated here because the reviewer's guard 18 rests on it |
| P8-72 | **The public constant is `REQUIRED_CORE`**, not this document's `CORE_REQUIREMENT` | `P8-21`; §2.3's version-skew guard; `gems/dexpace-serde-json/lib/dexpace/serde/json.rb`'s `REQUIRED_CORE` | 7a shipped the one idea under one name and its suite pins it equal to the gemspec's requirement; a second spelling for the same `~> M.N` string would be two public constants for one fact across two gems, and `NFR-4` would lock both. Renamed, never aliased; the design's spelling appears nowhere in `lib/` |
| P8-73 | **The composed test at convergence point 2 lives in this gem's test tree and names `Dexpace::Transport::NetHTTP`, `Dexpace::Conformance::WireServer` and `Scripts`, and the repository's `test/support/net_http_warmup`** — `gems/dexpace-async-thread/test/dexpace/async/thread/composed_transport_test.rb` — where this document said "nothing in `8b` opens a socket, needs a wire fixture, or names `Dexpace::Transport::NetHTTP`" | The charter's convergence point 2 ("belongs to whichever of `8a` and `8b` lands second"); `ASYNC-1`, `ASYNC-2`, `ASYNC-5`, `ASYNC-14`, `ASYNC-19`, `ASYNC-20`, `PIPE-33`; P8-62 | 8a landed first, so the charter's rule for point 2 wins over this document's independence sentence, which was written before the order was known. The reach is through the root `Gemfile`'s path-load of every `gems/*` directory under `bundle exec`, exactly as 8a's own `adapter_fixtures.rb` reaches `dexpace/conformance`; `lib/` still names neither sibling, so `gates:clean_bundle` — whose subprocess has only core and this gem on the path and cannot see `test/` — is unaffected, and the gem's own `rake test` runs the file because the workspace resolves both gems. Ten cases: a 200 delivered as the exact object the adapter built, a scripted read failure as the adapter's own retryable `TransportError` bounded by a per-call timeout that reached it, a refused connection, a closed pool opening no socket, the options object `assert_same`, a cancel in the window after the adapter produced a pump-backed response (closed once, connection released, gated on the worker being inside the send), a late cancel leaving the body readable, `sync_over` round-tripping and surfacing a cancellation under a blocked send, and `Pipeline.standard` over the adapter as one posted unit. The one file in the gem that opens a socket is the one that requires the warm-up |
| P8-74 | **Every worker-side read of the diagnostic context in the suite goes through `Diagnostics.capture`, the "empty" the two clears return a worker to is a map of nil-valued keys on the 3.2 floor, core's `dexpace.`-prefixed slots travel in the snapshot, and the ensure-clear proof writes a key the worker has never held** | `ASYNC-9`, `ASYNC-11`, `OBS-23`, `OBS-24`; `P8-20`; `P5-72`, `P5-97`; `observability/65191069`'s re-run, which this phase was to clear and phase 5c cleared first (2026-09-17) | Measured on 3.2.11: `Fiber[:k] = nil` retains the key with a nil value, so `Fiber.current.storage` on a cleared worker reads `{tenant: nil, …}` there and `{}` on 3.3+; four of the plan's five diagnostics tests, written against the raw map, failed on the floor, and the ensure-clear proof stayed green there because `:tenant` — set at pool-build time and cleared — sat in the prior map as a retained nil that the union restore reset. `.capture` compacts nil-valued keys (P5-97), so it reads the same on every row and is the reader the suite uses; the proof writes `:leaked_by_work`, which no boundary has seen, and is red on every row with the ensure clear gone. `R8`'s "the union restore returns the worker to empty" holds at every reader that skips nulls — `Fiber[]`, `OBS-10`'s fold, `.capture` — and not at the raw map, which is the thing the note 8b files says. And `.capture` carries 5c's `dexpace.current_span` slot with everything else, so a span active on the caller is current on the worker — `ASYNC-8`'s purpose, and why the diagnostics suite drops the reserved prefix before comparing: under `rake test:gems` another suite can leave a no-op span on the main fiber |
| P8-75 | **`Timer#schedule` after `#stop` refuses the entry through its own `on_shutdown` and spawns no thread; `#stop` takes and clears the entry list under the mutex and fails what it took on the closing thread; entries are removed by identity; the timer measures against the pool's injected `clock:`; and `#release` joins the workers inside what remains of the budget after the sentinel drain** | `ASYNC-15`, `ASYNC-16`, `ASYNC-18`; `R11`'s lifecycle paragraph; `R12`'s teardown sequence; `XCUT-11` | Three mechanisms this document's timer sketch did not have and the build needed. A `#delay` can read the pool's latch open a moment before `#close` runs `stop_timer`; without the stop flag `@thread ||= spawn_thread` would spawn a timer thread after the timer was stopped, a leak `#close` never joins. Taking the list under the mutex makes an entry either the thread's (fired) or the closer's (failed), never both, which is what "exactly one settlement" needs under the race; the callbacks the closer runs are run on the closing thread, so a caller's stuck `#on_settle` on a delay parks `#close` — the caller's own doing, as on any `Completer#fail`, and the README says where each callback runs. Entries are removed by identity (`delete_if … equal?`) as a statement of intent, not a guarded property: every entry carries two lambdas built for it alone, so `Data#==` is already identity in practice and a delete by equality is an equivalent mutant (review round 0's R0-5 measured it surviving the delay suite). The bounded join after the sentinel drain is what makes `Thread.list.size` stable when `#close` returns: the sentinel is pushed inside the worker's `ensure` and the thread exits a moment later, and the base's per-test thread count read those as two moments — a gap the GVL closes in practice, so the join is pinned by a test that widens it deterministically rather than by any plain count (review round 0's R0-3; checklist guard 32) |
| P8-76 | **`#close` completes when issued from the pool's own threads**: `Timer#stop` skips its bounded join when the timer thread is the current one, and the drain counts the calling worker as already exited and never joins it — where this document's timer sketch and `R12`'s teardown sequence said "join the thread" and "wait for every worker's exit sentinel" with no self case | `ASYNC-15`(a), `ASYNC-16`, `SEAM-25`, `XCUT-13`; `R11`'s lifecycle paragraph; `R12`'s teardown sequence; the README's "Where callbacks run"; review round 0's R0-2 and R0-8 | The README sends a `#delay` future's `#on_settle` handler to the timer thread and offers the grace-period idiom, and a handler that closes the pool is exactly that idiom. As first cut, `Thread#join` on the current thread raised `ThreadError` out of `#close` with `Closeable`'s latch already flipped, so no shutdown event was ever emitted, the other outstanding delays were never failed (a 10 s delay stayed unsettled past a 1.5 s deadline) and the timer thread was gone — `ASYNC-15`(a)'s "safe" and `SEAM-25`'s "the first close emits" both false on that path, on 4.0.6 and 3.2.11. The thread needs no join to stop: the wake queue is closed regardless, the next pop answers nil and `#run` exits once the handler returns; the leftovers are failed on the closing thread as on any other. One thread over, a `#close` issued from inside a posted task waited for its own worker's exit sentinel, which cannot be pushed until `#release` returns, so every such close burned the whole `shutdown_timeout` and reported `drained: false`; the drain now counts that worker as exited and joins the others only, and it exits by itself once the task returns, running whatever the closed queue still holds first. The event's `drained` stays honest under that reading: nothing was abandoned, and every other worker is gone. Two tests, one per thread kind, with the mutations red on both rows (checklist guards 30 and 31) |
| P8-77 | **`#delay` refuses a non-finite (`NaN`, either infinity) or non-real (`Complex`) duration beside a negative or non-`Numeric` one, and `.build` refuses the same shapes for `shutdown_timeout`** — where `R11`'s clause table says "a negative or non-`Numeric` duration raises" and the object model says "`shutdown_timeout` a non-negative `Numeric`" | `ASYNC-18` ("MUST reject"), `ASYNC-2`, `ASYNC-15`(a), `XCUT-13`; `R11`'s clause table and the object model's `.build` sentence; `P8-22`'s extension; review round 1's R1-1 | The two checks the table names admit a `NaN`: it is a `Numeric`, `negative?` is false and so is `zero?`, so it was scheduled. Measured on 4.0.6 and 3.2.11: the entry's `NaN` deadline made `next_wait`'s `max` raise inside the timer thread's net — one `http.instrumentation.hook` diagnostic, the thread gone for good with its slot still set, the future never settled — and every later `#delay` on that pool raised a bare `ArgumentError` synchronously from `sort_by!`, the dead-timer outcome `P8-22`'s extension exists to prevent, reached through an argument rather than a callback. A `Complex` is a `Numeric` with no order and no `#negative?`: a bare `NoMethodError` from a method whose contract is `InvalidArgumentError`. `validate_duration!` asks `real?` before `negative?` and `finite?` after it, one message per clause, before anything is scheduled; `finite?` is `Numeric`'s own total protocol (`Integer` and `Rational` true, a `Float` or `BigDecimal` `NaN` or infinity false), which is why Infinity is refused with `NaN` rather than admitted as an entry that never fires — an entry that never fires is not a delay, and one method covers both; an Integer beyond Float's range stays finite and admitted, its deadline saturating to Infinity in the clock's Float arithmetic, stated in the source and not screened. One keyword over, `non_negative_numeric!` admitted the same shapes for the close budget: a `NaN` made `#release`'s deadline arithmetic raise a bare `ArgumentError` out of `#close` with `Closeable`'s latch already flipped — `ASYNC-15`(a) false again, by an argument — and an infinity is the unbounded close `XCUT-13` forbids for this gem by name, so the helper is `finite_non_negative!` (real, non-negative, finite). Core's `Async.validate_delay` and `Clock::Guard.duration`, the precedent the clause table cites, share the hole — `Queue#pop(timeout: Float::NAN)` parks indefinitely on every row — and are phase 10's, by date and content; 8b edits no core file. Four tests, red against the previous validation on both rows (checklist guards 33 and 34) |
| P8-78 | **The timer thread carries a delay's diagnostic context the way a worker carries a task's: `Timer#schedule` captures `Diagnostics.capture` per entry on the scheduling caller's thread, the `Entry` carries the snapshot, the timer thread clears its inherited storage once at start and again after every fired callback, `Timer#fire` runs `on_fire` under `Diagnostics.with` over the entry's snapshot, and the shutdown callback runs on whichever thread stopped the timer under the same install-and-restore with that thread's own context put back** — where this document's timer sketch, `R11` and the `Timer` class comment gave the second thread the gem owns no context discipline, and `P8-20`'s two-boundary floor was stated for the workers alone | `ASYNC-8` ("work **or callbacks** … reinstating it on the executing/**callback** thread"), `ASYNC-9`, `ASYNC-10` ("a stale snapshot from when it was assembled"), `ASYNC-12`; `P8-20`; this document's finding 1 and the knowledge note it files; review round 2's R2-1 | Measured on 4.0.6 and 3.2.11 against the round-2 tree: the first positive `#delay` spawns the timer thread from *that caller's* fiber, `::Thread.new` copies the caller's `Fiber[]` into it, and nothing ever cleared it or installed anything — so caller B's `#on_settle` handler and a contextless caller C's both read caller A's trace id and tenant, a `#then` continuation on a second request's delay read the first request's, and a key one handler wrote was visible to every later one; the same two hops through `#post` read the caller's own context, because the workers had the floor. This is the mechanism finding 1 names ("a `::Thread.new` carrier inheriting `Fiber[]` at creation and never cleared … the carrier must start empty") in the one place this phase created a carrier without applying it, and the lazy spawn from a caller's fiber makes it the worse case of the two: what the thread inherits is an arbitrary request's context, not the assembler's. The repair is `Pool#run`'s shape on the timer — per-submission capture at the point that identifies the logical caller (`Timer#schedule` runs synchronously inside `Pool#delay`), install-and-restore through 5b's `Diagnostics.with` unchanged, and the two clears on the thread the gem owns; the shutdown callback's thread is a caller's, a worker's or the timer's own, so it is restored and never cleared. `Timer`'s `schedule(delay, on_fire:, on_shutdown:)` signature is unchanged; `Entry` gains `snapshot`; `Metrics/ClassLength` on `Timer` is an inline disable with its reason. Five cases in the diagnostics suite's `TimerContextTest`, all red against the round-2 timer on both rows; checklist guards 36–40 |

**Two rows extended rather than re-numbered**, as the ledger's own rule ("a ledger id … can never be
renumbered") and the manager's decision of 2026-09-21 direct:

- **`P8-22` reaches the timer thread.** `Timer.new(name:, on_error:, clock:)` takes the pool's contained
  diagnostic (`Pool#report_failure`, `Instrumentation.contain` under `INSTRUMENTATION_LOG`, ERROR under
  `INSTRUMENTATION_HOOK`) as `on_error:`; every callback the timer runs — `on_fire`, the refused
  `on_shutdown`, the closer's `on_shutdown` — runs inside `guarded` (`rescue ::Exception`), the thread
  body sets `report_on_exception = false` and is netted itself, and `#stop`'s bounded join can never
...
- **`P8-25` names where a delay's callbacks run.** A `#on_settle` on a `#delay` future runs on the
  **timer** thread, not on a pool worker; when `#close` fails an outstanding delay the handler runs on
  ...
  A handler that closes the pool from the timer thread completes the close there (`P8-76`). On either
  thread the handler runs under the diagnostic context of the caller who asked for the delay (`P8-78`).

**One statement of this document corrected by measurement and not a deviation — and the correction
itself corrected by review round 0.** The *Thread-safety proof obligations* table says a timer that
held its mutex across `pop(timeout:)` "would deadlock" two fibers of one thread under a scheduler,
because the pop "routes through the scheduler's `#block` hook", and the plan's Task 7 Step 8 wrote the
two-fiber test as the proof. The **mechanism** is wrong: the pop runs on the TIMER thread, which has no
`Fiber.scheduler` and never touches the caller's `#block` hook, so the per-fiber `ThreadError` the
sentence predicts cannot occur and the two-fiber test cannot see the scope. The **conclusion** — the
mutation is red — is right, by a different route: with the mutex held across the wait, `Timer#stop`
parks on the mutex the parked timer thread holds, `#close` never returns, and the delay suite hangs in
`teardown` — three runs in three with a thread dump, the main thread in `Timer#stop`'s
`Thread::Mutex#synchronize` against the timer's indefinite `Queue#pop` (checklist guard 29). The first
cut of this paragraph said the mutant "stays green" and routed a phase-10 bullet on that: a false
measurement, review round 0's R0-1, and the bullet was removed before it reached `main`. The two-fiber test is
kept as a liveness check with a deadlined `Future#value` (an untimed gate pop parks under the probe
scheduler with a nil deadline and its run loop breaks before the timer's cross-thread unblock arrives);
the lock scope is asserted since the round by `LockScopeTest` — a cancel and a close issued from helper
threads while the timer is provably parked, both inside bounded joins, red by two reported failures
under the mutation where the rest of the file is red by the hang — and the table's row for the timer's
mutex stands with its "never held across" column proven rather than argued. The design's finding 2
(`AsyncOver`'s check before dispatch) was built by phase 2 and its finding 3 (the thread count at
`teardown`) by phase 0; both are verified closed, not routed.
