...
**Re-audited 2026-09-25, when a human applied the amendment set and consolidated the phase ledgers into
§10.** Every entry was checked again against as-built source (`gems/**/lib`, `sig/`, the suites), never against the
phase documents. Of the nineteen, **seven held exactly as written** (3, 7, 8, 11, 14, 16 and 19); **five held and
gained a dated clause** carrying a narrower as-built fact a phase ledger recorded (1, 2, 4, 9 and 15); and **seven
were corrected in place**, each with a dated note quoting what it replaced — 5 (its "only entries" sentence, and
`C12`'s attribution), 6 (`C14`), 10 (`C19`), 12 (`C3`), 13 (`C18`), 17 (whose last clause described a close path that
does not exist) and 18 (`C15`). The chapter's opening claim — two unmet MUSTs and nothing else narrowed — was
corrected too. **Nineteen entries were added, 20–38**, one per group of
genuine deviations the phase ledgers recorded and §10 did not, three of them MUST clauses unmet on a stated domain
(20–22). No row's verdict is *contradicted*: where a sentence misdescribed the code, the chapter was wrong and the
code right, and no live defect was found. The rows below were re-derived on that date; rows 20–38 are new.
...
| 2 | The canonical body is a duck type, not a nominal interface | SEAM-3, BODY-1, BODY-35, HTTP-36 | as-built: confirmed (re-audited 2026-09-25; the entry now carries 3b's `P3-15`: the body slot is typed `Dexpace::Body?` in `gems/dexpace-core/sig/dexpace/http/request.rbs`) -- `Dexpace::IO::BufferedSource.over` (`gems/dexpace-core/lib/dexpace/io/buffered_source.rb:91`) takes no ownership, measured: closing the source leaves the upstream open, where `.wrapping` (`:48`) closes it; every body yields BINARY chunks from `#each` (`gems/dexpace-core/lib/dexpace/http/body.rb`) |
...
| 4 | Cancellation is cooperative; the orphaned-response close moves to the producer | SEAM-13, SEAM-30, XCUT-1–XCUT-3, CFG-17, CFG-20, CFG-21, RETRY-23, TRANSPORT-3, ASYNC-5, ASYNC-4, SEAM-18, RECOV-11 | as-built: confirmed, narrower (re-audited 2026-09-25; the entry now carries 2's `P2-4` and 4b's `P4-17`: `Dexpace::Bridge::SyncOver` raises `Dexpace::CancelledError`, and no RECOV-11 re-assert exists because the token is a latch) -- `Dexpace::Async::Completer#fulfil` closes an undelivered result (`gems/dexpace-core/lib/dexpace/async/completer.rb:96`); the socket-closing hooks are the transports' (`gems/dexpace-transport-net_http/lib/dexpace/transport/net_http/response_pump.rb:120`, `gems/dexpace-transport-async_http/lib/dexpace/transport/async_http/exchange.rb:134`) and the pool owns no socket, so the mitigation names a transport, not an adapter (C12, applied to entry 5 on 2026-09-25) |
...
| 9 | SEAM-10's multi-loader de-duplication is vacuous and is replaced by a version-skew guard | SEAM-10, SEAM-18 | as-built: confirmed (re-audited 2026-09-25; the entry now carries 8b's `P8-21`: `Dexpace::Async::Thread::REQUIRED_CORE` is checked directly at require time in `gems/dexpace-async-thread/lib/dexpace/async/thread.rb`, there being no executor registry) -- vacuous with a replacement: `Dexpace::Registry#register(key, factory, core:)` (`gems/dexpace-core/lib/dexpace/registry.rb:123`) is the version-skew guard; `gates:single_instance` green |
| 10 | Runtime encapsulation of models is partially unachievable | HTTP-2/SEAM-29, HTTP-4, HTTP-7, HTTP-17, HTTP-18, IO-28/BODY-37, XCUT-18 | as-built: confirmed, narrower -- `Dexpace::Request.send(:new, ...)` bypasses `.build` but not validation: it runs the validating `#initialize` (`gems/dexpace-core/lib/dexpace/http/request.rb:51`), so a GET with a body raises HTTP-7 and a non-`Headers` headers value is refused (measured on 4.0.6 and 3.2.11, and re-measured 2026-09-25); the residual holes are `Dexpace::Request.allocate`, which yields a Request whose four members are nil, and duck typing, and the mitigation for both is that both transports re-validate header names and outbound values at the wire (`gems/dexpace-transport-net_http/lib/dexpace/transport/net_http/request_mapper.rb:58`, `gems/dexpace-transport-async_http/lib/dexpace/transport/async_http/request_mapper.rb:50`) (C19, applied 2026-09-25) |
...
| 12 | One stream-ownership rule for bodies, resolving a reference inconsistency | BODY-8, SEAM-3, SEAM-20, SEAM-21, IO-6 | as-built: confirmed, narrower -- a body closes only what it opened (`gems/dexpace-core/lib/dexpace/http/body/stream_body.rb`), ownership-on-wrap is `Dexpace::IO::BufferedSource.wrapping`'s, a codec closes nothing (`gems/dexpace-serde-json/lib/dexpace/serde/json/codec.rb`); §10.12 attributed the I/O rule to SEAM-3 where the live ID is IO-6 (C3, applied 2026-09-25) |
| 13 | The serde seam ships four encode profiles, two of which are one Ruby type | SEAM-20, SERDE-4 | as-built: confirmed, narrower -- all four profiles ship (`gems/dexpace-serde-json/lib/dexpace/serde/json/codec.rb:145`), `dump_string` tagged UTF-8 and `dump_bytes` ASCII-8BIT (measured); each takes the value first, which §10.13 did not spell (C18, applied 2026-09-25); narrower again by 7a's `P7-5`, now in the entry -- the buffer profile takes a mutable BINARY `String` only |
...
| 17 | The interruptible sleep is a cancellable queue wait, not `Kernel#sleep` | CFG-15, CFG-17, CFG-18, RETRY-26, XCUT-3, XCUT-13, ASYNC-15 | as-built: confirmed, narrower (corrected 2026-09-25: the entry's last clause, that the queue wait "lets §3.7's graceful close wait without parking a cancelled caller", described no code -- `Dexpace::Async::Thread::Pool#close` takes no token and drains within `shutdown_timeout`, `gems/dexpace-async-thread/lib/dexpace/async/thread/pool.rb`, 8b's `P8-24`; the clause is replaced in place, `C21`) -- `Dexpace::Clock#sleep` is a per-call queue pop woken by the token (`gems/dexpace-core/lib/dexpace/clock.rb:113`); phase 10 refused NaN and Complex in its guard and in `Dexpace::Async.delay` |
| 18 | Platform-constant substitutions where Ruby has no constant | IO-9, BODY-32, SSE-11, RECOV-34, SSE-19, RETRY-16, RETRY-18 | as-built: confirmed, narrower -- `Dexpace::IO::MAX_MATERIALIZED_BYTES` (`gems/dexpace-core/lib/dexpace/io.rb:27`), `Dexpace::SSE::MAX_RETRY_MS` (`gems/dexpace-core/lib/dexpace/sse.rb:58`), `Dexpace::Resilience::Policy::MAX_DURATION_NANOSECONDS` (`gems/dexpace-core/lib/dexpace/resilience/policy.rb:77`); 7b added `Dexpace::SSE::MAX_LINE_BYTES` and `Dexpace::SSE::MAX_EVENT_BYTES`, which the entry did not list (C15, applied 2026-09-25); the entry now also carries 5a's `P5-56` (the ceiling is read per call through `Dexpace::IO.max_materialized_bytes`), 3a's `P3-4` and 6a's `P6-61` (the pacing parser's fifteen-digit, 64-byte bound, `gems/dexpace-core/lib/dexpace/resilience/pacing_parsers.rb`) |
...
| 20 | SERDE-27's no-materialisation clause is not satisfied by the shipped JSON codec | IO-9, SERDE-12, SERDE-27 | as-built: confirmed as admitted, added 2026-09-25 from 7a's `P7-1` -- `Dexpace::Serde::JSON::Codec#load` drains the source into one String under `Dexpace::IO.max_materialized_bytes` (`gems/dexpace-serde-json/lib/dexpace/serde/json/codec.rb`), while `Dexpace::Serde::DecodingHandler` hands it the unread source (`gems/dexpace-core/lib/dexpace/serde/decoding_handler.rb`); also `docs/first-release.md` § Unsatisfied MUSTs |
| 21 | TRANSPORT-14's header-name clause is unreachable on the asynchronous transport | TRANSPORT-14, TRANSPORT-27 | as-built: confirmed as admitted, added 2026-09-25 from 8c's `P8-38` -- `WAIVED = %w[TRANSPORT-14 TRANSPORT-27]` with the measured "unreachable here" test in `gems/dexpace-transport-async_http/test/dexpace/transport/async_http/conformance_test.rb`; the failure is a retryable `Dexpace::TransportError`; the TRANSPORT-27 half had no ledger row |
| 22 | HTTP-45's no-pinning clause holds for threads, not for a second fiber of one thread with no scheduler | BODY-22, HTTP-45 | as-built: confirmed as admitted, added 2026-09-25 from 3b's `P3-27` and `P3-28` -- `Dexpace::TypedResponse#value` (`gems/dexpace-core/lib/dexpace/http/typed_response.rb`) and `Dexpace::ResponseLoggingBody`'s drain (`gems/dexpace-core/lib/dexpace/http/body/response_logging_body.rb`) wait on a `Thread::ConditionVariable`, which parks the carrier thread when no `Fiber.scheduler` is installed |
| 23 | The orchestrator's "every throwable" is every StandardError | PIPE-30, RECOV-2, RECOV-8 | as-built: confirmed, added 2026-09-25 from 4b's `P4-19` and 4c's `P4-52` -- `Dexpace::Recovery::Orchestrator#call` re-raises `Dexpace::OutcomeError` and converts `::StandardError` alone (`gems/dexpace-core/lib/dexpace/recovery/orchestrator.rb`) |
| 24 | Retryable throwables are classified by capability, not by I/O type | RECOV-17, RETRY-2, XCUT-4, XCUT-6 | as-built: confirmed, added 2026-09-25 from 6a's `P6-4` -- `Dexpace::Resilience::Policy.throwable_retryable?` asks `#retryable?` over `Dexpace.each_cause` (`gems/dexpace-core/lib/dexpace/resilience/policy.rb`); both transports wrap in `Dexpace::TransportError` |
| 25 | The asynchronous transport runs only inside a reactor, and its response does not outlive that reactor | TRANSPORT-21, TRANSPORT-25 | as-built: confirmed, added 2026-09-25 from 8c's `P8-39` and `P8-94` -- with no `Async::Task` current the adapter fails the future with `Dexpace::SeamError` (`REACTOR_MESSAGE`, `gems/dexpace-transport-async_http/lib/dexpace/transport/async_http/adapter.rb`) |
| 26 | A borrowed Net::HTTP is used verbatim: one origin, serialised calls, no per-call timeout | TRANSPORT-15, TRANSPORT-19, TRANSPORT-5, XCUT-22 | as-built: confirmed, added 2026-09-25 from 8a's `P8-6`, `P8-15` and `P8-53` -- the borrowing call refuses `options.timeout` and a foreign origin and takes a one-permit `SizedQueue` (`gems/dexpace-transport-net_http/lib/dexpace/transport/net_http/adapter.rb`, `Dexpace::Transport::NetHTTP`) |
| 27 | ASYNC-18's delay blocks no caller and no worker, but one timer thread per pool | ASYNC-18 | as-built: confirmed, added 2026-09-25 from 8b's `P8-25` -- `Dexpace::Async::Thread::Pool#delay` schedules onto one lazily spawned timer thread whose `#cancel` removes the entry (`gems/dexpace-async-thread/lib/dexpace/async/thread/timer.rb`) |
| 28 | CFG-18's delay settles with true and refuses to block | CFG-18, SEAM-16 | as-built: confirmed, added 2026-09-25 from 5a's `P5-9` and `P5-52` -- `Dexpace::Async.delay` settles with `ELAPSED = true` and raises `Dexpace::SeamError` with no scheduler (`gems/dexpace-core/lib/dexpace/async/delay.rb`) |
| 29 | Two clauses are vacuous by a false antecedent: CFG-19's unwrap and PAGE-15's wrapping | CFG-19, NFR-4, PAGE-15 | as-built: confirmed as vacuous, added 2026-09-25 from 5a's `P5-11` and 7c's `P7-1` -- no unwrap exists and `Dexpace::Async::Completer` delivers the caller's own object (`gems/dexpace-core/lib/dexpace/async/completer.rb`); a page close failure is raised as itself (`gems/dexpace-core/lib/dexpace/page/closing.rb`); design §12 lists both (`C13`) |
| 30 | Where OBS-11's userinfo redaction meets OBS-16's "returned verbatim", OBS-11 wins | HTTP-19, OBS-11, OBS-14, OBS-16 | as-built: confirmed, added 2026-09-25 from 5b's `P5-100` and `P5-107` -- `Dexpace::Instrumentation::Redactor` scrubs every `//`-authority's userinfo on its surgery route (`gems/dexpace-core/lib/dexpace/instrumentation/redactor.rb`) |
| 31 | The diagnostic context restores, snapshots and folds with three stated narrowings | OBS-10, OBS-23, OBS-24, XCUT-20 | as-built: confirmed, added 2026-09-25 from 5c's `P5-49`, `P5-72` and 5b's `P5-22`, `P5-39`, `P5-97` -- `Dexpace::Instrumentation::Diagnostics` compacts and shallow-freezes its snapshot and skips `RESERVED_PREFIX` keys (`gems/dexpace-core/lib/dexpace/instrumentation/diagnostics.rb`); the restore is `Fiber[k] = prior` in `gems/dexpace-core/lib/dexpace/instrumentation/scope.rb` |
| 32 | Trace identity follows the flavour, and the no-op tracer is shared across operations | CTX-15, OBS-25, OBS-26, OBS-27, OBS-29 | as-built: confirmed, added 2026-09-25 from 4a's `P4-7` and 5c's `P5-43` -- `Dexpace::Instrumentation::TraceIdFlavour` carries its own invalid id (`gems/dexpace-core/lib/dexpace/instrumentation/trace_id_flavour.rb`); `Dexpace::Instrumentation::NO_TRACER_FACTORY` answers `Dexpace::Instrumentation::NO_TRACER` every call |
| 33 | A redirect predicate overrides the follow decision but not the hop cap, and an explicit default port is not kept | AUTH-29, REDIR-13, REDIR-17, REDIR-18, REDIR-19, REDIR-20, REDIR-21, REDIR-23, REDIR-8 | as-built: confirmed, added 2026-09-25 from 6b's `P6-91` and `P6-96` -- `Dexpace::Redirect::Step#plan` checks the cap after the predicate (`gems/dexpace-core/lib/dexpace/redirect/step.rb`); the default-port residue is pinned in `gems/dexpace-core/test/dexpace/redirect/step_test.rb` |
| 34 | Four authentication clauses yield to the port's other rules | AUTH-11, AUTH-16, AUTH-22, AUTH-23, AUTH-31, AUTH-36, AUTH-37, HTTP-18 | as-built: confirmed, added 2026-09-25 from 6c's `P6-2`, `P6-5`, `P6-7` and `P6-76` -- `Dexpace::Auth::AsyncBearerStamper`, `Dexpace::Auth::ChallengeHandlerChain`, `Dexpace::Auth::Step` and `Dexpace::Auth::DigestHandler` (`gems/dexpace-core/lib/dexpace/auth/`) |
| 35 | Two pagination clauses are met by stated readings | PAGE-10, PAGE-16, PAGE-19 | as-built: confirmed, added 2026-09-25 from 7c's `P7-5`, `P7-117` and `P7-6` -- `Dexpace::Page.next_request_from` ends on a raw same-document target (`gems/dexpace-core/lib/dexpace/page.rb`); `Dexpace::Page::CursorStrategy` calls its extractor once per page (`gems/dexpace-core/lib/dexpace/page/cursor_strategy.rb`) |
| 36 | SERDE-24's round-trip holds on a stated precision domain | SERDE-24 | as-built: confirmed, added 2026-09-25 from 7a's `P7-8` -- `Dexpace::Serde::Instant` encodes with `#iso8601(6)` (`gems/dexpace-core/lib/dexpace/serde/instant.rb`), both halves pinned in `gems/dexpace-core/test/dexpace/serde/instant_test.rb` |
| 37 | Three model clauses take a Ruby shape | BODY-9, HTTP-1, HTTP-34, HTTP-38, HTTP-9 | as-built: confirmed, added 2026-09-25 from 1's `P1-10`, `P1-14` and 3b's `P3-16` -- `Dexpace::Method::IDEMPOTENT` (`gems/dexpace-core/lib/dexpace/http/method.rb`), `Dexpace::RequestOptions`' String-only tags (`gems/dexpace-core/lib/dexpace/http/request_options.rb`), `Dexpace::StreamBody`'s `pos`/`seek` probe (`gems/dexpace-core/lib/dexpace/http/body/stream_body.rb`) |
| 38 | Two defects are detected as far as Ruby lets them be | BODY-11, BODY-17, PIPE-15, PIPE-5 | as-built: confirmed as admitted, added 2026-09-25 from 4c's `P4-33`, `P4-37` and 3b's `P3-21`, `P3-28` -- `Dexpace::Pipeline::Cursor`'s unsynchronised single-use latch (`gems/dexpace-core/lib/dexpace/pipeline/cursor.rb`), one `Dexpace::PipelineError` class, and `Dexpace::FileBody`'s documented abandoned-`#each` residue (`gems/dexpace-core/lib/dexpace/http/body/file_body.rb`) |
...
amendments in eleven notes** (as of 2026-09-13; phase 10's table above completes the set at nineteen). Phase 10's design numbers them `C1`–`C13` and its plan, Task 17 writes each
...
is **`C1`-`C19`**: the thirteen of phase 10's design, `C14` (filed by phase 4b in `docs/first-release.md`
and missing here until now), and five the as-built audit added (`C15`-`C19`). The notes below this table
stand as each amendment's measured record; `C2`, `C9` and `C14`-`C19` had none and are written in the
table alone. Its closing condition is the `docs/first-release.md` blocker "The fourteen recorded
corrections ... applied", which now enumerates all nineteen and names the one further human act this set
...
**Applied 2026-09-25, by a human, and the blocker answered.** Each of `C1`-`C19` was re-verified against as-built
source before it was applied, and each was found correct; every one is applied in place in the chapter it names
with a dated, bracketed note that quotes the sentence it replaced and cites the as-built file, so no frozen
sentence was silently overwritten. Two were narrowed while applying: `C19`'s "every model's `#initialize`
validates" holds for every HTTP domain model but not for `Dexpace::Pipeline::Entry`, whose validation lives in
`.build` alone (said in §4's note), and `C8`'s closure was re-measured against the bundle this tree resolves
(`net-http` 0.9.1, `async` 2.46.0, `async-http` 0.105.0, `io-event` 1.22.1), with the same counts. `C11` is applied
to appendix C's two rows as a dated note in the text cell, in the recommendation idiom the amendment names, and
touches no chapter of the specification. The reconciliation found six more sentences of the same kind — the rule
right, the mechanism sentence wrong about the shipped code — and applied them the same way, as `C20`-`C25` below.
The consolidation half is done too: design §10 now carries entries 20-38, and the audit table above has a row for
each. **This section is now a record, not a holding area**: the dated notes below are the measured history of each
amendment, and a new finding against a frozen chapter lands here only until the next deliberate amendment.
...
| C19 | §4 (`04-…md:45`): "`Req.send(:new, ...)` reaches the generated constructor anyway"; §10 item 10 (`10-…md:78-79`): "`private_class_method :new` is bypassable by `send` by design" | "`send(:new, ...)` bypasses `.build` but not validation -- every model's `#initialize` override validates and calls `super`, so it re-runs HTTP-4, HTTP-7 and the member type checks; what remains open is `.allocate`, which yields an instance whose members are all nil, and duck typing" | `Dexpace::Request#initialize` (`gems/dexpace-core/lib/dexpace/http/request.rb:51`); `Request.send(:new, ...)` with a GET and a body raises HTTP-7, and `Request.allocate` returns all-nil members, measured on 4.0.6 and 3.2.11 (phase 10's review round 0, R0-6) |
| C20 | §10's opening claim ("With two named exceptions ... none narrows a MUST-level correctness guarantee"; "Nothing new appears here"), §10.5's "the only entries in this catalogue that are not mechanism substitutions", and §12's two list heads, its `TRANSPORT` row's `TRANSPORT-30` deferral and its MUST-level summary | a dated correction naming the three MUST clauses unmet on a stated domain (§10.20-§10.22) and the four stated edges (§10.6, §10.26, §10.33, §10.34), and `TRANSPORT-30` as shipped | `gems/dexpace-serde-json/lib/dexpace/serde/json/codec.rb`, `gems/dexpace-transport-async_http/test/dexpace/transport/async_http/conformance_test.rb` (`WAIVED`), `gems/dexpace-core/lib/dexpace/http/typed_response.rb`; `PROXY_LIMITATION_EVENT` in `gems/dexpace-transport-net_http/lib/dexpace/transport/net_http.rb` (applied 2026-09-25) |
| C21 | §10 item 17's closing clause, "is what lets §3.7's graceful close wait without parking a cancelled caller", and §3.7's "the wait itself performed through §8.3's cancellable queue wait and a bounded deadline" | the close is bounded by the pool's construction-time `shutdown_timeout` and takes no token; no close path calls `Clock#sleep` | `Dexpace::Async::Thread::Pool#close` (`gems/dexpace-async-thread/lib/dexpace/async/thread/pool.rb`, 8b's `P8-24`) (applied 2026-09-25) |
| C22 | §3.4: "the round-trip **SERDE-24** requires holds by construction" | add "for every `Time` whose sub-second part is a whole number of microseconds" | `Dexpace::Serde::Instant` truncates through `#iso8601(6)` (`gems/dexpace-core/lib/dexpace/serde/instant.rb`, 7a's `P7-8`) (applied 2026-09-25) |
| C23 | §3.4's "`JSON` itself is stateless and the configuration is a frozen options hash", §7.3's `SERDE-26` sentence, §11.18's "`JSON` is stateless" and §12's `SERDE` row -- all written against json 2.9.1 | each codec instance owns a private `::JSON::Coder` built from options, and no caller engine is accepted, so SERDE-26 is satisfied literally | `gems/dexpace-serde-json/lib/dexpace/serde/json/codec.rb` (7a's `P7-4`) (applied 2026-09-25) |
| C24 | §7.2: the SSE machine runs "over §3.1's line-reading primitive" | it runs over its own byte-level reader, because §3.1's primitive keeps a lone CR as content under IO-14 | `Dexpace::SSE::LineReader` over `#getbyte` (`gems/dexpace-core/lib/dexpace/sse/line_reader.rb`, 7b's `P7-20`) (applied 2026-09-25) |
| C25 | §9's `NFR-10` row: `required_ruby_version = ">= 3.2"` "in every gemspec" | every gemspec but `dexpace-transport-async_http`'s, which isolates `async-http`'s `>= 3.3` floor as NFR-10's own isolated unit | `VERSIONS`' `ruby floor:dexpace-transport-async_http 3.3` row and `gems/dexpace-transport-async_http/dexpace-transport-async_http.gemspec` (8c's `P8-36`) (applied 2026-09-25) |
...
permanent second ledger. Every note above was folded on 2026-09-25, when `C1`-`C25` were applied; the notes stay
as the record of what each amendment measured. See `docs/README.md`'s "Frozen means frozen" for why routine work does
