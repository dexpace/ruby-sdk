...
| 10 | Runtime encapsulation of models is partially unachievable | HTTP-2/SEAM-29, HTTP-4, HTTP-7, HTTP-17, HTTP-18, IO-28/BODY-37, XCUT-18 | as-built: confirmed, narrower -- `Dexpace::Request.send(:new, ...)` bypasses `.build` but not validation: it runs the validating `#initialize` (`gems/dexpace-core/lib/dexpace/http/request.rb:51`), so a GET with a body raises HTTP-7 and a non-`Headers` headers value is refused (measured on 4.0.6 and 3.2.11); the residual holes are `Dexpace::Request.allocate`, which yields a Request whose four members are nil, and duck typing, and the mitigation for both is that both transports re-validate header names and outbound values at the wire (`gems/dexpace-transport-net_http/lib/dexpace/transport/net_http/request_mapper.rb:58`, `gems/dexpace-transport-async_http/lib/dexpace/transport/async_http/request_mapper.rb:50`) (C19) |
