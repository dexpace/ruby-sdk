...
|---|---|---|---|
| 1 | `rubocop` | `NFR-7`, `NFR-13`: style and lint findings are fatal; the seven custom cops mechanise `CLAUDE.md`'s ban list — phase 2's `Dexpace/QualifiedCoreConstant` is the seventh, refusing a bare `Thread`/`Queue`/`Mutex`/`SizedQueue`/`ConditionVariable`/`JSON`/`IO` anywhere inside `module Dexpace` in every gem's `lib/` (phase 3a widened it from two namespaces and one gem, and taught it that `module Dexpace; module IO` declares the shadow rather than referring to Ruby's) | any finding at `--fail-level=convention` |
| 2 | `cops:test` | the custom cops themselves — a cop that silently stops matching stays silent otherwise; the four ban cops see a safe-navigation call (`@thread&.kill`, `name&.downcase(:turkic)`) and the interrupt cop a block-pass (`threads.each(&:kill)`, `(&:exit)`) as well as the plain spelling | `.rubocop/test/cops_test.rb`: 63 rejected and 37 accepted sources, the seventh cop's in two nested classes of their own — phase 2's ten and seven, phase 3a's eight and ten |
