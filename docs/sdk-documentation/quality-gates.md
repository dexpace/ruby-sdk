...
phase 0 (`docs/work/mvp/phase0/`), extended by phase 7b (`gates:serde_boundary`), by phase 9
(`gates:cause_walk`, `gates:bounded_map`, `gates:seam_names`) and by phase 10 (`gates:ledger_audit`,
`gates:spdx_rbs`, `gates:sole_parse`); the design it implements is
...
`bundle exec rake` runs all twenty-four, in the order below, and stops at the first failure. Each is
...
| 13 | `gates:ledger_audit` | `NFR-17`, and phase 10's `P10-2` made mechanical: `docs/deviations.md` still describes design §10 -- register row N's title is a word subsequence of §10 entry N's bolded subject, its `IDs touched` column (every range expanded) EQUALS the IDs entry N names, and a row carrying a verdict cites at least one `gems/…` path that exists and names only `Dexpace::` constants that are defined, with core and every adapter the running Ruby's floor admits loaded first. A row citing only `docs/…` fails: a design document is never evidence | `test/fixtures/gates/ledger/`: a row missing an ID, two swapped rows, a verdict citing only a document, a verdict naming `Dexpace::Completer`; `ledger/workspace` for the task |
| 14 | `gates:spdx_rbs` | `NFR-13`, `NFR-3`: line 1 of every shipped `sig/**/*.rbs` is `# SPDX-License-Identifier: MIT` (the RuboCop cop cannot parse RBS), and every shipped signature declares something -- read as the third element of `RBS::Parser.parse_signature`, because the Buffer it returns first survives a `flatten` | `test/fixtures/gates/spdx_rbs/`: no header, the header on line 3, a comment-only file; `spdx_rbs/workspace` for the task |
| 15 | `gates:sole_parse` | `NFR-6`: `AstScan.parse` -- the one `$VERBOSE = nil` window -- is the only `RubyVM::AbstractSyntaxTree.parse_file` in `tools/`, `tasks/`, `test/gates/` and `.claude/skills/`, direct or reflective (`send(:parse_file, …)`, `public_send("parse_file", …)`), exempting the owner METHOD and three test contrasts listed with reasons; `Prism.parse_file` returns its diagnostics and is not the hazard. Stated gap: a receiver or a method name held in a variable | `test/fixtures/gates/sole_parse/`: a direct call, two reflective ones, an owner file whose second method calls it; `sole_parse/workspace` for the task |
| 16 | `gates:clean_bundle` | `SEAM-1`, `NFR-1`, `NFR-10`: each gem loads inside a scratch bundle holding only itself (and core, for an adapter); Bundler refuses every undeclared gem | a `require` of a bundled gem nobody declared — `logger` on Ruby 4.0 |
| 17 | `gates:rbs_surface` | `NFR-11`: no constant outside `Dexpace::` and a stdlib allowlist in any public signature. The signatures are loaded as one set and every name resolved against it, the way `rbs validate` does, so a relative `Headers` inside `module Dexpace` and a type variable `T` are not leaks; every type tree is then walked, so a leak inside an optional, a union, a tuple, a record, a proc or a type argument is seen | `Async::Task` as a return type, superclass, mixin (name or type argument), type alias, class or module alias, class- or method-level generic bound, or anywhere inside a type; `DexpaceX::Thing`; a set that does not resolve |
| 18 | `gates:sig_diff` | `NFR-4`: every public declaration in `sig/`, parsed and flattened to one line per overload with `private` honoured, against the last `v*` tag, per gem; a removed **or narrowed** declaration fails | a declaration or file gone since the tag, a dropped overload, a method moved under `private` (vacuous, by stated design, until the first tag) |
| 19 | `gates:surface_snapshot` | `NFR-4`: the runtime constant and method tree each gem adds to `Dexpace`, walked from the root and not from the gem's own constant — so an export placed *beside* a namespace (`Dexpace::Transport::Shared`, a method on `Dexpace::Transport`) is in the manifest — `Data.define` readers (those still public on the model) and singleton methods included, a `private_constant` excluded, one line per method, against `test/fixtures/surface/<gem>.txt`; an adapter's manifest is what its entry file adds with core already loaded, so core's lines are in core's manifest only | any export not in the manifest, inside or beside the gem's namespace; `rake surface:regenerate` is the deliberate act |
| 20 | `gates:single_instance` | design §2.4: one resolved path per core file; `Dexpace::VERSION` equals the gemspec. A second copy of core cannot finish loading once core holds `Data.define` models — Ruby refuses the re-opened class with `superclass mismatch` — and the gate reports that in the same terms, beside the features loaded twice before it | one core feature resolved from two directories; a version literal that disagrees |
| 21 | `gates:versions` | `NFR-14`, `NFR-10`: `.ruby-version`, the CI matrix, six gemspec versions and floors and six `VERSION` literals all agree with `VERSIONS` | any consumer that drifted; a gemspec that does not load |
| 22 | `gates:reproducible` | `NFR-12`: each gem built twice under a fixed `SOURCE_DATE_EPOCH` is byte-identical | a gemspec whose build depends on anything but its inputs |
| 23 | `yard` | styleguide 14.1: every public object documented | `yard stats --list-undoc` below 100% |
| 24 | `bundler_audit` | a known CVE in the resolved bundle | `bundler-audit check --update` |
...
DEXPACE_GATE_ROOT=test/fixtures/gates/ledger/workspace bundle exec rake gates:ledger_audit
DEXPACE_GATE_ROOT=test/fixtures/gates/spdx_rbs/workspace bundle exec rake gates:spdx_rbs
DEXPACE_GATE_ROOT=test/fixtures/gates/sole_parse/workspace bundle exec rake gates:sole_parse
