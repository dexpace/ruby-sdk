...
| `registers` | An aggregate `## Open Findings` / `## Deferred Items` / `## Open Items` section living inside a spec, design or plan document instead of in `docs/open-items.md` — or, for postponed work, in the owning plan task or `docs/first-release.md` |
| `citations` | Two namespaces. **Live:** an `OI-<n>` citation with no matching entry in `docs/open-items.md`, the only register left; no-ops when that file does not exist. **Retired:** every `DEF-<n>` citation, anywhere — the deferral register `docs/deferred-items.md` was retired on 2026-09-13 with every row resolved, and a deferral now names its owner, a plan task or a `docs/first-release.md` entry. A retired prefix never no-ops, because a missing file is exactly what would hide the leftovers; if the old file itself lingers, that is one finding, not one per row |
