...
# deliberately malformed fixtures, the bounded-time measurement that discharges the no-regexp
# house rule by measurement rather than by claim, and the per-pattern timeout pinned on each of
# the eight scanner patterns. Split into nested cases under Metrics/ClassLength.
class DexpaceAuthChallengesTest < DexpaceTestCase
  Challenges = Dexpace::Auth::Challenges

  # The one helper both cases share.
  module Fixtures
    def shapes(value)
  ...
  # AUTH-13: leniency, the bounded-time measurement and the per-pattern timeout pin.
  class LeniencyTest < DexpaceTestCase
    include Fixtures
    ...
    test "AUTH-13: a malformed value recovers to the next top-level comma, earlier params kept" do
    ...
    # The measurement above holds without a timeout because the classes are linear; the house
    # rule (design §4, §6.3) is pinned as a property of each pattern too, as http_date_test.rb
    # pins CFG-31's grammar, so removing one `timeout:` is a red test and not a silent regression
    # (review round 0's R0-2).
    test "every scanner pattern is a private, frozen Regexp compiled with a per-pattern timeout" do
      names = %i[TOKEN TOKEN68 SEPARATORS BOUNDARY EQUALS SPACES OWS QUOTE]

      names.each do |name|
        pattern = Challenges.const_get(name)

        assert_kind_of(Regexp, pattern, name.to_s)
        refute_nil(pattern.timeout, name.to_s)
        assert_predicate(pattern, :frozen?, name.to_s)
        refute_includes(Challenges.constants, name)
      end
    end
  end
end
