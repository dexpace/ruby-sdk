...
class ThreadTest < DexpaceTestCase
  ...
  # `Dexpace::Async::Shared` -- is outside the gem's namespace and inside nothing this suite
  ...
  test "the VERSION matches the gemspec this gem is built from" do
    gemspec = File.expand_path("../../../dexpace-async-thread.gemspec", __dir__)
  ...
  test "defines nothing outside the Dexpace namespace" do
    ...
    assert_equal(%i[VERSION], Dexpace::Async::Thread.constants(false).sort)
  end
