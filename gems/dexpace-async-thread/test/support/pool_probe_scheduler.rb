...
# The minimal Fiber::Scheduler the delay suite's two-fibers-on-one-thread liveness check needs:
# a fiber that blocks on a Thread::Queue#pop(timeout:) is PARKED with its deadline and the
# carrier thread is free. A copy of phase 5a's ParkingScheduler (gems/dexpace-core/test/support/
# parking_scheduler.rb) under a gem-unique name, because core's fakes stay in core (8a's decline)
# and `rake test:gems` loads every gem's suite into one process; core's ProbeScheduler is a
# different, nested-loop double and is not what this copies.
#
# A hook must NEVER sleep on the carrier thread: #block and #kernel_sleep are called from inside
# a non-blocking fiber, and Kernel.sleep there re-enters #kernel_sleep until SystemStackError.
# Both park the current fiber with a deadline and yield; the only real sleep is in #run_loop,
# on the carrier thread's root fiber. #close is what the interpreter calls when the scheduler's
# thread ends, so a suite drives the loop by joining that thread.
#
# #fiber_interrupt is defined because Ruby 4.0.6 alone warns "Scheduler should implement
# #fiber_interrupt" without it, and the shared test case turns a warning into a failure
# (3.2.11, 3.3.12 and 3.4.10 are silent).
class PoolProbeScheduler
