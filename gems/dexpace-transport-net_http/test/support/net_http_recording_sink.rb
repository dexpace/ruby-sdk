...
# A logging sink that records every write, for the tests that assert what the adapter logs and
# -- TRANSPORT-30's first embedded MUST -- what it never logs. The gem's own double: core's test
# support is not reachable from an adapter gem's suite, and a double this small is not worth a
# shared home. Every method the facade's duck-typed `_Sink` names, and nothing else.
#
